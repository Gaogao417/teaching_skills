...
- `generate_candidate`
  ...
  - 坐标几何：agent 生成解析对象和 renderer spec 草案。
...
- `compile_renderer_spec`
  ...
  - 包含 `points`、`segments`、`polygons`、`markers`、`labels`、`teaching_focus`。
...
`geometry-render-spec/v1` 是教学渲染层可消费的中间表达，不绑定 Wolfram，也不要求 Notebook/FE rasterize。
