...
为了缩短调试反馈，另有一个可选测试入口，从已经生成的 assignment YAML 开始跑到图片产物：

```text
assignment.yaml 中的 diagram 字段
  -> 收集题目级 diagram jobs
  -> 写 diagram/jobs/<job_id>/diagram-request.json
  -> 运行本仓库内置 scripts/geometry_diagram_workflow
  -> 生成 final_renderer_spec.json
  -> render_geometry_spec.py 输出 rendered/prompt.png 或 rendered/solution.png
  -> diagram_acceptance_report.json 汇总验收
```

这个测试入口不定义 workflow 的业务职责，也不负责结构分析、题目生成、LaTeX 排版或 PDF 编译。调试几何插图时可运行：

```bash
python3 scripts/run_diagram_acceptance.py <assignment.yaml> --jobs 4
```
...
- `route_diagram_type`
  ...
  - 函数图默认走 `wolfram_plot` 或 `coordinate_renderer`，不复用合成几何的点位求解逻辑。

- `generate_candidate`
  - 合成几何：agent 生成 `GeometricScene`。
  - 坐标几何：agent 生成解析对象、坐标点、直线/圆等对象和 renderer spec 草案。
  - 函数图：agent 生成函数表达式、定义域、视窗、坐标轴和关键点/交点对象；Wolfram 负责校验表达式、采样或导出 Plot/ListPlot 图像。

- `solve_candidate`
  - 合成几何：Wolfram 只求解 `GeometricScene` instance，抽取点坐标和几何对象。
  - 坐标几何：确定性解析坐标对象、视窗、坐标轴和标注。
  - 函数图：Wolfram 计算采样点、交点、零点、极值等可选对象，并返回可渲染 `samples` 或直接渲染产物。
...
`geometry-render-spec/v1` 是教学渲染层可消费的中间表达。综合几何不绑定 Wolfram 前端；函数图可以由 Wolfram 计算/采样，最终仍把可审计的 spec 写出来。若选择 `engine: wolfram_plot` 直接导出 PNG/SVG，也必须同时记录足够的 `viewport`、`axes`、`functions`、`objects` 或 `samples` 供 gate 和复核使用。
...
agent extracts functions / viewport / key points
  -> Wolfram validates expression and samples Plot data
  -> compile function_graph geometry-render-spec/v1
  -> Wolfram Plot PNG/SVG or deterministic coordinate renderer outputs image
  -> gate checks axis range, object references, clean/annotated policy
```

重要原则：

- `function_graph` 使用 `engine: wolfram_plot` 或 `engine: coordinate_renderer`，不走 `geometric_scene`。
- prompt 图只画题干给出的函数、点、坐标轴和必要网格；不要用颜色或标注提前泄露判断结论。
- solution 图可以额外标零点、交点、辅助虚线、面积阴影或导数/单调性提示，但必须是教师版或讲解版。
- Mathematica 可以直接 `Plot`/`ListPlot` 导出图片，但仍要保存 `final_renderer_spec.json`，方便后续复核、重渲染和 stale 检查。
