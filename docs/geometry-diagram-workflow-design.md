...
题目信息
  ...
  -> Wolfram/代码渲染图片
  -> 便宜 VLM 或本地 VLM 评价图片是否可用
  -> 若不可用，返回缺陷并补充约束
  -> 最多重试 3 次
  -> 输出 diagram-spec、图片、日志和失败原因
```
...
7. `teaching_skills` 应只负责提出画图需求和消费产物，不应承担 Wolfram、VLM、重试等底层细节。
...
- `diagram_spec` 主要由 agent 生成，尚未从真实 Wolfram instance 坐标抽取。
- 合成几何与坐标几何未分流。
...
- 调用 Wolfram 或确定性 renderer 生成图片。
- 调用便宜 VLM 或本地 VLM 评价图片。
...
- 超时控制和 fail type 分类。
- 从求解结果抽取真实坐标。
- 将 instance 转为 `diagram-spec`。
- 对坐标图执行确定性渲染。
- 输出 PNG/SVG、spec、metrics。
...
- `generate_candidate`
  ...
  - 坐标几何：agent 生成解析对象和 `diagram-spec` 草案。

- `render_candidate`
  - 合成几何：Wolfram 求解并渲染。
  - 坐标几何：确定性 renderer 渲染坐标轴、点、线、曲线、区域。

- `evaluate_image`
  - 调 cheap VLM/local VLM。
  - 返回 `usable`、`score`、`defects`、`suggested_feedback`。
...
- `finalize`
  - 输出最终 `diagram-spec`、图片、scene code、评价结果、attempt logs。
...
- 固定 `DiagramRequest`、`DiagramPackage`、`diagram-spec` 输出。
- 保证调用方可以稳定消费。
...
`diagram-spec` 应是教学渲染层可消费的中间表达，不应绑定 Wolfram。
...
```json
{
  ...
  "points": [
    {"id": "A", "x": 320, "y": 60, "label": "A"},
    {"id": "B", "x": 120, "y": 340, "label": "B"},
    {"id": "C", "x": 520, "y": 340, "label": "C"}
  ],
  "objects": [
  ...
  "annotations": [
    {"type": "equal_tick", "segments": [["A", "B"], ["A", "C"]]},
...
agent extracts analytic objects
  -> deterministic coordinate renderer
  -> VLM checks readability
...
```text
agent 负责表达意图和生成候选
code 负责执行、校验、渲染、评价闭环
skills 负责给 agent 提供领域约束
```

也就是说，GSB 不应变成“固定规则生成器”，而应变成“带工具、带评测、带重试的 agent runtime”。
...
- `math-student-explanation-html` 能消费 `DiagramPackage`。
- `math-adaptive-practice-html` 能为练习题生成或复用图。
- 图失败时有降级显示策略。
...
1. `diagram-spec` 的最终 renderer 是 HTML/SVG/canvas 里的哪一种。
2. 坐标图是否需要支持函数解析式直接绘制。
3. VLM 评价用云端 cheap model 还是本地 model，默认 base_url 如何配置。
4. `max_retries=3` 是否按“3 次修正”解释。
5. teaching HTML 中图片是内嵌 base64，还是使用相对路径。
6. 何时需要从简单 Python loop 升级到 LangGraph。

## 15. 最小可验收版本

MVP 应满足：

- `teaching_skills` 能产生 `diagram-request.json`。
- GSB 能读取 request，并完成合成几何图闭环。
