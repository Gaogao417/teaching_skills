...
> artifacts. For synthetic geometry, the active agent provider is now local
> Codex SDK launched from `scripts/diagram_workflow/geometry_diagram_workflow/`;
> DashScope/OpenAI-compatible model calls are not a fallback path.

## 0. 当前实现与测试入口

当前实现层面的主结论：

- `GeometricScene-Builder` 外部仓库不再是默认运行依赖；其 agentic `workflow.py`、Wolfram 包和 prompt skills 已迁入 `scripts/diagram_workflow/geometry_diagram_workflow/`，其中 prompt skills 使用 `.codex/skills`。
- 模型继续负责写 Wolfram `GeometricScene`，workflow 负责把 `.codex/skills` 作为 Codex SDK `SkillInput` 传给 agent、执行 Wolfram、抽取坐标、编译 renderer spec、渲染和记录日志。
...
3. 新系统不应废弃原 skills 和 tools，而应把 skills 作为 Codex SDK `SkillInput` 传给 agent，并在提示词中要求使用。
4. LangGraph 不是刚需。MVP 可以先用 `Codex SDK + skills + tools + 简单 Python loop` 组织闭环，成本更低，也更贴近现有仓库。
5. 无论使用 Codex SDK 前端、普通 Python loop 还是 LangGraph，几何工作区内的 `.codex/skills` 都应作为 agent 的 skill 输入，而不是手动读取正文再拼进 prompt。
...
### 2.3 当前 Codex SDK subagent 原型

当前原型方向：

```text
workflow.py
  -> agent_runner.py starts Codex SDK subagent
  -> agent_prompt.py gives the full loop contract
  -> subagent writes scene_payload.json
  -> subagent calls workflow.py --action render
  -> subagent calls workflow.py --action compile_spec
  -> subagent calls render_geometry_spec.py
  -> subagent calls workflow.py --action audit
  -> subagent reads preview PNG path and visually inspects it
  -> subagent calls workflow.py --action finalize_round
```

它已经接近新需求，但仍有关键缺口：

- 几何工作区 `.codex/skills` 已通过 `SkillInput` 传给 Codex SDK subagent。
- renderer spec 已由真实 Wolfram instance 坐标和 agent 的 diagram_spec 合并生成。
- 合成几何与坐标几何未分流。
- CLI 是入口，但还没有稳定 Python API。
- deterministic audit 只负责硬门禁；视觉质量由同一 subagent 按 preview PNG 路径自行检查。
...
- Codex SDK 前端可以通过提示词、agent、skills 和 tools 低成本驱动同一套闭环。
- 通过 Codex SDK `SkillInput` 传入 `geometry_diagram_workflow/.codex/skills/<name>/SKILL.md`，并在 prompt 中要求 agent 使用对应 skills。
...
- `attach_codex_skills`
  - 按节点选择 `scripts/diagram_workflow/geometry_diagram_workflow/.codex/skills` 下的 skill 名和路径。
  - 通过 Codex SDK `SkillInput` 交给 agent；prompt 只点名要求使用这些 skills，不拼接正文。
...
无论由 Codex SDK 前端、普通 Python loop 还是 LangGraph 编排，都需要明确告诉 agent 本轮该使用哪些 skills。应实现 skill manifest builder：

```text
skill_input(name)
  -> resolve geometry_diagram_workflow/.codex/skills/<name>/SKILL.md
  -> pass SkillInput(name, path) to Codex SDK
  -> mention the attached skill names in the prompt
```
...
### 方案 A：Codex agent/skill + tools 前端编排
...
- 改动最小，最贴近几何工作区内现有 `.codex/skills` 和本地 workflow tools。
...
- 容易暴露为 CLI、Codex tool 或 Python API。
- 更适合作为 MVP 主线。
...
- 当前阶段会增加框架成本。
- 不能自动替你选择任务需要的 skills，仍需维护节点到 skill 名单的映射。
...
### Phase 1：Codex SDK 前端验证

在 `GeometricScene-Builder` 中完成：

- 新增或整理 agentic diagram workflow skill。
- 增加 Codex SDK agent 调用：生成候选、渲染候选、评价图片、运行一次完整闭环。
- 让 Codex agent 能按提示词执行：
  - 读取题目信息。
  - 使用随请求附上的 GSB skills。
...
2. Codex 视觉评价是否需要长期保留，还是 spec-only 成为默认验收入口。
...
- MVP 不要求 LangGraph；可以由 Codex SDK agent/tool 或简单 Python loop 驱动。
- 最多 3 次修正。
- 成功时输出：
