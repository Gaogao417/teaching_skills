...
% ---------- 包 ----------
\usepackage{geometry}
\geometry{top=0mm,bottom=14mm,left=8mm,right=8mm}
...
已知一次函数 $y = kx + b$ 的图像经过点 $A(0,2)$ 和点 $B(4,0)$。
\begin{eduSubQuestions}
  \item 求这个一次函数的解析式；  \item 若点 $C$ 在该图像上，且点 $C$ 的横坐标为 $2$，求点 $C$ 的坐标；  \item 若直线 $y = kx + b$ 与直线 $y = -x + 4$ 交于点 $M$，求点 $M$ 的坐标。\end{eduSubQuestions}
\end{eduProblemBlock}
...
\begin{eduReadTipBox}
\begin{eduTipList}
  \item 先解决第（1）问，得到解析式，后面两问都要建立在第（1）问的结果上。  \item 这是一道“先求式，再代入，再联立”的递进题。\end{eduTipList}
\end{eduReadTipBox}
...
\begin{eduRouteFlow}
\begin{tikzpicture}
  \node[edu route node] (route0) {由 $A、B$ 两点求解析式};
  \node[edu route node, right=10mm of route0] (route1) {代入 $x=2$ 求点 $C$};
  \node[edu route node, right=10mm of route1] (route2) {联立两条直线求点 $M$};
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{思考引导 / 易错提示}
    \begin{eduNumberList}
      \item 已知两个点，优先想到用待定系数法求 $k,b$。      \item 点 $A(0,2)$ 在 $y$ 轴上，代入后可以直接得到 $b$。      \item 第（2）（3）问不要急着做，先把第（1）问算清楚。    \end{eduNumberList}
  \end{eduExplainLeft}
  \eduColumnDivider
  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{规范讲解}
    \begin{eduNumberList}
      \item 把 $A(0,2)$ 代入 $y = kx + b$，得 $b = 2$。      \item 把 $B(4,0)$ 代入 $y = kx + b$，得 $0 = 4k + 2$，所以 $k = -\dfrac{1}{2}$。      \item 所以一次函数解析式为 $y = -\dfrac{1}{2}x + 2$。    \end{eduNumberList}

    \vspace{1mm}
    \eduSubTitle{后两问如何承接}
    \begin{eduBulletList}
      \item 第（2）问：把 $x = 2$ 代入 $y = -\dfrac{1}{2}x + 2$，得 $y = 1$，所以 $C(2,1)$。      \item 第（3）问：联立 $y = -\dfrac{1}{2}x + 2$ 与 $y = -x + 4$，可求交点 $M$。    \end{eduBulletList}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduBottomGrid}
  \begin{eduSummaryLeft}
    ...
    \begin{eduPlainList}
      \item $y = -\dfrac{1}{2}x + 2$      \item $C(2,1)$      \item $M(4,0)$    \end{eduPlainList}
    \end{eduSummaryBlock}
  \end{eduSummaryLeft}
  \eduSummaryDivider
  \begin{eduSummaryRight}
    ...
    \begin{eduBulletList}
      \item 先做基础问，再做依赖前面结果的后续问。      \item 题干与讲解分离，避免学生一开始就淹没在推导里。      \item 有小问的题，先看递进关系，再决定书写顺序。    \end{eduBulletList}
    \end{eduSummaryBlock}
  \end{eduSummaryRight}
\end{eduBottomGrid}
