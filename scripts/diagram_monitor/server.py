...
def create_app(artifacts_root: str | Path | None = None) -> FastAPI:
    ...
    def job_detail(folder: str, job_id: str) -> dict[str, Any]:
        try:
            return scanner.job_detail(folder, job_id)
        except FileNotFoundError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the local read-only diagram pipeline monitor")
