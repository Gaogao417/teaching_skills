"""Deterministic staging ports (architecture §8.1, naming §8.1).

The stable business name for the post-source-paper pipeline is *staging*, not the
relative-position name "downstream". These stages wrap the existing deterministic
scripts (projector / evidence / expand / materialize / audit / notify). They run
strictly serially — each depends on the previous stage's actual files, so they are
not parallelised in the graph (design §12).
