...
依赖方向（固定不变量）::
    ...
    CLI / deployment config
      -> composition.py            # 唯一选择 adapter 的模块
...
- :mod:`.state`：``WorkflowState`` 只保存小型可序列化状态与 ``ArtifactRef``，不保存
  页图/PDF/模型响应大对象。
- :mod:`.ports`：业务端口（``Protocol``），fake adapter 与真实 adapter 实现同一端口。
- :mod:`.config`：``RuntimeAdapterConfig``，仅 ``composition`` import；state/node 不得
  import。
- :mod:`.composition`：唯一 adapter 选择与装饰（retry/cache/limiter）。
- :mod:`.graph`：``build_graph(deps)``，StateGraph + 两条 interrupt 点。
- :mod:`.cli`：``start / status / resume``。
- :mod:`.artifact_store` / :mod:`.checkpoint` / :mod:`.tracing`：内核支撑。
