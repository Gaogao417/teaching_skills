"""CLI: ``start`` / ``status`` / ``resume`` (design §12).
...
never enter graph state (design §12 last paragraph).
"""
...
def _repo_root() -> Path:
    # The workflow package lives at <root>/scripts/question_transcription/workflow/.
