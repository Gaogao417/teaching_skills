...
立体几何使用 `engine: spatial_renderer` 与 `diagram_kind: spatial_geometry`：三维关系在 Python 层校验，`points3d` 保留到 TikZ compiler，最终按 `textbook_oblique`、`hinge_planes`、`orthographic_3d` 或 `axial_solid` 投影。不得先投成二维 `points` 再交给 TikZ。详细契约见 `references/spatial-geometry.md`。
