...
交互式任务默认由当前主 Agent 负责 scene authoring 和逐步检查，不再启动嵌套
Codex SDK writer/reviewer。先运行 collect 得到每个 job 的 `request.json`，然后对
`geometric_scene` job 按以下三步执行；Wolfram job 必须串行。

### 1. render

主 Agent 按题面原样编写 `rounds/round_0/scene_payload.json`，然后调用：

```bash
./.venv-diagram/bin/python \
  scripts/diagram_workflow/geometry_diagram_workflow/core/workflow.py \
  --action render --request <request.json> --out <job-dir> --round-index 0 \
  --scene-payload <job-dir>/rounds/round_0/scene_payload.json
```

调用结束后立即读取 `render_result.json`，并打开其中的 Wolfram 调试图。检查点、交点、
点序和退化。WL 语法或构型错误最多修一次；第二次仍失败时写出
`needs_human_confirmation`，附失败类型、预览和建议确认的问题，不得继续猜条件。

### 2. preview

render 通过后调用复合 action；它内部固定执行
`compile_spec -> TikZ render -> deterministic audit`：

```bash
./.venv-diagram/bin/python \
  scripts/diagram_workflow/geometry_diagram_workflow/core/workflow.py \
  --action preview --request <request.json> --out <job-dir> --round-index 0 \
  --scene-payload <job-dir>/rounds/round_0/scene_payload.json \
  --render-result <job-dir>/rounds/round_0/render_result.json
```

主 Agent 必须读取 `final_renderer_spec.json`、`audit_result.json` 并打开真实 preview PNG。
等长/平行/直角/角弧的对象目标在 compile 前就必须完整声明；看到实际坐标后，只允许用
`visual_patch.json` 调整标签或文字位置和 marker 样式：
...
./.venv-diagram/bin/python \
  ...
  --scene-payload <job-dir>/rounds/round_0/scene_payload.json \
  --render-result <job-dir>/rounds/round_0/render_result.json \
  --visual-patch <visual_patch.json>
```

visual patch 不得修改点、线、交点、scene code 或 marker 目标。每个 Round 最多两次
preview（首次加一次调整）；仍不通过时停止并请人决定。

### 3. finalize_round

只有 audit 为 `pass` 且主 Agent 已打开最终预览时才能 finalize：

```bash
./.venv-diagram/bin/python \
  scripts/diagram_workflow/geometry_diagram_workflow/core/workflow.py \
  --action finalize_round --request <request.json> --out <job-dir> --round-index 0 \
  --scene-payload <job-dir>/rounds/round_0/scene_payload.json \
  --render-result <job-dir>/rounds/round_0/render_result.json \
  --renderer-spec <job-dir>/rounds/round_0/final_renderer_spec.json \
  --renderer-result <job-dir>/rounds/round_0/renderer_result.json \
  --audit-result <job-dir>/rounds/round_0/audit_result.json
```
...
显式选择无人值守 SDK 后备链路时，禁止把一键脚本当成一个可以沉默等待到结束的黑盒命令。
...
无论使用主 Agent交互链路还是 SDK 后备链路，每个 job 都由唯一 owner 完成
solve/spec/render/audit/visual/finalize，Batch 不再二次调用 TikZ renderer。
脚本内部顺序为：
