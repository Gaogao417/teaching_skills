...
当题目没有指定梯形角度、腰长或高，而教学图需要选择一个稳定版面时，使用以下默认构图质量区间：

- 较长底边两端的两个底角优先落在 $50^\circ$--$70^\circ$，并明确避开直角；
- `较长底边 / 高` 优先接近黄金比例 $\varphi\approx1.618$，通常接受 $1.55$--$1.68$；
- 避免高远大于底边的瘦长梯形，也避免高远小于底边的扁平梯形；
- 两个底角不必相等，除非题设明确给出等腰梯形；可用相邻但不同的角度得到自然、略不对称的教材构型；
- 这些角度和纵横比只能作为 layout-only 约束，不得画角度数值、等腰标记或把它们写进题设条件清单。

例如，若上下底分别为 $4,9$，取较长底边两端底角约 $62^\circ,69.8^\circ$，则
`下底 / 高` 约为 $1.619$，接近黄金比例，同时不会暗示直角或等腰。
...
梯形还要单独检查较长底边两端底角与 `较长底边 / 高`。若题设没有给角度，优先使用“四边形与多边形”一节的梯形质量区间；不要仅凭 `Convex` 和 `Horizontal` 就接受一个过扁、过高或近似直角的随机实例。
