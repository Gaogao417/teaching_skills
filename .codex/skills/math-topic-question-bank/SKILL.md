...
- 教师版在文字解答后按顺序渲染题目级 `source_solution_images`；学生版派生时必须移除
  答案、解答、校注和官方解答图。`source_solution_images` 是题目级解答图片列表，
  `word_evidence.official_solution` 是不可变整页来源凭证，两者分区存储。
...
- 解析区会在对应文字步骤下只读显示 `solution_steps[].diagram_col` 的解析图（旧
  数据逐步教学图）；手工补充解答图不再绑定解题步骤，改在独立的“解答图”图库按
  题目管理，编辑目标为 `official_solution`。完整官方解答截图仍在来源凭证区只读显示。
- staging 的题图和解答图都以可选中槽位显示：点击槽位使其高亮后，直接用
  `⌘V` / `Ctrl+V` 粘贴教师手工截图；悬浮图片时右上角 `×` 可把图片移出槽位。
  题图缺失时显示 `+` 供教师补图；解答图图库为空时同样显示 `+`，可连续追加多张。
  Review UI 会把手工图保存为新的全图来源证据，同步 assignment 图片引用并刷新
  `content_hash`；删除只移除证据/assignment 引用，不物理删除原图片文件。此前审核
  决定因此自动过期，必须重新核对。`solution_steps[].diagram_col` 保留给既有逐步
  教学图与旧数据，不作为 Review UI 的图片编辑入口。
