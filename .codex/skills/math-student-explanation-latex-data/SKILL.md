...
description: "根据 01-structure-analysis.md 生成学生讲解 assignment.yaml，并在图形 resolve 后直接渲染、检查和打开 TeX。Use when: 已有结构分析，用户要求讲解 YAML/TeX、explanation assignment、讲义内容、知识点复习、专题复习、期末复习、薄弱点讲义或端到端作业补齐讲解阶段。Skip when: 没有结构分析、用户要求独立练习题、只要求几何图，或只要求渲染已有 YAML/PDF。需要配图时按 diagram-slot-contract 声明 diagram_slot；真实出图交给 math-geometry-diagram-renderer。"
---
...
2. 可用 `./.venv/bin/python scripts/model_rules/search_model_rules.py --topic <topic>` 或 relation id 检索模型库；若没有匹配 relation，继续使用结构分析 prose，不阻断讲解生成。
...
12. 使用 `exam-zh-explanation` 模板输出 assignment YAML。输出后必须检查每个 `route.steps[].content_latex`：解答步骤只讲 how，要求简洁、严谨、规范；把讲 why、讲“所以然”、入口追问和易混判断移到 `dual_explanation.side_items` 的小贴士提问中。每条 side item 必须解决一个真实卡点，删除与步骤正文重复的解释。
   - 默认不显示机械的 Step 编号，也不得为没有真实字母小问的题目自行添加 a、b、b1、b2。多路线题只保留一套自然分支名；页面已有路线图时，将仅作为解答数据源的 `route` 设为 `show_navigation: false`，避免重复导航。
   - 分类公式或连续证明只需连续正文、但仍要通过 route step 绑定讲解图时，在 `dual_explanation` 设置 `plain_solution: true`，撤掉动作式 step 标题。
   - 初中几何证明和连续推导默认用 `\because` / `\therefore` 组织“条件—结论”链；不要通篇只用“由、故、所以”的口语句式。
   - 使用 `\because` / `\therefore` 的独立推理行，句末不得添加中文或英文的逗号、句号、分号；可见换行直接接 `\\`。普通叙述句不受此限制。
...
15. YAML 校验通过且图形已 resolve 后，直接 handoff 给 `math-assignment-latex` 渲染并检查 `.tex`，随后用 VS Code 打开生成的 `.tex`；流程不中断、不设置人工确认点，用户要求 PDF 时再继续编译。
...
9. 初中几何推导已在关键因果链中使用 `\because` / `\therefore`，且这些独立推理行的句末没有逗号、句号或分号；若有等腰三角形之间的相似，已明示顶角/底角和腰/底，并优先检查腰底比路径。
