...
- 空间点线面、棱柱棱锥、二面角、异面直线距离等使用 `diagram_kind: "spatial_geometry"`，三维输入写入 `engine_options.spatial_spec`；Host 路由到 `spatial_renderer`。一般关系图用 `textbook_oblique`，两面相交用 `hinge_planes`，空间坐标/向量用 `orthographic_3d`。
