...
The adapter exposes Claude Code as a PydanticAI ``Model`` (:class:`ClaudeCodeModel`)
and drives ``Agent(model=..., output_type=QuestionTranscriptionBundle).run()`` —
structurally symmetric with the OpenCode adapter. The SDK turn is an injectable
:class:`ClaudeQueryPort`, so these tests never import ``claude_agent_sdk`` nor reach
the network/CLI. They assert the adapter:
...
def test_transcribe_cache_hit_short_circuits(tmp_path):
    ...
    adapter = ClaudeCodeTranscriber(
        model="sonnet", store=store, cache_dir=cache_dir, query_port=_CountingPort(),
    )
    ...
    adapter2 = ClaudeCodeTranscriber(
        model="sonnet", store=store, cache_dir=cache_dir, query_port=_ExplodingPort(),
    )
