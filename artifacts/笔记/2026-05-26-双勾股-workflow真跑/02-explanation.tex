...
\eduSectionTitle{双勾股讲解}

\begin{eduProblemBlock}[原题]
\noindent
...
\end{minipage}\hfill
\diagramcolfigure{diagram/jobs/orig-prompt/rendered/prompt.png}{0.30\linewidth}{先看点序和共高。}
\end{eduProblemBlock}
...
作 $AH\perp BD$$\;\to\;$由 $AD=AB$ 得 $H$ 是 $BD$ 中点$\;\to\;$把 $BH,CH$ 都写成 $BC$ 的倍数$\;\to\;$在 $\triangle ABH$ 与 $\triangle ACH$ 中两次勾股并相减\end{eduAuxItem}




\begin{eduAuxItem}{核心思路}
这类题的共同动作是：两条斜边已知，两个直角三角形共用同一条高 $AH$。
所以分别写勾股式后相减，可以把 $AH^2$ 消掉，只留下底边上的线段关系。
\end{eduAuxItem}
...
设 $BC=x$。由 $2BD=9BC$，得 $BD=\dfrac92x$。\par
因为 $AD=AB$，且 $AH\perp BD$，所以 $H$ 是 $BD$ 的中点，$BH=\dfrac12BD=\dfrac94x$。\par
点序为 $B-C-H-D$，所以 $CH=BH-BC=\dfrac94x-x=\dfrac54x$。\par
在 $\triangle ABH$ 与 $\triangle ACH$ 中：$15^2=AH^2+\left(\dfrac94x\right)^2$，$13^2=AH^2+\left(\dfrac54x\right)^2$。\par
两式相减：$56=\dfrac{81-25}{16}x^2=\dfrac72x^2$，所以 $x^2=16$，$x=4$。\par
  \endgroup
\end{eduSolutionBox}
...
\begin{eduBottomGrid}
  ...
  \begin{eduSummaryRight}
    ...
    \begin{eduBulletList}
      \item 看到 $AD=AB$，优先想到向 $BD$ 作高，得到中点。      \item 看到两条斜边长度，优先写两个共高直角三角形的勾股式。    \end{eduBulletList}
    \end{eduSummaryBlock}
  \end{eduSummaryRight}
\end{eduBottomGrid}
