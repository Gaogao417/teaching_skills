...
\begin{center}
  \includegraphics[width=0.52\linewidth]{\detokenize{diagram/jobs/orig-prompt/rendered/prompt.png}}
  \par\vspace{1mm}{\small\color{edu-muted}先看点序 $B-C-H-D$，再看公共高 $AH$。}
\end{center}
...
过 $A$ 作 $AH\perp BD$$\;\to\;$由 $AD=AB$ 得 $H$ 是 $BD$ 中点$\;\to\;$把 $BH,CH$ 都写成 $BC$ 的倍数$\;\to\;$两次勾股相减，消去 $AH^2$\end{eduAuxItem}




\begin{eduAuxItem}{核心思路}
双勾股的关键不是先算高，而是让两个直角三角形共用同一个高 $AH$。这样两个勾股式相减时，$AH^2$ 会自动消掉。
\end{eduAuxItem}
...
设 $BC=x$，先表示 $BH$ 和 $CH$。\end{eduSubQuestionItem}

\begin{eduSolutionBox}{先把底边说清楚}
  \begingroup
  \setlength{\parskip}{2.5mm}
由 $2BD=9BC$，得 $BD=\dfrac92x$。\par
因为 $AD=AB$，且 $AH\perp BD$，所以 $H$ 是 $BD$ 的中点。\par
  \endgroup
\end{eduSolutionBox}
...
列两次勾股并相减。\end{eduSubQuestionItem}

\begin{eduSolutionBox}{两次勾股}
  \begingroup
  \setlength{\parskip}{2.5mm}
$15^2=AH^2+\left(\dfrac94x\right)^2$。\par
$13^2=AH^2+\left(\dfrac54x\right)^2$。\par
  \endgroup
\end{eduSolutionBox}
...
\begin{eduBottomGrid}
  ...
  \begin{eduSummaryRight}
    ...
    \begin{eduBulletList}
      \item 等腰给中点，共高给消元。      \item 先表达两条底边，再列两个勾股式。    \end{eduBulletList}
    \end{eduSummaryBlock}
  \end{eduSummaryRight}
\end{eduBottomGrid}
