...
\section*{一、关键点与参数（每题 8 分，共 24 分）}


\begin{question}[points=8]
  点 $P(x,y)$ 在第一象限，满足 $xy=8$，且 $P$ 在直线 $y=x+2$ 上。点 $P$ 的坐标为（\quad）。
  ...
  \paren[A]
  \begin{solutionblock}
    由 $y=x+2$ 和 $xy=8$ 得 $x(x+2)=8$，解得 $x=2$ 或 $x=-4$。第一象限取 $P(2,4)$。
  \end{solutionblock}
\end{question}
...
\begin{question}[points=8]
  点 $P(x,y)$ 在第一象限，满足 $xy=12$，且 $P$ 在直线 $y=x+1$ 上。若 $A(0,0)$，$B(3,0)$，$C=P$，四边形 $ABCD$ 为矩形，则 $D=$ \underline{\hspace{2cm}}。

  \paren[$(0,4)$]
  \begin{solutionblock}
    由 $x(x+1)=12$ 得第一象限点 $P(3,4)$，故 $C(3,4)$，$D(0,4)$。
  \end{solutionblock}
\end{question}
...
\begin{question}[points=8]
  $A(0,0)$，$B(6,0)$，点 $C$ 在直线 $y=x$ 上。若 $A,B,C,D$ 可组成面积为 $18$ 的矩形，且 $C$ 在 $y=k/x$ 上，则 $k=$ \underline{\hspace{2cm}}。

  \paren[$9$]
  \begin{solutionblock}
    矩形候选中，面积为 $18$ 的点为 $C(3,3)$，所以 $k=xy=9$。
  \end{solutionblock}
\end{question}


\section*{二、条件筛选（每题 12 分，共 36 分）}
...
\end{enumerate}
  ...
  \begin{solutionblock}
    \textbf{答案：}$P(4,5)$，$k=20$；$OP:y=\dfrac54x$；$D(0,5)$\par\medskip
    \solstep{求P和k}{$4y=20$，得 $y=5$，所以 $P(4,5)$，$k=4\cdot5=20$。}
    \solstep{求直线OP}{直线 $OP$ 过原点和 $P(4,5)$，故 $OP:y=\dfrac54x$。}
    \solstep{求D}{矩形 $ABCD$ 中 $D(0,5)$。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=12]
  点 $C$ 在直线 $y=x+2$ 上，且满足 $x_C+2\ge 8/x_C$。已知 $A(0,0)$，$B(2,0)$，$A,B,C,D$ 可组成矩形。
\begin{enumerate}[label=(\arabic*)]
  \item 求不等式的解集；
  \item 求所有 $C,D$。
\end{enumerate}
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$[-4,0)\cup[2,+\infty)$；$C(2,4)$，$D(0,4)$\par\medskip
    \solstep{求解集}{由 $x+2=\dfrac8x$ 得 $x=-4,2$，分段比较得 $[-4,0)\cup[2,+\infty)$。}
    \solstep{筛选矩形}{矩形候选中 $C(0,2)$ 被 $x=0$ 排除，保留 $C(2,4)$，此时 $D(0,4)$。}
  \end{solutionblock}
\end{problem}
...
\end{enumerate}
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$C(4,4)$，$D(0,4)$；是矩形\par\medskip
    \solstep{求菱形}{设 $C(t,t)$。由 $AB=BC$ 得 $(t-4)^2+t^2=16$，取非退化解 $t=4$，所以 $C(4,4)$，$D(0,4)$。}
    \solstep{验矩形}{$AB$ 水平，$BC$ 竖直，故 $AB\perp BC$，所以它也是矩形。}
  \end{solutionblock}
\end{problem}
...
\end{enumerate}
  ...
  \begin{solutionblock}
    ...
    \solstep{求矩形}{$AB=8$，面积 $16$，故高为 $2$，所以 $C(8,2)$，$D(0,2)$。}
    \solstep{求参数}{$m=\dfrac{2}{8}=\dfrac14$，$k=8\cdot2=16$。}
  \end{solutionblock}
\end{problem}
...
\end{enumerate}
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$C(4,8)$，$D(0,8)$；不是菱形\par\medskip
    \solstep{求矩形}{矩形中 $BC\perp AB$，故 $x_C=4$。由 $y=2x$ 得 $C(4,8)$，$D(0,8)$。}
    \solstep{验菱形}{$AB=4$，$BC=8$，邻边不相等，不是菱形。}
  \end{solutionblock}
\end{problem}
...
\end{enumerate}
  ...
  \begin{solutionblock}
    ...
    \solstep{求P}{由 $x(x+3)=18$ 得 $x=3$ 或 $x=-6$，第一象限取 $P(3,6)$。}
    \solstep{求OP}{直线 $OP$ 为 $y=2x$。}
    \solstep{求E,F}{设 $E(t,2t)$。由 $AB=BE$ 得 $(t-6)^2+(2t)^2=36$，非退化解 $t=\dfrac{12}{5}$。故 $E\left(\dfrac{12}{5},\dfrac{24}{5}\right)$，$F=A+E-B=\left(-\dfrac{18}{5},\dfrac{24}{5}\right)$。}
  \end{solutionblock}
\end{problem}
...
\end{enumerate}
  ...
  \begin{solutionblock}
    ...
    \solstep{求k和P}{$xy=12$，故 $k=12$。由 $x(x+1)=12$ 得第一象限点 $P(3,4)$。}
    \solstep{求D}{$B(3,0)$，$C(3,4)$，故 $D(0,4)$。}
    \solstep{求m}{直线 $OC$ 过 $C(3,4)$，所以 $m=\dfrac43$。}
  \end{solutionblock}
\end{problem}
