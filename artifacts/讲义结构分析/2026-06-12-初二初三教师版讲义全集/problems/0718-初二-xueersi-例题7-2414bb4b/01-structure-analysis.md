# 结构分析：初二 xueersi 例题7：函数/坐标关系建模

## 原题
【例题7】 ★★★☆☆

(1) 已知直线 $ y=k x+b(k\neq0) $与双曲线 $ y=\frac{k^{2}}{x} $交于点 M（-1,m）、N（2,n）.求不等式 $ k x+b>\frac{k^{2}}{x} $的解集.

- 来源讲义：（数）2021年春八年级培优体系_教师版_第1讲_一次函数综合（敏学班）
- 来源定位：page=5；例题7
- 主题标签：函数、综合题
...
(2) 应用函数图像求不等式 $ \frac{1}{x} > 2 x-1 $的解集.


【分析】 【注】一般地，不等式 $ f ( x ) > g ( x ) $可以看作函数 f(x)的图像在函数 g(x)的图像上方的所有点的横坐标的集合；不等式 $ f ( x ) < g ( x ) $可以看作函数 f(x)的图像在函数
g(x)的图像的下点的横坐标的集合.


设 $ y_{1}=\frac{1}{x} $ $ y_{2}=2 x-1 $，不等式 $ \frac{1}{x} > 2 x-1 $的解集即是函数值 $ y_{1}>y_{2} $时自变量x的取值范围.


作出函数 $ y_{1}=\frac{1}{x} $ $ y_{2}=2 x-1 $的图像，交点坐标为 $ (-\frac{1}{2},-2) $、(1,1),


由图像可知，当 $ x < -\frac{1}{2} $或0<x<1时， $ y_{1}>y_{2} $，


即不等式 $ \frac{1}{x} > 2 x-1 $的解集是 $ x < -\frac{1}{2} $和0<x<1.
