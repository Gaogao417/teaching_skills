...
- 来源讲义：（数）2020年_寒_八年级_培优体系_教师版_第7讲_四边形中的动点问题（二）（敏学班）
- 来源定位：page=12；延伸1
...
【分析】 (1)过点 $ A_{1} $作 $ A_{1} D \bot x $轴于点 D，如图1所示.


$ \therefore $点 $ A_{1} $的坐标为 $ \left(\sqrt{3},3\right) $


$ \because $当 x= $ \sqrt{3} $时， y=- $ \frac{\sqrt{3}}{3} \times\sqrt{3}+4=3 $ ， $ \therefore $点 $ A_{1} $在直线l上.


(2) 由题，点 $ C_{1} $的坐标为 $ \left( 2\sqrt{3},0\right) $


设边 $ A_{1} C_{1} $所在直线的解析式为 $ y=k x+b(k\neq0) $ ，则 $ \left\{\begin{array}{l l} {\sqrt{3} k+b=3} \\ {2\sqrt{3}
k+b=0}\end{array} \right. $ ，解得： $ \left\{\begin{array}{l l} {k=-\sqrt{3}} \\ {b=6}\end{array} \right. $ $ \therefore
y=-\sqrt{3} x+6 $


(3) 点 F的坐标为 $ \left( 4\sqrt{3},0\right) $


分三种情况考虑（如图2）：


$\textcircled{1}$当 $A_{1}F$ 为对角线时，点 $P_{1}$ 的坐标为 $\left(3\sqrt{3},3\right)$；


$ \textcircled{2} $当 $ A_{1}C_{1} $为对角线时，点 $ P_{2} $的坐标为 $ \left(-\sqrt{3},3\right) $；


$ \textcircled{3} $当 $ C_{1}F $为对角线时，点 $ P_{3} $的坐标为 $ \left( 5\sqrt{3},-3\right) $


综上所述：点P的坐标为 $ \left( 3\sqrt{3},3\right) $或 $ \left(-\sqrt{3},3\right) $或 $ \left( 5\sqrt{3},-3\right). $


![](page=12,bbox=[490, 2151, 998, 2477])
...
![](page=12,bbox=[1014, 2126, 1561, 2514])


![](page=13,bbox=[69, 27, 284, 95])
