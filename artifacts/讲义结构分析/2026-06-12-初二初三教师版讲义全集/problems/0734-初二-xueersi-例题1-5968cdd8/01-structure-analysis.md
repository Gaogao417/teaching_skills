# 结构分析：初二 xueersi 例题1：几何关系与辅助线/图形不变量
...
- 来源讲义：（数）2021年春八年级培优体系_教师版_第3讲_平行四边形综合（勤思班）
- 来源定位：page=1；例题1
...
$ \because $四边形ABCD为平行四边形， $ \therefore AB=CD $ ， $ AD=BC $ ， $ \angle BAC=\angle DCA $


$$

\because B E \bot A C, D F \bot A C, \therefore B E / / D F \therefore \angle A B E = \angle C D F
...
\therefore \triangle A B E \cong \triangle C D F, \therefore B E = D F

$$


又 $ \because $ G、H分别是BC、AD中点， $ \therefore E G=G C=\frac{1}{2} B C \angle G E C=\angle G C E $


同理 FH=AH $ = \frac{1}{2} A D $ $ \angle D A F=\angle A F H $


$ \therefore E G / / H F $且 $ E G=H F $


$ \therefore $四边形 EGFH为平行四边形， $ \therefore BE+EH+HF=DF+FG+EG $
