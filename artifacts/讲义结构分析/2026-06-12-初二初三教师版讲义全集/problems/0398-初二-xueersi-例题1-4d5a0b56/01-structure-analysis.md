...
已知关于 x的方程 $ ( a^{2}-1 ) x^{2}-2 ( 5 a+1 ) x+2 4=0 $有两个不相等的负整数根，求整数 a的值.

- 来源讲义：第2讲_一元二次方程的应用（一）_教师版（勤思班）
- 来源定位：page=3；例题1
- 主题标签：一元二次方程、OCR需复核
...
利用因式分解： $ \left[ ( a+1 ) x-4 \right] \left[ ( a-1 ) x-6 \right]=0 $得： $ x_{1}=\frac{6}{a-1} $ $ x_{2}=\frac{4}{a+1} $


当 $ a-1=-1 $ ，-2，-3，-6，即 $ a=0 $ ，-1，-2，-5时， $ x_{1} $ 为负整数.


当 a+1=-1，-2，-4，即 a=-2，-3，-5时， $ x_{2} $为负整数；


$ \therefore a=-5 $ ，-2.但当 a=-5时， $ x_{1}=x_{2}=-1 $
