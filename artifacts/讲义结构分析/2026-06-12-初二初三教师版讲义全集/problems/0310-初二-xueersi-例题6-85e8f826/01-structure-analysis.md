# 结构分析：初二 xueersi 例题6：一元二次方程判别式/根系关系

## 原题
【例题6】 ★★★☆☆

已知关于 x的方程 $ x^{2}+(2 k-3) x+k^{2}-3=0 $有两个实数根 $ x_{1}, x_{2} $ ，且 $ x_{1}+x_{2}=\frac{1}{x_{1}}+\frac{1}{x_{2}} $ ，求 k值.

- 来源讲义：第3讲_一元二次方程根与系数的关系_教师版（敏学班）
- 来源定位：page=3；例题6
- 主题标签：一元二次方程
...
$ \because $方程 $ x^{2}+(2 k-3) x+k^{2}-3=0 $有两个实数根 $ x_{1}, $ $ x_{2}, $


$$

\therefore \left\{ \begin{array}{l} \Delta = (2 k - 3) ^ {2} - 4 \left(k ^ {2} - 3\right) = 2 1 - 1 2 k \geq 0 \\ x _
{1} + x _ {2} = - (2 k - 3) \\ x _ {1} \cdot x _ {2} = k ^ {2} - 3 \end{array} , \right.

$$


由(1)得： $k\leq \frac{7}{4}$.


$$

\because x _ {1} + x _ {2} = \frac {1}{x _ {1}} + \frac {1}{x _ {2}}, \therefore x _ {1} + x _ {2} = \frac {x _ {1} +
x _ {2}}{x _ {1} x _ {2}}, x _ {1} + x _ {2} = 0 \text {或} x _ {1} x _ {2} = 1

$$


当 $ x_{1}+x_{2}=0 $时， $ 3-2 k=0 $ $ k=\frac{3}{2} $ $ \because k=\frac{3}{2}<\frac{7}{4} $所以 $ k=\frac{3}{2} $符合题意当 $
x_{1}x_{2}=1 $时， $ k^{2}-3=1 $ $ k=\pm 2 $ $ \because k\leq \frac{7}{4} $ $ \therefore k=2 $舍去.


$ \therefore k $的值为 $ \frac{3}{2} $或-2．此题是已知方程两根满足的条件，求参数的取值.


![](page=4,bbox=[69, 36, 283, 104])
