# 结构分析：初二 xueersi 例题4：一元二次方程判别式/根系关系

## 原题
【例题4】 ★★★★☆

试确定一切有理数r，使得关于x的方程 $ r x^{2}+(r+2) x+r-1=0 $有根且只有整
...
当r=0时，原方程为 $ 2 x-1=0 $，得 $ x=\frac{1}{2} $，不满足原方程有根且只有整数根.


当r≠0时，设 $ r x^{2}+(r+2) x+r-1=0 $的两个实根为 $ x_{1} $、 $ x_{2} $（ $ x_{1}\leq x_{2} $）；


由韦达定理得， $ x_{1}+x_{2}=-\frac{r+2}{r} $， $ x_{1} x_{2}=\frac{r-1}{r} $；


所以 $ 2 x_{1} x_{2}-\left(x_{1}+x_{2}\right)=2 \times \frac{r-1}{r}-\left(-\frac{r+2}{r}\right)=3 $；


所以 $ 2 \left[ 2 x_{1} x_{2}-\left(x_{1}+x_{2}\right)\right]+1=\left(2 x_{1}-1\right)\left(2 x_{2}-1\right)=3 \times
2+1=7 $；


因为 $ x_{1}, x_{1} $为整数，所以 $ 2 x_{1}-1 $， $ 2 x_{2}-1 $都是7的约数；


因为 $ x_{1}\leq x_{2} $，所以 $ 2 x_{1}-1\leq 2 x_{2}-1 $；


所以 $ \left\{ \begin{array}{l l} 2 x_{1}-1=1 \\ 2 x_{2}-1=7 \end{array} \right. $或 $ \left\{ \begin{array}{l l} 2
x_{1}-1=-7 \\ 2 x_{2}-1=-1 \end{array} \right. ; $


![](page=4,bbox=[70, 48, 283, 113])


当 $ \left\{ \begin{array}{l l} 2 x_{1}-1=1 \\ 2 x_{2}-1=7 \end{array} \right. $时， $ \left\{ \begin{array}{l l} x_{1}=1
\\ x_{2}=4 \end{array} \right. $ $ -\frac{r+2}{r}=x_{1}+x_{2}=1+4=5 $ $ \frac{r-1}{r}=x_{1}x_{2}=1\times 4=4 $ ，得： $
r=-\frac{1}{3} $；


当 $ \left\{ \begin{array}{l l} 2 x_{1}-1=-7 \\ 2 x_{2}-1=-1 \end{array} \right. $时， $ \left\{ \begin{array}{l l}
x_{1}=-3 \\ x_{2}=0 \end{array} \right. $ $ -\frac{r+2}{r}=x_{1}+x_{2}=-3+0=-3 $ $ \frac{r-1}{r}=x_{1}x_{2}=-3\times
0=0 $ ，得： $ r=1 $.


综上所述， $ r=-\frac{1}{3} $或 $ r=1. $
