...
(2) 在一个三角形中，如果一边上的中线等于这边的一半，那么这个三角形为直角三角形.


(3) $ \textcircled{1} $若 a+b是偶数，则 a、b都是奇数. 假


$ \textcircled{2} $如果一个图形既是中心对称图形，也是轴对称图形，那么这个图形是圆。假


$ \textcircled{3} $如果一个三角形中有一个角是钝角，那么另外两个角一定是锐角. 真


$ \textcircled{4} $如果两个数互为相反数，那么这两个数的绝对值相等. 真


$ \textcircled{5} $如果一个数是奇数那么这个数是质数。假 (4) B
