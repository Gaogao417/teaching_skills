...
- 来源讲义：（数）2021年春八年级培优体系_教师版_第5讲_平面向量（勤思班）
- 来源定位：page=6；例题5
- 主题标签：平面向量、OCR需复核
- OCR/图形状态：需人工复核：题干含空格/填空占位。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。

## 一、题目场景
- 数学对象：向量表示与线性运算中的题面对象、变量、图形/函数/方程关系。
...
(2) $
\overrightarrow{m}-4\vec{n}=\left(3\vec{a}-\frac{2}{3}\vec{b}\right)-4\left(\frac{1}{2}\vec{b}+\frac{1}{4}\vec{a}\right)=2\vec{a}-\frac{8}{3}\vec{b}
$ ；故选 A.


(3) 由向量方程组 $ \left\{ \begin{array}{l l} {\vec{m}=2\vec{a}+\vec{b}} \\ {\vec{n}=\vec{a}-\vec{b}} \end{array} \right.
$；解得 $ \left\{ \begin{array}{l l} {\vec{a}=\frac{1}{3}\vec{m}+\frac{1}{3}\vec{n}} \\
{\vec{b}=\frac{1}{3}\vec{m}-\frac{2}{3}\vec{n}} \end{array} \right. . $
...
## 七、变式原则
- 核心不变量：保持“向量表示与线性运算”这一核心结构不变，只改变一个表层维度。
- 表层特征：平面向量、OCR需复核。
...
```json
{
  ...
  "variation_rules": {
    "core_invariant": "保持“向量表示与线性运算”这一核心结构不变，只改变一个表层维度。",
    "surface_features": [
      "平面向量",
      "OCR需复核"
    ],
