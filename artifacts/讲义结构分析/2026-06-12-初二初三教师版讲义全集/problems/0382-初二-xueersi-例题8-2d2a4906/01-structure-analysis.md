...
【敏学】在 Rt $ \triangle A B C $中， $ \angle C=9 0^{\circ} $ ，AM是中线， $ M N\bot A B $ ，垂足为N.求证： $ A N^{2}-B N^{2}=A C^{2}. $

- 来源讲义：第8讲_几何综合（一）_教师版（敏学班）
- 来源定位：page=6；例题8
- 主题标签：几何综合、综合题、OCR需复核
- OCR/图形状态：需人工复核：数字可能被拆空格；选择项/表格排版可能丢列。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
\therefore A M ^ {2} = A N ^ {2} + M N ^ {2}, B M ^ {2} = B N ^ {2} + M N ^ {2}
...
\therefore A M ^ {2} - B M ^ {2} = A N ^ {2} - B N ^ {2}
...
\text {又} \because B M = C M, \angle C = 9 0 ^ {\circ}
...
\therefore A N ^ {2} - B N ^ {2} = A C ^ {2}

$$


![](page=7,bbox=[69, 34, 283, 103])
