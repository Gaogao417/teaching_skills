...
某山山脚的 M处到山顶的 N处有一条长为600米的登山路，小李沿此路从 M走到 N，停留后再原路返回，其间小李离开 M处的路程 y米与离开 M处的时间 x分（ x>0 ）之间的函数关系如图中折线 OABCD所示.

(1) 求上山时 y关于 x的函数解析式，并写出定义域；

(2) 已知小李下山的时间共26分钟，其中前18分钟内的平均速度与后8分钟内的平均速度之比为2:3，试求点 C的纵坐标.

- 来源讲义：（数）2020年秋八年级培优体系_教师版_第8讲_函数应用题（敏学班）
- 来源定位：page=4；例题6
- 主题标签：函数、应用建模、OCR需复核
...
(1) 设 y关于 x函数解析式为 $ y=k x \left(k \neq 0\right) $则 $ 6 0 0=2 0 k $ ，得 k=30所以 $ y=3 0 x \left(0<x\leq 2 0\right). $


(2) 因为前18分钟内的平均速度与后8分钟内的平均速度之比为2:3，所以设前18分钟内的平均速度为 $ 2 v $ ，后8分钟内的平均速度 $ 3 v $则 $ 2 v\cdot1 8+3 v\cdot8=6 0 0. $


解得 $v = 10$ .


所以，前18分钟内的平均速度为20米/分，行走的路程=360米所以，点C的纵坐标为240.


![](page=5,bbox=[69, 47, 283, 112])
