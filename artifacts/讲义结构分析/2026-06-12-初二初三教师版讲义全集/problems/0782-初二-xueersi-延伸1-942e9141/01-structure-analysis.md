...
(2) 作 A关于 y轴的对称点 $ A^{\prime} $ ，则 $ PA=PA^{\prime} $


所以 $ \left| P A-PC\right|=\left| P A^{\prime}-PC\right| \leq A^{\prime} C $ ，当且仅当三点共线时取等号.


直线 $A^{\prime}C:y = 3x - 6$ ，与y轴交点为（0，-6），所以点P的坐标为（0，-6）.


(3) 由将军饮马模型，将点 B向右平移两个单位长度，设为 $ B_{1} $ ，作 $ B_{1} $ 关于 x轴的对称点 $ B_{2} $ ，联结 $ B_{2} C $ ，与 x轴的交点即为点 E.


$ B_{1}\left(2,2\right), B_{2}\left(2,-2\right), $直线 $ C B_{2}: y=4 x-1 0 $ ，则 $ E\left(\frac{5}{2},0\right) $点 $
D\left(\frac{1}{2},0\right) $


![](page=11,bbox=[69, 26, 283, 96])
