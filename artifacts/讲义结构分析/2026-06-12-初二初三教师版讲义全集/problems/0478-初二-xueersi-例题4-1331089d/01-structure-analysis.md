...
如图，平行四边形ABCD中，AQ、BN、CN、DQ分别是 $ \angle D A B $ $ \angle A B C $ $ \angle B C D $ $ \angle C D A $的平分线，AQ与BN交于P，CN与DQ交于M，证明：四边形PQMN是矩形.

- 来源讲义：（数）2020年秋八年级培优体系_教师版_第10讲_特殊的平行四边形（一）（敏学班）
- 来源定位：page=3；例题4
...
$ \because $四边形ABCD为平行四边形， $ \therefore AB//CD $ ，AD//BC
...
$ \because A Q $ 、 BN分别是 $ \angle D A B $ 、 $ \angle A B C $的平分线


$$

\therefore \angle Q P N = 9 0 ^ {\circ}

$$


同理 $ \angle P Q M=\angle Q M N=\angle M N P=9 0^{\circ} $ $ \therefore $四边形PQMN是矩形.


![](page=4,bbox=[83, 44, 293, 109])
