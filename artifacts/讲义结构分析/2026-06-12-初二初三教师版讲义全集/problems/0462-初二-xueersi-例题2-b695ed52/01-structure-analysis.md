...
(1) $\left\{ \begin{array}{l l} 3m - 7 < 0 \\ m - 1 > 0 \end{array} \right.$ ，得 $1 < m < \frac{7}{3}$ ，得整数m为2；


(2) 根据题意可得： $ \left\{\begin{array}{l l}5-a>0\\ a-1>0\end{array}\right. $ ，解得 $ 1<a<5; $


(3) 根据题意可得 $ -\frac{a}{b} < 0 $ $ \frac{a}{c} < 0 $ ，故选择 D；


(4) $ \left\{ \begin{array}{l l} 2(1-k)<0 \\ \frac{k}{2}-1\leq 0 \end{array} \right. $ ，得 $ 1<k\leq 2 $．（此题学生容易漏解等号）
