...
【分析】延长 AF，交 BC的延长线于点 G；因为 AD// BC；所以 $ \angle D=\angle F C G $；


因为 $ \triangle A D F\cong \triangle G C F $ ；所以 AF=GF ；因为 AF $ \bot $ AB， AE=BE ；所以 BE=AE=GE；


所以 EF是 $ \triangle A B G $的中位线；所以 $ E F=\frac{1}{2} A B $；因为 $ A B=m $；所以 $ E F=\frac{1}{2} m. $


![](page=4,bbox=[69, 27, 284, 96])
