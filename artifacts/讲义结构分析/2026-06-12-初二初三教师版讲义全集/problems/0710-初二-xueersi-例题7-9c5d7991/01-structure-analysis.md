...
如图，在平面直角坐标系中，点 A（0,5）和 C（10,0）在一次函数 $ y=k x+b $的图像上，与反比例函数 $ y=\frac{k^{\prime}}{x} $交于点 B（8,t）.

(1) 求一次函数和反比例函数的函数关系式；

(2) 将一次函数的图像向下平移 m个单位，恰好与反比例函数只有一个交点，求 m的值.
...
【分析】 （1）一次函数： $ y=-\frac{1}{2} x+5 $ ；反比例函数： $ y=\frac{8}{x} $


(2) m=1或9


![](page=6,bbox=[70, 27, 283, 96])
