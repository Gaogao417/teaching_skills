# 结构分析：初二 xueersi 例题3：一元二次方程判别式/根系关系

## 原题
【例题3】 ★★★☆☆☆

(1)已知 m,n是一元二次方程 $ x^{2}-4 x-3=0 $的两个实数根，则代数式 $ ( m+1 ) ( n+1 ) $的值为（ ）
...
(4) 若关于 x的方程 $ x^{2}-4 x+m+\frac{1}{2}=0 $有两个相等的实数根，则 m的值为

(5) 某公司1月份的销售额为200万元，2月、3月的销售额共800万元．若第一季度平均每月的增长率为 x，则由题意可列方程为 ___.

- 来源讲义：第10讲_暑假复习_教师版（勤思班）
- 来源定位：page=2；例题3
- 主题标签：一元二次方程、应用建模、OCR需复核
- OCR/图形状态：需人工复核：题干含空格/填空占位；选择项/表格排版可能丢列。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
(3) $ 4 \left(x-\frac{3+\sqrt{2 1}}{4} y\right)\left(x-\frac{3-\sqrt{2 1}}{4} y\right) $


(4) $ \frac{7}{2} $


(5) $ 2 0 0 \left( 1+x\right) + 2 0 0 \left( 1+x\right)^{2}=8 0 0 $
