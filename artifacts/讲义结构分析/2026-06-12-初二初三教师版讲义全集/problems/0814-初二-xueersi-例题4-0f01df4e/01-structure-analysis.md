...
(1) 若函数 $ y=ax^{2}+2x-1 $的图像与 x轴有公共点，则实数 a的取值范围___.

(2) 关于 x的函数 $ y=mx^{2}-(3m+1)x+2m+1 $的图像与坐标轴只有两个交点， m= ___.

- 来源讲义：（数）2021年春八年级培优体系_教师版_第9讲_二次函数（二）
- 来源定位：page=7；例题4
- 主题标签：函数、OCR需复核
...
【分析】 (1) $ \because $函数 $ y=ax^{2}+2x-1 $的图像与 x轴有公共点，


$ \textcircled{1} $ $ a=0 $时，必能满足要求


$\textcircled{2}$ $a \neq 0$ 时， $\Delta = 4 + 4a\geq 0$ ，解得： $a\geq -1$ 且 $a\neq 0$
