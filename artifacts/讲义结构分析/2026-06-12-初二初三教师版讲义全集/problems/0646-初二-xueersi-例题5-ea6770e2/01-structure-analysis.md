...
- 来源讲义：（数）2020年_寒_八年级_培优体系_教师版_第5讲_中位线及其应用（敏学班）
- 来源定位：page=3；例题5
- 主题标签：几何综合、OCR需复核
- OCR/图形状态：需人工复核：含图形条件，需对照原图；选择项/表格排版可能丢列；OCR文本较短，需复核完整性；题目依赖图形，仅生成 diagram_request_packet，不生成图片。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
【分析】联结AC，取AC中点G，联结EG、FG


【分析】联结AC，取AC中点G，联结EG、FG $ \because E、F $分别是BC、AD的中点， $ \therefore EG、FG $分别是 $ \triangle ACB、\triangle ACD $的中位线 $
\therefore EG//AB $ $ EG=\frac{1}{2}AB $ $ FG//CD $ $ FG=\frac{1}{2}CD $ $ \because AB//CD $ $ \therefore EG//FG $ $
\therefore E、F、G $三点在一条直线上 $ \therefore EF=FG-EG=\frac{1}{2}CD-\frac{1}{2}AB $ $ \because AB=a $ $ CD=b $ $ \therefore
EF=\frac{1}{2}(b-a) $或者联结AE并延长交CD于G


![](page=4,bbox=[69, 28, 284, 94])
