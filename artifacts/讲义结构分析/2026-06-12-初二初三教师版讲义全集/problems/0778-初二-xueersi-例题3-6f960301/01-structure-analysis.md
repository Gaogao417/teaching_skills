...
- 来源讲义：（数）2021年春八年级培优体系_教师版_第6讲_一次函数与动点问题（勤思班）
- 来源定位：page=3；例题3
- 主题标签：函数、动点与存在性、几何综合、综合题、OCR需复核
- OCR/图形状态：需人工复核：数字可能被拆空格；含图形条件，需对照原图；题目依赖图形，仅生成 diagram_request_packet，不生成图片。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
【分析】 (1)易知 $ A ( 0,1 0 ) $ $ B \left( 1 0 \sqrt{3}, 0 \right) $ ，由勾股定理，可得 $ AB=2 0 $


(2) 易知 $\angle ABO = 30^{\circ}$ ，则 $BD = \frac{\sqrt{3}}{2} x$ ， $OD = 10\sqrt{3} - \frac{\sqrt{3}}{2} x$ ， $OE = 10
- \frac{x}{2}$ $y = \frac{1}{2}\times \frac{x}{2}\left(10\sqrt{3} - \frac{\sqrt{3}}{2} x\right)$ ，化简得 $y =
-\frac{\sqrt{3}}{8} x^{2} + \frac{5\sqrt{3}}{2} x$ .


(3) 因为 $ \angle C D E=6 0^{\circ} $ ，故对另外两个角进行分类讨论.


$ \textcircled{1} $ $ \angle DCE=90^{\circ} $时，此时 $ CD=OE $ . x=5 . 此时C点坐标为 $ \left(5\sqrt{3},5\right) $


$ \textcircled{2} $ $ \angle CED=90^{\circ} $时，此时 $ CD=2DE=4OE $ ，此时 $ \frac{x}{2}=4\left(10-\frac{x}{2}\right) $ ，解得
x=16.此时C点的坐标为 $ \left(2\sqrt{3},8\right). $


综上，C的坐标为 $ \left( 5\sqrt{3},5\right) $和 $ \left( 2\sqrt{3},8\right). $


![](page=4,bbox=[70, 28, 283, 95])
