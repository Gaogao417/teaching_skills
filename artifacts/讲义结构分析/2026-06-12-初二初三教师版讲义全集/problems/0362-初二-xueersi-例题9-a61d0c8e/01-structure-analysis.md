# 结构分析：初二 xueersi 例题9：几何关系与辅助线/图形不变量
...
- 来源讲义：第7讲_命题与证明_教师版（勤思班）
- 来源定位：page=7；例题9
- 主题标签：几何综合、证明与判定、OCR需复核
- OCR/图形状态：需人工复核：含图形条件，需对照原图；OCR文本较短，需复核完整性；题目依赖图形，仅生成 diagram_request_packet，不生成图片。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
轨迹是由四个角处的四个直角扇形，与正方形的四条边上的四条线段组成。四个角上的图形合起来刚好是一个半径为0.5的圆，长度为 $ 2 \pi \times0. 5+2 \times4=8+\pi $


![](page=8,bbox=[69, 36, 282, 104])
...
B. 如果两个角是同旁内角，那么这两个角的角平分线互相垂直


C. 到角两边距离相等的点，在这个角的平分线上
...
C. 两个三角形全等，那么它们的周长一定相等


D. 有一个角等于 $ 6 0^{\circ} $的等腰三角形是等边三角形
...
C. 若 $ b^{2}-4 a c<0 $ ，则方程 $ a x^{2}+b x+c=0 ( a \neq0 ) $ 没有实数根
...
![](page=9,bbox=[70, 36, 283, 104])


## 【巩固4】


指出下列命题的题设和结论，并把它改写成“如果……，那么……”的形式.
...
(1) 如果两条直线平行于一条直线，那么这两条直线平行


(2) 如果两个数的乘积为1，那么这两个数互为倒数
...
(1) 题设：如果这两个三角形等底等高；结论：那么这两个三角形的面积相等.


逆命题：如果这两个三角形的面积相等，那么这两个三角形等底等高.


(2) 逆命题一，题设：如果这两条弦是同圆或等圆中相等的圆心角所对的弦；结论：那么这两条弦相等.


逆命题：如果两条弦相等，那么这两条弦是同圆或等圆中相等的圆心角所对的弦.


逆命题二，题设：如果两条弦是相等的圆心角所对的弦；结论：那么在同圆或等圆中，这两条弦相等.


逆命题：如果在同圆或等圆中，两条弦相等，那么这两条弦是相等的圆心角所对的弦.


(3) 题设：如果两条线段是等腰三角形两底角的平分线，结论：那么这两条线段相等.


逆命题：如果有两条线段相等，那么两条线段是等腰三角形两底角的平分线.


## 【巩固6】


如图，圆 O的半径为1，正方形 ABCD的边长为6，若将圆 O绕正方形 ABCD滚动一周，在滚动过程中保持与正方形的边相切，则这一过程中圆心 O运动的路线长为___


![](page=9,bbox=[307, 1992, 740, 2332])


【分析】


弧长为 $ \frac{9 0 \pi \times1}{1 8 0}=\frac{\pi}{2} $ ，因此圆心O共转过 $ 4\times 6+4\times \frac{\pi}{2}=2 4+2 \pi $
，即圆心O运动的路线长为 $ 2 4+2 \pi. $


![](page=10,bbox=[70, 37, 282, 103])
