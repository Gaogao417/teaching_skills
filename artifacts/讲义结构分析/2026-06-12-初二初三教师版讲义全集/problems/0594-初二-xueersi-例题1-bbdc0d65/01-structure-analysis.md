# 结构分析：初二 xueersi 例题1：方程组结构变形

## 原题
【例题1】 ★★★☆☆☆

解关于 x的方程 $ a x^{2}+b x+c=0. $
...
【分析】 （1）当 a=0时，bx+c=0，即 bx=-c，


1. b=0时， $ 0 x=-c $


$ \textcircled{1} $ $ c=0 $时，x为任意实数；
...
$ \textcircled{1} $ $ \Delta<0 $时，此时该二次方程无实数解；


$ \textcircled{2} $ $ \Delta=0 $时， $ x_{1}=x_{2}=-\frac{b}{2a} $；


$ \textcircled{3} $ $ \Delta > 0 $时， $ x_{1}=\frac{-b+\sqrt{b^{2}-4ac}}{2a} $ $ x_{2}=\frac{-b-\sqrt{b^{2}-4ac}}{2a}
$；
