# 结构分析：初二 xueersi 例题8：一元二次方程判别式/根系关系
...
已知关于 x的方程 $ a x^{2}+b x+c=0 $ ， $ b x^{2}+c x+a=0 $和 $ c x^{2}+a x+b=0 $有一个公共根，求 $ \frac{b+c}{a}+\frac{c+a}{b}+\frac{a+b}{c} $的值.

- 来源讲义：第3讲_一元二次方程的应用（二）教师版_（勤思班）
- 来源定位：page=6；例题8
- 主题标签：一元二次方程、综合题
- OCR/图形状态：OCR 风险低，当前文本可作为后续讲解与练习的数据锚点。

## 一、题目场景
- 数学对象：一元二次方程判别式/根系关系中的题面对象、变量、图形/函数/方程关系。
...
易知 a+b+c=0，即 $ \frac{b+c}{a}+\frac{c+a}{b}+\frac{a+b}{c}=-3 $
...
![](page=7,bbox=[70, 48, 284, 113])


当 m为整数时，关于 x的方程 $ ( 2 m-1 ) x^{2}-( 2 m+1 ) x+1=0 $是否有有理根？如果有，求出 m的值；如果没有，请说明理由.


## 【分析】


因为 $m$ 为整数，所以二次项系数 $2m - 1\neq 0$ ；所以原方程是关于 $x$ 的二次方程；


若关于 x的方程 $ ( 2 m-1 ) x^{2}-( 2 m+1 ) x+1=0 $有有理根；


设 $\Delta = \left(2m + 1\right)^{2} - 4\times \left(2m - 1\right)\times 1 = 4m^{2} - 4m + 5 = \left(2m - 1\right)^{2}
+ 4 = k^{2}$ （ $k$ 为自然数）；


所以 $k^{2} - \left(2m - 1\right)^{2} = 4$ ， $[k - (2m - 1)][k + (2m - 1)] = 4$ ；


因为 $ \left( 2 m-1\right)^{2}+4=k^{2} $ ；所以2m-1与k同奇偶；


所以 $\left\{ \begin{array}{l l} k - (2m - 1) = 2 \\ k + (2m - 1) = 2 \end{array} \right.$ 或 $\left\{ \begin{array}{l l}
k - (2m - 1) = -2 \\ k + (2m - 1) = -2 \end{array} \right.$ ；所以 $k = \pm 2$ ， $2m - 1 = 0$ ；


$$

\left(2 m - 1\right) x ^ {2} - \left(2 m + 1\right) x + 1 = 0
...
已知关于 x的方程： $ x^{2} + (a - 6)x + a = 0 $的两根都是整数，求 a的值.


## 【分析】


设 $x_{1}$ 、 $x_{2}$ 是方程 $x^{2} + (a - 6)x + a = 0$ 的两个整数根（ $x_{1}\leq x_{2}$ ）；


由韦达定理， $ x_{1}+x_{2}=-a+6 $ ， $ x_{1}x_{2}=a $；


所以 $ \left(x_{1}+1\right)\left(x_{2}+1\right)=x_{1}x_{2}+x_{1}+x_{2}+1=a+\left(-a+6\right)+1=7; $


因为 $x_{1}$ 、 $x_{2}$ 都是整数， $x_{1}\leq x_{2}$ ；所以 $x_{1} + 1$ 、 $x_{2} + 1$ 都是整数， $x_{1} + 1\leq x_{2} + 1$ ；


所以 $\left\{ \begin{array}{l}x_{1} + 1 = 1\\ x_{2} + 1 = 7 \end{array} \right.$ 或 $\left\{ \begin{array}{l}x_{1} + 1 =
-7\\ x_{2} + 1 = -1 \end{array} \right.;\left\{ \begin{array}{l}x_{1} = 0\\ x_{2} = 6 \end{array} \right.$ 或 $\left\{
\begin{array}{l}x_{1} = -8\\ x_{2} = -2 \end{array} \right.;$ 所以 $a = 0$ 或 $a = 16$


## 【巩固3】


已知关于 x的一元二次方程 $ x^{2}+px+q=0 $的两根是非零整数，且 p+q=198.求 p的值.


## 【分析】


设已知方程的两根为 $x_{1}, x_{2}$ ， $x_{1}\leq x_{2}$ ，则 $x_{1}x_{2} - \left(x_{1} + x_{2}\right) + 1 = \left(x_{1} -
1\right)\left(x_{2} - 1\right) = 199$


所以 $\left\{ \begin{array}{l}x_{1}-1=1\\ x_{2}-1=199 \end{array} \right.$ 或 $\left\{ \begin{array}{l}x_{1}-1=-199\\
x_{2}-1=-1 \end{array} \right.$


因为 $x_{1}$ 、 $x_{2}$ 为非零整数，所以 $x_{1} = 2$ ， $x_{2} = 200$


所以 $ p=-\left(x_{1}+x_{2}\right)=-202 $


## 【巩固4】


求满足如下条件的所有 k值：使关于 x的方程 $ k x^{2}+(k+1) x+(k-1)=0 $的根都
...
当 $k = 0$ 时，原方程化为 $x - 1 = 0$ ，原方程的根 $x = 1$ 是整数.


当 $ k\neq 0 $时，原方程为二次方程，设原方程的两个根是 $ x_{1} $、 $ x_{2} $，不妨另 $ x_{1}\leq x_{2} $


由Viete定理，得 $x_{1} + x_{2} = -\frac{k + 1}{k} = -1 - \frac{1}{k}$ ， $x_{1}x_{2} = \frac{k - 1}{k} = 1 - \frac{1}{k}$ ；


两式相减，得 $x_{1}x_{2} - \left(x_{1} + x_{2}\right) = 2$ ， $(x_{1} - 1)(x_{2} - 1) = 3$ ；


因为 $x_{1}$ 、 $x_{2}$ 是整数，所以 $x_{1} - 1$ 、 $x_{2} - 1$ 是整数；


![](page=8,bbox=[70, 47, 283, 113])


所以 $ \left\{\begin{array}{l l}x_{1}-1=1\\ x_{2}-1=3\end{array}\right. $或 $ \left\{\begin{array}{l l}x_{1}-1=-3\\
x_{2}-1=-1\end{array}\right. $；解方程组解为 $ \left\{\begin{array}{l l}x_{1}=2\\ x_{2}=4\end{array}\right. $或 $
\left\{\begin{array}{l l}x_{1}=-2\\ x_{2}=0\end{array}\right. $所以 $ k=-\frac{1}{7} $或1. 综上所述， $ k=0 $或1或 $
-\frac{1}{7}. $
...
如果方程 $ x^{2}-px+2q=0 $和 $ x^{2}-qx+2p=0(p\neq q) $有公共根，求公共根
...
则 $ \left\{ \begin{array}{l} a^{2}-pa+2q=0 \\ a^{2}-qa+2p=0 \end{array} \right. $


两式相减得 $(q - p)(a + 2) = 0$


因为 p≠q，所以 a+2=0，解得 a=-2 所以公共根是-2
...
## 七、变式原则
- 核心不变量：保持“一元二次方程判别式/根系关系”这一核心结构不变，只改变一个表层维度。
- 表层特征：一元二次方程、综合题。
...
```json
{
  "problem_pattern": "一元二次方程判别式/根系关系",
  ...
  "variation_rules": {
    "core_invariant": "保持“一元二次方程判别式/根系关系”这一核心结构不变，只改变一个表层维度。",
    "surface_features": [
      "一元二次方程",
      "综合题"
    ],
