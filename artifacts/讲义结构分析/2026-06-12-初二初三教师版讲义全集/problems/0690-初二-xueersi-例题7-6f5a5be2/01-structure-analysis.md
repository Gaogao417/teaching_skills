...
如图，在 $ \triangle A B C $中，AD、CE分别为BC、AB边上的高，且AD为12， $ S_{\triangle E B D}:S_{\triangle A B C}=1:4 $ ，则BD=___.

- 来源讲义：（数）2021年春八年级培优体系_教师版_第13讲_相似三角形（二）
- 来源定位：page=5；例题7
- 主题标签：几何综合、OCR需复核
...
【分析】 $ \triangle A D B \sim \triangle C E B \Rightarrow \triangle D B E \sim \triangle A B C; $


所以 $ S_{\triangle DBE}:S_{\triangle ABC}=\left(BD:BA\right)^{2}=1:4 $ ；所以BD：BA=1：2


因为 $ AD\bot BC $ ；所以 $ BD:BA:AD=1:2:\sqrt{3} $ ， $ BD=4\sqrt{3} $


![](page=6,bbox=[69, 28, 283, 96])
