...
(2) y与 $ x^{2} $成反比例，且 $ x=4 $时， $ y=-\frac{1}{2} $ ，那么求y与x之间的函数解析式.

(3) 已知 $ | b |=3 $ ，且反比例函数 $ y=\frac{1+b}{x} $的图像在每个象限内，y随x的增大而增大，如果点 (a,3)在双曲线 $ y=\frac{1+b}{x} $上，求a的值.

- 来源讲义：第6讲_正比例函数与反比例函数（一）_教师版_（勤思班）
- 来源定位：page=5；例题6
- 主题标签：函数
...
(1) -3 (2) $ y=-\frac{8}{x^{2}} $ (3) $ \because\left|b\right|=3 $ $ \therefore b=\pm 3 $ .又 $ \because $反比例函数 $
y=\frac{1+b}{x} $的图像在每个象限内， y随x的增大而增大， $ \therefore 1+b<0 $ .可得 $ b<-1 $ $ \therefore b=-3 $ .则 $ 1+b=-2 $ $
\therefore y=-\frac{2}{x} $又 $ \because $点（a，3）在双曲线上 $ y=\frac{1+b}{x}=-\frac{2}{x} $ $ \therefore a=-\frac{2}{3} $


![](page=6,bbox=[69, 35, 283, 104])
