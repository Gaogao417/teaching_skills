...
- 来源讲义：第5讲_正比例函数与反比例函数（二）教师版（勤思班）
- 来源定位：page=8；悬赏题
- 主题标签：函数、动点与存在性、几何综合、综合题、OCR需复核
- OCR/图形状态：需人工复核：数字可能被拆空格；含图形条件，需对照原图；题干含空格/填空占位；题目依赖图形，仅生成 diagram_request_packet，不生成图片。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
【分析】 6, $ \frac{1 2 n}{n+1} $ （提示：将所有矩形向左平移）.


![](page=9,bbox=[48, 55, 260, 121])
...
如果函数 $ y=kx^{2 k^{2}+k-2} $的图像是双曲线，且在第二、四象限，那么k的值是多少？


【分析】由题意 $ \left\{\begin{array}{l l}2 k^{2}+k-2=-1\\ k<0\end{array}\right. $ ，得 k=-1.


【巩固2】


已知 $ \left(x_{1},y_{1}\right),\left(x_{2},y_{2}\right),\left(x_{3},y_{3}\right) $是反比例函数 $ y=-\frac{4}{x} $图像上的三个点，且 $
x_{1}<x_{2}<0 $ $ x_{3}>0 $则 $ y_{1} $ $ y_{2} $ $ y_{3} $的大小关系是（）


A. $ y_{3}<y_{1}<y_{2} $ B. $ y_{2}<y_{1}<y_{3} $


C. $ y_{1}<y_{2}<y_{3} $ D. $ y_{3}<y_{2}<y_{1} $


【分析】作图易得 A


【巩固3】如图，点 A在双曲线 $ y=\frac{1}{x} $上，点 B在双曲线 $ y=\frac{3}{x} $上，且 AB // x轴，C、D在 x轴上，若四边形 ABCD为矩形，则它的面积为___


![](page=9,bbox=[319, 1203, 738, 1622])


【分析】2. 提示：过 A 点作 y 轴的垂线， $ S_{ABCD}=3-1=2. $


【巩固4】


如图，直线l交y轴于点C，与双曲线 $ y=\frac{k}{x} ( k<0)
$交于A、B两点，P是线段AB上的点（不与A、B重合），Q为线段BC上的点（不与B、C重合），过点A、P、Q分别向x轴作垂线，垂足分别为D、E、F，联结OA、OP、OQ，设 $ \triangle A O D $的面积为 $ S_{1}
$ $ \triangle POE $的面积为 $ S_{2} $ $ \triangle Q O F $的面积为 $ S_{3} $ ，则有（）


![](page=9,bbox=[319, 2075, 736, 2361])


A. $ S_{1}<S_{2}<S_{3} $ B. $ S_{3}<S_{1}<S_{2} $ C. $ S_{3}<S_{2}<S_{1} $ D. $ S_{1}=S_{2}=S_{3} $


【分析】B


![](page=10,bbox=[48, 55, 260, 121])


【巩固5】


如图，在平面直角坐标系中，四边形 ODEF和四边形 ABCD都是正方形，点 F在 x轴的正半轴上，点 C在边 DE上，反比例函数 $ y=\frac{k}{x} $ （ $ k\neq0,x>0 $ ）的图像过点 B,E. 若
AB=2，则 k的值为 ___.


![](page=10,bbox=[315, 494, 790, 914])
...
$ \because $反比例函数的图像过点B,E，


解得 $ x_{1}=1+\sqrt{5}, $ $ x_{2}=1-\sqrt{5} $ （舍）


$$

\therefore k = x ^ {2} = 6 + 2 \sqrt {5}
...
如图， $ \Delta P_{1} O A_{1} $ 、 $ \Delta P_{2} A_{1} A_{2} $都是等腰直角三角形，点 $ P_{1} $ 、 $ P_{2} $在函数 $ y=\frac{4}{x} $ （ $
x>0 $ ）的图像上，斜边 $ O A_{1} $ 、 $ A_{1} A_{2} $都在x轴上，求点 $ A_{2} $的坐标.


![](page=10,bbox=[313, 1511, 811, 2026])


【分析】


分别过点 $P_{1}$ 、 $P_{2}$ 做 $x$ 轴的垂线，


根据题意设 $ P_{1} C=a $ ， $ P_{2} D=b $ ，则 $ P_{1} (a,a) $ ， $ P_{2} (2 a+b,b) $ ， $ A_{2} (2 a+2 b,0) $


易得 $ \left\{ \begin{array}{l} a^{2}=4 \\ (2a+b)b=4 \end{array} \right. $


解得 a=2， b=2 $ \sqrt{2}-2 $


所以 $A_{2}\left(4\sqrt{2},0\right)$


![](page=11,bbox=[48, 55, 260, 121])
