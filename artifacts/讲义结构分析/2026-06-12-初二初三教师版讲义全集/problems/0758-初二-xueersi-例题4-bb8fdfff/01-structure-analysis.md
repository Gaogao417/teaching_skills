# 结构分析：初二 xueersi 例题4：几何关系与辅助线/图形不变量

## 原题
【例题4】 ★★★☆☆（总结： $ \left\{\begin{array}{l l} {\mathrm {上 底} + \mathrm {下 底}} \\ {\mathrm {对 角 线 相 互 垂 直}} \end{array} \right. \Rightarrow $ 平移腰）

已知：如图，等腰梯形ABCD中，AB//CD，对角线AC $ \bot $ BD于O， $ BC=1 3 \sqrt{2} $设 AB=a，CD=b，a+b=34.求a、b的值.

分析】过点C作CE//DB交AB的延长线于E.

$ \because B D \bot A C $ $ \therefore C E \bot A C $

$ \because $四边形ABCD为等腰梯形， $ \therefore A C=B D $

$ \because A B / / C D $ $ \therefore $四边形DCEB是平行四边形

$ \therefore B E=C D $ $ \therefore A E=A B+B E=A B+C D=3 4 $

作CF $ \bot $ AB于F，

则CF是等腰直角三角形ACE斜边上的中线，

$ \therefore C F=1 7 $，

在Rt $ \triangle C B F $中，由勾股定理可得 $ B F=\sqrt{B C^{2}-C F^{2}} $

$ \because B F=\frac{1}{2} \left( A B-C D \right)=7 $ $ \therefore A B-C D=1 4 $

$ \because A B+C D=3 4 $ $ \therefore A B=2 4 $ $ CD=1 0 $

即a=24，b=10.

- 来源讲义：（数）2021年春八年级培优体系_教师版_第4讲_梯形综合（敏学班）
- 来源定位：page=3；例题4
- 主题标签：几何综合、综合题、OCR需复核
- OCR/图形状态：需人工复核：数字可能被拆空格；含图形条件，需对照原图；选择项/表格排版可能丢列；题目依赖图形，仅生成 diagram_request_packet，不生成图片；源 YAML 邻近文本未抽取到可靠【分析】片段。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
- 面积/方程/关系式：未能从源 YAML 稳定抽取，需人工复核。
- 完整求解过程：

需人工复核/无法锁定：源 YAML 邻近文本未抽取到可靠【分析】片段，本文件不编造标准解。

- 最终答案：无法锁定。
- 排除值：OCR/图形未复核前，保留潜在排除值。
- 退化情形：需检查点重合、边界位置、图形方向或函数交点退化。
- 验算：把结果代回原条件；应用题检查单位和实际意义；几何/函数题检查图形位置与定义域。
- 本题最短可靠路径：未在源 YAML 附近稳定抽取到【分析】片段，无法锁定完整标准解，需人工回源复核。
...
```json
{
  ...
  "canonical_solution": {
    ...
    "verification": "无法验算；需人工锁定标准解。",
    "shortest_reliable_path": "回源确认题面与答案后再确定最短路径。"
  },
  "common_blockers": {
