...
## 一、题目场景
- 数学对象：命题改写与证明逻辑中的题面对象、变量、图形/函数/方程关系。
...
(1) 如果有垂直于同一条直线的两条直线，那么这两条直线互相平行；


(2) 如果两个角是同一个角的补角，那么这两个角相等；


(3) 如果两条直线平行于同一直线，那么这两条直线互相平行.


![](page=4,bbox=[69, 36, 283, 103])
...
## 七、变式原则
- 核心不变量：保持“命题改写与证明逻辑”这一核心结构不变，只改变一个表层维度。
- 表层特征：证明与判定、OCR需复核。
...
```json
{
  ...
  "variation_rules": {
    "core_invariant": "保持“命题改写与证明逻辑”这一核心结构不变，只改变一个表层维度。",
    "surface_features": [
      "证明与判定",
      "OCR需复核"
    ],
