...
注意到 $ \angle B C A-\angle A C D=6 0^{\circ} $ ，这提示我们可以进行对称变换以“创造”出 $ 6 0^{\circ} $角以AC为对称轴将 $ \Delta D A C $翻折到 $
\Delta D^{\prime} A C $的位置，连接BD'.则 $ CD^{\prime}=CD=BC $ $ \angle B C D^{\prime}=\angle B C A-\angle A C D=\angle B C
A-\angle A C D^{\prime}=6 0^{\circ} $ ，故 $ \Delta D^{\prime} B C $为等边三角形从而 $ AD+CD=AD^{\prime}+D^{\prime} B\geq AB $
，等号成立时AC平分 $ \angle B A D. $


![](page=5,bbox=[69, 30, 283, 97])
