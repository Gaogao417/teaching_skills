...
- 来源讲义：（数）2021年春八年级培优体系_教师版_第3讲_平行四边形综合（敏学班）
- 来源定位：page=3；例题6
...
【分析】过点B作 $ l_{1} $的垂线交 $ l_{1} $、 $ l_{3} $于点E、F，得 $ \triangle AEB\cong \triangle BFC $三垂直全等，所以 $ A B^{2}=A E^{2}+B
E^{2}=B F^{2}+B E^{2}=7^{2}+5^{2}=4 9+2 5=7 4 $


(2) 如图，A在线段 BG上，ABCD和DEFG都是正方形，面积分别为 $ 7 c m^{2} $和 $ 1 1 c m^{2} $ ，则 $ \triangle C D E $的面积等于___.


![](page=3,bbox=[325, 1828, 763, 2139])


【分析】 $ \sqrt{7} c m^{2} $


![](page=4,bbox=[69, 27, 284, 95])
