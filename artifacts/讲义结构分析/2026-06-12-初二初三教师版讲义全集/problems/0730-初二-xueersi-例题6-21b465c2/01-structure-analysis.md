...
解方程组： $ \left\{ \begin{array}{l l} x+y+\frac{9}{x}+\frac{4}{y}=1 0 \\ \left(x^{2}+9\right)\left(y^{2}+4\right)=2 4 x y \end{array} \right. $

- 来源讲义：（数）2021年春八年级培优体系_教师版_第2讲_方程与方程组综合（敏学班）
- 来源定位：page=3；例题6
- 主题标签：方程与方程组、综合题、OCR需复核
- OCR/图形状态：需人工复核：数字可能被拆空格。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
【分析】 原方程组化为 $ \left\{\begin{array}{l l} \left(x+\frac{9}{x}\right)+\left(y+\frac{4}{y}\right)=1 0 \\
\left(x+\frac{9}{x}\right)\left(y+\frac{4}{y}\right)=2 4 \end{array} ; \right. $


由Viete定理，原方程组的 $ x+\frac{9}{x} $、 $ y+\frac{4}{y} $可以看作一元二次方程 $ z^{2}-1 0 z+2 4=0 $


的两个根； $ z=4 $或 $ z=6 $； $ \left\{\begin{array}{l l} x+\frac{9}{x}=4 \\ y+\frac{4}{y}=6 \end{array} \right. $或 $
\left\{\begin{array}{l l} x+\frac{9}{x}=6 \\ y+\frac{4}{y}=4 \end{array} \right. $


当 $ \left\{\begin{array}{l l} x+\frac{9}{x}=4 \\ y+\frac{4}{y}=6 \end{array} \right. $， $ \left\{\begin{array}{l l}
x^{2}-4 x+9=0 \\ y^{2}-6 y+4=0 \end{array} \right. $ $ \Delta_{x}=(-4)^{2}-4\times1\times9=-2 0<0 $，无实数解；


当 $ \left\{\begin{array}{l l} x+\frac{9}{x}=6 \\ y+\frac{4}{y}=4 \end{array} \right. $， $ \left\{\begin{array}{l l}
x^{2}-6 x+9=0 \\ y^{2}-4 y+4=0 \end{array} \right. $， $ \left\{\begin{array}{l l} x=3 \\ y=2 \end{array} \right. ; $


![](page=4,bbox=[33, 91, 245, 160])


经检验，原方程组的解是 $ \left\{\begin{array}{l}x=3\\ y=2\end{array}\right. $或者对第二个方程去括号化简为 $
\left(xy-6\right)^{2}+\left(2x-3y\right)^{2}=0 $，从而算出 x,y的值，用第一个方程进行检验.
