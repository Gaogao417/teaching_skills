# 结构分析：初二 xueersi 例题9：函数/坐标关系建模

## 原题
【例题9】 ★★★★☆

已知直线 y=x+3的图像与x、y轴交于A、B两点，直线l经过原点，与线段AB交于点C，把 $ \triangle A O B $的面积分为2:1的两部分，求直线l的解析式.

- 来源讲义：第6讲_一次函数的图像与性质-教师版（勤思班）
- 来源定位：page=7；例题9
...
由题意，得 $ A(-3,0), $ $ B(0,3), $ ，设直线l的解析式为 $ y=kx $ ，设C(m,m+3)


$ \because $直线 l把 $ \triangle A O B $的面积分为2:1的两部分； $ \therefore $ $ S_{\triangle A O C}:S_{\triangle A O B}=1:3 $或2:3


当 $ \mathrm{S}_{\triangle A O C}:\mathrm{S}_{\triangle A O B}=1:3 $时，即 $ \mathrm{S}_{\triangle A O
C}:\mathrm{S}_{\triangle A O B}=\left[\frac{1}{2}\cdot 3\cdot(m+3)\right]:(\frac{1}{2}\cdot 3\cdot 3)=(m+3):3=1:3 $ $
\therefore m=-2 $ $ \therefore C(-2,1) $ ， $ 1=-2\mathrm{k} $ $ k=-\frac{1}{2} $ ，此时直线l的解析式为 $ y=-\frac{1}{2}x $当 $
\mathrm{S}_{\triangle A O C}:\mathrm{S}_{\triangle A O B}=2:3 $时，即 $ \mathrm{S}_{\triangle A O
C}:\mathrm{S}_{\triangle A O B}=\left[\frac{1}{2}\cdot 3\cdot(m+3)\right]:(\frac{1}{2}\cdot 3\cdot 3)=(m+3):3=2:3 $ $
\therefore m=-1 $ $ \therefore C(-1,2) $ ， $ 2=-k $ $ k=-2 $ ，此时直线l的解析式为 $ y=-2x $ $ \therefore $直线l的解析式为 $
y=-\frac{1}{2}x $或 $ y=-2x $
