...
## 原题
【例题2】 $ \star\star\star\star\star $如图，矩形ABCD中，AB=6，AD=8，P，E分别是线段AC、BC上的点，且四边形PEFD为矩形.
...
- OCR/图形状态：需人工复核：含图形条件，需对照原图；选择项/表格排版可能丢列；题目依赖图形，仅生成 diagram_request_packet，不生成图片；源 YAML 邻近文本未抽取到可靠【分析】片段。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
