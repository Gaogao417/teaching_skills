...
(2) 当 k=6时，求下列关于方程两根 $ x_{1} $ 、 $ x_{2} $的代数式的值： $ \textcircled{1} x_{1}^{2}+x_{2}^{2} $ $ \textcircled{2} $ $ x_{1}^{3}+x_{2}^{3} $ $ \textcircled{3} $ $ | x_{1}-x_{2} | $.

- 来源讲义：第10讲_暑假复习_教师版（敏学班）
- 来源定位：page=2；例题4
- 主题标签：一元二次方程
- OCR/图形状态：需人工复核：选择项/表格排版可能丢列。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
(1)因为关于 x的一元二次方程 $ \left(\frac{1}{2} k-1\right)x^{2}-(k+1)x+\frac{1}{2} k+1=0 $有实数根，


所以 $ \left\{\begin{array}{l l} {\frac{1}{2} k-1\neq 0} \\ {\triangle=[-(k+1)]^{2}-4\left(\frac{1}{2}
k-1\right)\left(\frac{1}{2} k+1\right)\geq 0} \\ \end{array} \right. $，


解得 k $ \geq -\frac{5}{2} $且 k $ \neq 2 $


(2) $ k=6 $时，一元二次方程为： $ 2 x^{2}-7 x+4=0 $ $ x_{1}^{2}+x_{2}^{2}=\frac{3 3}{4}, $ $ x_{1}^{3}+x_{2}^{3}=\frac{1 7
5}{8}, $ $ |x_{1}-x_{2}|=\frac{\sqrt{1 7}}{2} $


![](page=3,bbox=[71, 35, 283, 103])
