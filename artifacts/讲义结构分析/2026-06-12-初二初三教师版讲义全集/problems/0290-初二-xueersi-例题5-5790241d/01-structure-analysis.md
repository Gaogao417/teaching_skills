...
(1) $ \Delta=4 m^{2}-4 \big( m+1 \big) \big( m-1 \big)=4>0 $


$ \therefore $无论 m取何值，这个方程总有实数根；


(2) 由 $ x^{2}-2mx+\left(m+1\right)\left(m-1\right)=0 $得： $ \left(x-m-1\right)\left(x-m+1\right)=0 $


方程的两根为： $ x_{1}=m+1 $ $ x_{2}=m-1 $


$\textcircled{1}$当3为底边时： $m + 1 = m - 1$ ，无解；


$ \textcircled{2} $当 m+1=3时，m=2，三边为1、3、3，此时周长为7；


$ \textcircled{3} $当 m-1=3时，m=4，三边为3、3、5，此时周长为11.
