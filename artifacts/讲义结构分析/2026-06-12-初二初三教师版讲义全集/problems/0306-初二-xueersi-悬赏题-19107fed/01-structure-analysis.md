# 结构分析：初二 xueersi 悬赏题：一元二次方程判别式/根系关系
...
- 来源讲义：第3讲_一元二次方程根与系数的关系_教师版（勤思班）
- 来源定位：page=5；悬赏题
- 主题标签：一元二次方程、综合题
- OCR/图形状态：需人工复核：OCR文本较短，需复核完整性。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
...
【分析】 由 $ \Delta=4 p^{2}-4>0 $得， p>1或 p<-1


设方程两个根为 $x_{1}, x_{2}$


又韦达定理知 $x_{1} + x_{2} = -2p,x_{1}x_{1} = 1$


即 $ \left(x_{1}-1\right)\left(x_{2}-1\right)=x_{1}x_{2}-\left(x_{1}+x_{2}\right)+1=2+2 p<0 $
...
一元二次方程 $ x^{2}-3 x+1=0 $的两根分别是 $ x_{1}, x_{2} $ ，则 $ x_{1}^{2} x_{2}+x_{1} x_{2}^{2} $的值是（    )
...
A. 由韦达定理知， $ x_{1}+x_{2}=3, x_{1}x_{2}=1, $ $ \therefore
x_{1}^{2}x_{2}+x_{1}x_{2}^{2}=x_{1}x_{2}\left(x_{1}+x_{2}\right)=1\times 3=3. $


【巩固2】


已知方程 $ x^{2}-3 x-5=0 $的两根为 $ x_{1}, x_{2} $，求：(1) $ x_{1}^{2}+x_{2}^{2} $；(2) $ x_{1}^{3}+x_{2}^{3}. $


【分析】


由韦达定理得： $ x_{1}+x_{2}=3 $ ， $ x_{1}\cdot x_{2}=-5. $


(1) $ x_{1}^{2}+x_{2}^{2}=\left(x_{1}+x_{2}\right)^{2}-2 x_{1} x_{2}=9-2 \times(-5)=1 9. $


$$

\begin{array}{l} x _ {1} ^ {3} + x _ {2} ^ {3} = \left(x _ {1} + x _ {2}\right) \left(x _ {1} ^ {2} - x _ {1} x _ {2}
+ x _ {2} ^ {2}\right) \\ = 3 \times (1 9 + 5) \\ = 7 2 \\ \end{array}
...
设 m为实数，则关于 x的方程 $ x^{2}-\left(m^{2}+1\right)x-3=0 $的两个根一定是（    ）
...
C. $ \Delta=\left(m^{2}+1\right)^{2}-4\cdot\left(-3\right)>0 $ ，由韦达定理知 $ x_{1}x_{2}=-3<0 $ 故选C.


【巩固4】


已知关于 x的方程 $ x^{2}+2(m+2)x+m^{2}-5=0 $有两个实数根，并且这两个根的平方和比这两个根的积大16，求 m的值.


【分析】


有实数根，则 $ \triangle\geq0 $ ，且 $ x_{1}^{2}+x_{2}^{2}=x_{1}x_{2}+16 $ ，联立解得 m的值.
...
\left\{ \begin{array}{l} x _ {1} + x _ {2} = - 2 (m + 2) \\ x _ {1} x _ {2} = m ^ {2} - 5 \\ x _ {1} ^ {2} + x _ {2} ^
{2} = x _ {1} x _ {2} + 1 6 \\ \Delta = 4 (m + 2) ^ {2} - 4 \left(m ^ {2} - 5\right) \geq 0 \end{array} \right.

$$


解得： m=-1或 m=-15，又由 $ \textcircled{4} $可知 m $ \geq-{\frac{9}{4}} $ $ \therefore m=-15 $舍去，故 m=-1


![](page=7,bbox=[69, 36, 282, 103])


## 【巩固5】


已知关于 x的方程 $ x^{2}-a x+a-2=0 $ ，分别写出下列情形中 a 所满足的条件：(1)方程有两个正实数根；(2)方程两根异号.


【分析】


$ \Delta = ( a - 2 )^{2} + 4 > 0 $ ，所以不论a取何值，方程都有两个不等的实数根.


(1) 由根与系数的关系可得 $\left\{ \begin{array}{l l} a > 0 \\ a - 2 > 0 \end{array} \right.$ ，解得 $a > 2$；


(2) 两根异号积小于零，即 $ a-2<0 $ ， $ a<2. $


![](page=8,bbox=[70, 36, 282, 104])
