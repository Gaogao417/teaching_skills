# 结构分析：初二 xueersi 例题5：一元二次方程判别式/根系关系

## 原题
【例题5】 ★★★★☆

设 $ x_{1}, x_{2} $为关于 x的方程 $ x^{2}-2 p x-p=0 $的两根， p为实数.

(1) 求证： $ 2 p x_{1}+x_{2}^{2}+3 p\geq 0. $

(2) 当 $ \left|x_{1}-x_{2}\right| \leq\left|2 p-3\right| $时，求 p的最大值.
...
(1) $\Delta = 4p^{2} + 4p\geq 0$ ，解得 $p\geq 0$ 或 $p\leq -1$


由韦达定理知， $ x_{1}+x_{2}=2 p $ ， $ x_{1}x_{2}=-p $
...
\left| x _ {1} - x _ {2} \right| \leq \left| 2 p - 3 \right|

$$


平方得： $ \left(x_{1}+x_{2}\right)^{2}-4x_{1}x_{2}\leq 4p^{2}-12p+9 $


$ 4 p^{2}+4 p\leq 4 p^{2}-1 2 p+9 $ ，解得 $ p\leq \frac{9}{1 6} $ ，故 $ p_{\mathrm{max}}=\frac{9}{1 6}. $
