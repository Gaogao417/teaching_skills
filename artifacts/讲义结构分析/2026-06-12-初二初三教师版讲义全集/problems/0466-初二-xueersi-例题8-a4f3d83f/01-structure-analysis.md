# 结构分析：初二 xueersi 例题8：函数/坐标关系建模

## 原题
【例题8】 ★★★★☆

已知直线 $ y=x+3 $的图像与x、y轴交于A、B两点，直线l经过原点，与线段AB交于点C，

把 $ \triangle AOB $的面积分为2：1的两部分，求直线l的解析式.

- 来源讲义：第6讲_一次函数的图像与性质-教师版（敏学班）
- 来源定位：page=6；例题8
- 主题标签：函数、动点与存在性、证明与判定、综合题、OCR需复核
...
## 二、核心结构
- 表面考点：函数、动点与存在性、证明与判定、综合题、OCR需复核。
...
由题意，得 $ A(-3,0), $ $ B(0,3), $ ，设直线 l的解析式为 $ y=kx $ ，设 $ C(m,m+3) $ $ \because $直线 l把 $ \triangle AOB $的面积分为2:1的两部分； $
\therefore \mathrm{S}_{\triangle AOC}:\mathrm{S}_{\triangle AOB}=1:3 $或2:3当 $ \mathrm{S}_{\triangle
AOC}:\mathrm{S}_{\triangle AOB}=1:3 $时，即 $ \mathrm{S}_{\triangle AOC}:\mathrm{S}_{\triangle
AOB}=\left[\frac{1}{2}\cdot 3\cdot(m+3)\right]:\left(\frac{1}{2}\cdot 3\cdot 3\right)=(m+3):3=1:3 $ $ \therefore m=-2,
$ $ \therefore C(-2,1), $ $ 1=-2\mathrm{k}, $ $ k=-\frac{1}{2} $ ，此时直线 l的解析式为 $ y=-\frac{1}{2}x $当 $
\mathrm{S}_{\triangle AOC}:\mathrm{S}_{\triangle AOB}=2:3 $时，即 $ \mathrm{S}_{\triangle AOC}:\mathrm{S}_{\triangle
AOB}=\left[\frac{1}{2}\cdot 3\cdot(m+3)\right]:\left(\frac{1}{2}\cdot 3\cdot 3\right)=(m+3):3=2:3 $ $ \therefore m=-1,
$ $ \therefore C(-1,2), $ $ 2=-k, $ $ k=-2 $ ，此时直线 l的解析式为 $ y=-2x $ $ \therefore $直线 l的解析式为 $ y=-\frac{1}{2}x $或 $
y=-2x $


![](page=8,bbox=[70, 47, 284, 114])
...
(1) 一次函数 $ y=2 x+1 $的图像不经过（    ）


A. 第一象限B. 第二象限C. 第三象限D. 第四象限


(2) 如果直线 $ y=ax+b $经过第一、二、四象限，那么 ab ___ 0（填 >， <， =）


(3) 如果直线 y=ax-b经过一、三、四象限，那么直线 y=bx+a经过第___象限；直线 y=- $ \frac{b}{a} $ x经过第___象限.


【分析】 (1) D；(2) ab<0；(3）一、二、三；二、四


【巩固2】


一次函数 $ y=(k+1)x+k-2 $的图像经过一、三、四象限，求 k的取值范围.


【分析】因为一次函数 $ y=(k+1)x+k-2 $的图像经过一、三、四象限；


所以 $\left\{ \begin{array}{l} k + 1 > 0 \\ k - 2 < 0 \end{array} \right.$， $\left\{ \begin{array}{l} k > -1 \\ k < 2
\end{array} \right.$，得 $-1 < k < 2$；所以 $k$ 的取值范围是 $-1 < k < 2$.


【巩固3】


(1) 下列图像中，不可能是关于 x 的一次函数 $ y=mx-(m-3) $的图像的是（    ）


![](page=8,bbox=[317, 1406, 567, 1699])
...
![](page=8,bbox=[615, 1403, 862, 1694])


![](page=8,bbox=[913, 1406, 1162, 1699])
...
![](page=8,bbox=[1226, 1404, 1477, 1698])
...
(2) 如图，直线 $ l_{1}: y=ax+b $和 $ l_{2}: y=bx-a $，在同一坐标系中的图像大致是（    ）


![](page=8,bbox=[318, 1868, 656, 2129])
...
![](page=8,bbox=[987, 1851, 1251, 2128])


C.


![](page=8,bbox=[1290, 1868, 1534, 2134])
...
(1) C，将解析式变为 $ y=mx+(3-m) $较易判断


(2) C，可由 $ l_{1} $的图像来判断a，b符号，再去判断 $ l_{2} $的位置.


![](page=9,bbox=[70, 48, 283, 114])


【巩固4】


已知一个一次函数中自变量 x=-2时， y=-1，当 x=2时， y=11，则这个一次函数的解析式为___.
...
因为当 $x = -2$ 时， $y = -1$ ，当 $x = 2$ 时， $y = 11$ ；


所以 $ \left\{ \begin{array}{l l} - 1 = k \times(-2)+b \\ 1 1 = k \times 2+b \end{array} \right. $ ， $ \left\{
\begin{array}{l l} k=3 \\ b=5 \end{array} \right. ; $
...
如果一次函数的自变量 x的取值范围是 $ - 2 \leq x \leq 6 $ ，相应函数值的范围是 $ - 1 1 \leq y \leq 9 $ .求此函数的解析式.
...
(1)当 x=-2时， y=-11，当 x=6时， y=9；


有 $ \left\{\begin{array}{l l}-1 1=k \times(-2)+b\\9=k \times 6+b\end{array}\right. $ ，得 $ \left\{\begin{array}{l
l}k=\frac{5}{2}\\b=-6\end{array}\right. $


所以一次函数的解析式是 y $ =\frac{5}{2} x-6. $


(2)当 x=-2时，y=9，当 x=6时，y=-11；有 $ \left\{\begin{array}{l l}9=k \times(-2)+b\\-11=k \times 6+b\end{array}\right. $ ，得 $
\left\{\begin{array}{l l}k=-\frac{5}{2}\\b=4\end{array}\right. $


所以一次函数的解析式是 $ y=-\frac{5}{2} x+4. $


![](page=10,bbox=[70, 47, 283, 114])
...
- 表层特征：函数、动点与存在性、证明与判定、综合题、OCR需复核。
