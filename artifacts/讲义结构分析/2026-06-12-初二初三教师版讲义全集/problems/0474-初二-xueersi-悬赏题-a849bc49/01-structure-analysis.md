...
联结AC，易知 $ A B=B C,\angle B=6 0^{\circ} $ ，故 $ \triangle A B C $为等边三角形，同理 $ \triangle A C D $亦为等边三角形 $ \therefore
\angle B A C=6 0^{\circ}=\angle M A N $ ， $ A B=A C $


$$

\therefore \angle B A M = \angle C A N \because A B = A C, \angle A B M = \angle A C N = 6 0 ^ {\circ}
...
\therefore \triangle A B M \cong \triangle A C N (A. S. A)
...
\therefore A M = A N, \text {又} \because \angle M A N = 6 0 ^ {\circ}
