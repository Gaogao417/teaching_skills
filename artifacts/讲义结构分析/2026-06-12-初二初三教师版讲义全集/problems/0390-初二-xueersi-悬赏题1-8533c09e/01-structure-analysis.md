# 结构分析：初二 xueersi 悬赏题1：几何关系与辅助线/图形不变量

## 原题
【悬赏题1】 ★★★★☆

如图 $ \textcircled{1} $，已知点 D在 AB上， $ \triangle ABC $和 $ \triangle ADE $都是等腰直角三角形， $ \angle ABC=\angle ADE=90^{\circ} $且 M为EC的中点.

(1) 求证： $ \triangle BMD $为等腰直角三角形.

(2) 将 $ \triangle A D E $绕点A再逆时针旋转 $ 9 0^{\circ} $时（如图 $ \textcircled{2} $所示位置）， $ \triangle B M D $为等腰直角三角形的结论是否仍成立？若成立，请证明：若不成立，请说明理由.

图 $ \textcircled{1} $

图 $ \textcircled{2} $

- 来源讲义：第9讲_几何综合（二）_教师版（勤思班）
- 来源定位：page=6；悬赏题1
- 主题标签：几何综合、证明与判定、综合题、OCR需复核
...
(1) 延长 DM交 BC于 N，根据平行线的性质和判定推出 $ \angle D E M=\angle M C B $ ，根据推出 $ \triangle E M D\cong \triangle C M N $ ，证出
CN=AD即可；


(2) 作CN//DE交DM的延长线于N，连接BN，根据平行线的性质求出 $ \angle E=\angle N C M $ ，根据A.S.A证 $ \triangle D B A\cong \triangle N B C $ ，推出
$ \triangle D B N $是等腰直角三角形，根据等腰直角三角形的性质即可推出 $ \triangle B M D $为等腰直角三角形.


![](page=7,bbox=[68, 33, 282, 104])


![](page=8,bbox=[69, 34, 282, 103])
...
如图， $ \triangle A C B $和 $ \triangle E C D $都是等腰直角三角形， $ \angle A C B=\angle E C D=9 0^{\circ} $ ，D为AB边上一点，求证：(1） $
\triangle A C E\cong \triangle B C D $ (2） $ A D^{2}+D B^{2}=D E^{2} $


![](page=8,bbox=[308, 654, 773, 968])


【分析】


(1) 因为 $ \angle A C B=\angle E C D=9 0^{\circ} $所以 $ \angle A C D+\angle B C D=\angle A C D+\angle A C E $即 $ \angle B
C D=\angle A C E $因为 BC=AC，DC=EC，所以 $ \triangle A C E\cong \triangle B C D $


(2) 因为 $ \triangle A C B $是等腰直角三角形，所以 $ \angle B=\angle B A C=4 5^{\circ} $


因为 $ \triangle A C E\cong \triangle B C D $所以 $ \angle B=\angle C A E=4 5^{\circ} $


所以 $ \angle D A E=\angle C A E+\angle B A C=9 0^{\circ} $


所以 $ A D^{2}+A E^{2}=D E^{2} $


由(1)知 AE=DB


所以 $ A D^{2}+D B^{2}=D E^{2} $


【巩固2】


如图，等腰 Rt $ \triangle A B C $中， $ \angle A B C=9 0^{\circ} $ ，O是 $ \triangle A B C $内一点， $ O A=6 $ ， $ O B=4 \sqrt{2} $
， $ O C=1 0 $ $ O^{\prime} $为 $ \triangle A B C $外一点，且 $ \triangle C B O\cong \triangle A B O^{\prime} $ ，则四边形
AO'BO的面积为（    ）


![](page=8,bbox=[311, 1962, 711, 2274])
...
因为 $ \triangle C B O\cong \triangle A B O^{\prime} $ ，所以 $ O B=O^{\prime} B=4\sqrt{2} $ $ O C=O^{\prime} A=1 0 $ $
\angle O B C=\angle O^{\prime} B A $


![](page=9,bbox=[69, 35, 282, 103])


所以 $ \angle O^{\prime}BO=90^{\circ} $ ，所以 $ OO^{\prime}=8 $
...
如图，边长为1的正方形ABCD的对角线交于点O，点E是边AB上一动点，点F在边BC上且满足OE $ \bot $ OF，在点E由A运动到B的过程中，以下结论：


$ \textcircled{1} $线段 OE的大小先变小后变大；


$ \textcircled{2} $线段EF的大小先变大后变小；


$ \textcircled{3} $四边形 OEBF的面积先变大后变小.


正确的个数为（    ）.


![](page=9,bbox=[309, 920, 660, 1275])
...
因为 OE $ \bot $ OF，所以 $ \angle A O E+\angle E O B=\angle F O B+\angle E O B=9 0^{\circ} $即 $ \angle A O E=\angle B O F
$证明 $ \triangle A O E\cong \triangle B O F $


所以 $ S_{四边形OEBF}=S_{\triangle OEB}+S_{\triangle BOF}=S_{\triangle OEB}+S_{\triangle AOE}=S_{\triangle AOB} $是定值，故 $
\textcircled{3} $错


所以 $ O E=O F $ ，所以 $ E F=\sqrt{2}O E $


E在从A到B的过程中，OE先从大变小，再从小变大，故 $ \textcircled{1} $对


所以 $ E F=\sqrt{2} O E $先从大变小，再从小变大，故 $ \textcircled{2} $错。故选B


## 【巩固4】


如图，已知BD是正方形ABCD的对角线，AE，AF与BD相交于点P，Q，与BC相交于点E，与DC相交于点F．若EF=BE+DF，PB=2，DQ=3，则PQ的值为（    ）。


![](page=10,bbox=[69, 33, 282, 103])


![](page=10,bbox=[310, 252, 639, 586])


A. 3 B. $ \sqrt{5} $ C. $ \sqrt{1 3} $ D. 4


【分析】


根据夹半角的两条基本结论可得 $ P Q=\sqrt{P B^{2}+D Q^{2}}=\sqrt{1 3} $


【巩固5】


已知：如图， $ \triangle A B C $中， $ \angle C=90^{\circ} $ ，D是AB上一点，DE $ \bot $ CD于D，交BC于E，且有 $ AC=AD=CE $ .求证： $
DE=\frac{1}{2} CD $ .


![](page=10,bbox=[312, 1095, 768, 1376])


![](page=10,bbox=[793, 1084, 1301, 1373])


【分析】


在CD上取中点F， $ \triangle A C F\cong \triangle C E D $ ，故 DE $ = \frac{1}{2} C D $
