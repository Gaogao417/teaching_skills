...
如图，等腰梯形 ABCD中， AB//CD， $ \angle D A B=6 0^{\circ} $ ，AC平分 $ \angle D A B $ ，且 $ AC=2 \sqrt{3} $ ，则梯形 ABCD的周长等于___.
...
【分析】 $ \because $等腰梯形ABCD中， $ \angle D A B=6 0^{\circ} $ ，AC平分 $ \angle D A B $ $ \therefore \angle C A B=\angle C A
D=\angle A C D=3 0^{\circ} $ $ \therefore \angle A C B=9 0^{\circ} $ $ \because AC=2 \sqrt{3} $ $ \therefore BC=2 $
AB=4 $ \therefore AD=DC=2 $ ， $ \therefore $梯形ABCD的周长等于10.


![](page=4,bbox=[69, 28, 283, 96])
