...
(1)已知函数 $ y=kx(k\neq 0) $的图像经过 $ P_{1}\left(x_{1},y_{1}\right) $ $ P_{2}\left(x_{2},y_{2}\right) $ ，且 $ x_{1}0

(2) 已知正比例函数 $ y = \left(a + 1\right)x^{a^{2} + a + 1} $ ，那么它的图像经过哪些象限？

(3) 正比例函数 $ y=m x^{m^{2}+m} $的图像经过第一、三象限，求 m的值.

- 来源讲义：第6讲_正比例函数与反比例函数（一）_教师版（敏学班）
- 来源定位：page=2；例题2
- 主题标签：函数
...
(2) 由题意得 $ \left\{\begin{array}{l l}a^{2}+a+1=1\\ a+1\neq0\end{array}\right. $ ，解得 a=0，则函数解析式为 y=x，图像经过第一、三象限.


(3) $ \because $此函数为正比例函数， $ \therefore \left\{ \begin{array}{l l} m^{2}+m=1 \\ m\neq 0 \end{array} \right. $ ，解得 $
m=\frac{-1\pm \sqrt{5}}{2} $


$ \because $此函数经过第一，三象限， $ \therefore m > 0 $ $ \therefore m=\frac{\sqrt{5}-1}{2} $


![](page=3,bbox=[69, 35, 283, 104])
