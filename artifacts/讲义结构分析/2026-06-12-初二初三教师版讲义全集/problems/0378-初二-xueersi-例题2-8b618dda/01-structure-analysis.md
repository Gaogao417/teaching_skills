...
- 来源讲义：第8讲_几何综合（一）_教师版（敏学班）
- 来源定位：page=3；例题2
- 主题标签：动点与存在性、几何综合、综合题、OCR需复核
- OCR/图形状态：需人工复核：含图形条件，需对照原图；题干含空格/填空占位；题目依赖图形，仅生成 diagram_request_packet，不生成图片；源 YAML 邻近文本未抽取到可靠【分析】片段。本结构分析只锁定 OCR 文本可见条件，不补造图中缺失条件。
