...
四边形OABC中， $ BC//OA $ $ \angle OAB=90^{\circ} $ $ OA=6 $ ，腰AB上有一点D， $ AD=3 $ $ BD=a $ ，四边形ODBC的面积为18，建立如图所示的平面直角坐标系，反比例函数 $ y=\frac{k}{x}(x>0) $的图像恰好经过点C和点D.
...
(1) 反比例函数的解析式为： $ y=\frac{1 8}{x} $；


(2) BD=a， $ B\big(6,a+3\big) $ ，延长BC交y轴与点E则 $ S_{ABE O}=6\left(a+3\right) $ ，又 $ S_{\triangle COE}=S_{\triangle AOD}=9
$
...
(3) 分 $ PC=PD $和 $ DC=DP $两种情况考虑，即可得解.


P点的坐标为（0,0）或（3,0）.此题会出现三点共线的情况.


![](page=5,bbox=[69, 45, 283, 110])
