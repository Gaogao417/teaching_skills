...
解无理方程： $ 2 x+1+x\sqrt{x^{2}+2}+(x+1)\sqrt{x^{2}+2 x+3}=0. $
...
【分析】设 $ a=\sqrt{x} $ ， $ b=\sqrt{x+2} $ ， $ a^{2}+b^{2}=2x+2 $ （ $ a>0,b>0 $ ）则原方程可化为： $ (a+b)^{2}+(a+b)-6=0 $ ，解得： $
a+b=2 $ 或 $ a+b=-3 $ （舍）当 $ a+b=2 $时，即 $ \sqrt{x}+\sqrt{x+2}=2 $ ，解得： $ x=\frac{1}{4} $ ，经检验： $ x=\frac{1}{4} $
是方程的解，故方程的根为 $ x=\frac{1}{4} $ .
