...
已知关于 x的方程 $ m x^{2}+(2 m-1) x+(m+1)=0 $ $ \textcircled{1} $无实数根，那么请说明方程 $ x^{2}+m x-\frac{3 m+2}{4}=0 $ $ \textcircled{2} $的根

的情况.

- 来源讲义：第2讲_一元二次方程根的判别式_教师版（勤思班）
- 来源定位：page=1；例题1
- 主题标签：一元二次方程
...
由题， $\left\{ \begin{array}{l}m\neq 0\\ \Delta_1 = \left(2m - 1\right)^2 - 4m(m + 1) = -8m + 1 < 0 \end{array} \right.$
，解得 $m > \frac{1}{8}$


所以方程 $ x^{2}+mx-\frac{3m+2}{4}=0 $有两个不相等的实数根.
