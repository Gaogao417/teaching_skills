...
如图，已知 AB=8，P为线段 AB上一个动点，分别以 AP，PB为边在 AB的同侧作菱形 APCD和 PBFE，点 P，C，E在一条直线上， $ \angle D A P=6 0^{\circ} $ ，M，N分别是对角线 AC，BE的中点，当点 P在线段 AB上移动时，点 M，N之间的距离最短为___.
...
联结 PM、PN . 则 $ \angle M P N=6 0^{\circ}+3 0^{\circ}=9 0^{\circ} $


设 $ PA=2 a $ ，则 $ PB=8-2 a $ ， $ PM=a $ ， $ PN=\sqrt{3}(4-a) $


$$

\therefore M N = \sqrt {a ^ {2} + [ \sqrt {3} (4 - a) ] ^ {2}} = \sqrt {4 a ^ {2} - 2 4 a + 4 8} = \sqrt {4 (a - 3) ^
{2} + 1 2},

$$


$ \therefore a=3 $时，MN有最小值，最小值为 $ 2\sqrt{3} $


![](page=5,bbox=[69, 28, 284, 95])
