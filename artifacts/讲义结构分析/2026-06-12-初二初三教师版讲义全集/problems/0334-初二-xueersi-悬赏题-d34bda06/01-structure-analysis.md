...
如图，腰长为10cm等腰Rt $ \triangle ABC $以2cm/s的速度沿直线l向边长为10cm正方形移动，直到 AB与EF重合，设xs时，三角形与正方形重叠部分的面积为 $ y\mathrm{cm}^{2}. $
...
- 来源讲义：第5讲_函数初步_教师版（勤思班）
- 来源定位：page=8；悬赏题
- 主题标签：函数、动点与存在性、几何综合、OCR需复核
...
(1) 当 $ 0\leq x\leq 5 $时， $ y=\frac{1}{2}\cdot 2x\cdot 2x=2x^{2} $；当 $ 5<x\leq 10 $时， $ y=\frac{1}{2}\cdot 10\cdot
10-\frac{1}{2}\left(2x-10\right)^{2}=-2x^{2}+20x $


(2) 当 y=30时，令 $ 2 x^{2}=3 0 $ ，得 $ x=\sqrt{1 5} $


令 $ - 2 x^{2}+2 0 x=3 0 $ ，得 $ x_{1}=5+\sqrt{1 0} $ ， $ x_{2}=5-\sqrt{1 0} $ （舍）故当 y=30时， $ x=\sqrt{1 5} $或 $
x=5+\sqrt{1 0} $ .


![](page=9,bbox=[69, 34, 282, 103])
...
(1) 球的表面积 $ S(\mathrm{cm}^{2}) $与球半径 $ R(\mathrm{cm}) $的关系式是 $ S=4\pi R^{2} $；


(2) 设圆柱的底面半径 R(m)不变，圆柱的体积 $ V ( m^{3} ) $与圆柱的高 h(m)的关系式是 $ V=\pi R^{2} h $。 【分析】


(1) 变量是 S、R；常量是 $ 4\pi $


(2) 变量是 V、h；常量是 $ \pi R^{2} $
...
(1) $ y^{2}=(2 x-1)^{2} $; (2) $ y=\sqrt[3]{x} $; (3) $ |y|=-2x $; (4) $ y=|-3x|. $
...
(1) $ y=\sqrt{3-x} $ (2) $ y=\frac{1}{x^{2}-4} $; (3) $ y=\frac{1}{1+3x^{2}} $; (4) $ y=\frac{\sqrt{3x-4}}{x-1}. $
...
(4) 由 $\left\{ \begin{array}{l} 3x - 4 \geq 0 \\ x - 1 \neq 0 \end{array} \right.$，解得 $\left\{ \begin{array}{l} x \geq
\frac{4}{3} \\ x \neq 1 \end{array} \right.$，因此 $x \geq \frac{4}{3}$.


## 【巩固4】


(1) 打开某洗衣机开关，在洗涤衣服时（洗衣机内无水），洗衣机经历了进水、清洗、排水、脱水四个连续过程，其中进水、清洗、排水时洗衣机中的水量 y （升）与时间 x （分钟）之间满足某种函数关系，其函数图像大致为（    ）


![](page=9,bbox=[326, 2039, 615, 2278])
...
![](page=9,bbox=[986, 2038, 1276, 2276])
...
![](page=9,bbox=[1329, 2039, 1620, 2276])
...
![](page=10,bbox=[68, 34, 282, 103])


(2) 小亮从家O，步行到公交站台B，等公交车去学校C，如下图中的折线表示小亮的行程s （千米）与所花时间t（分）之间的函数关系.下列说法错误的是（    ）


![](page=10,bbox=[316, 359, 1144, 770])
...
(2) 由图像可知，选项A、B正确；对于选项C：由于步行的距离是1000米，用了10分钟，所以他步行的速度100米/分钟，C正确；对于选项D：由于乘车走了7000米，用的时间是14分钟，所以车速是500米/分钟。D错误。答案是D.


## 【巩固5】


(1) 已知函数 $ f ( x )=\frac{1}{x^{2}+1} $ ，那么 $ f (-1)= $ ___.


(2) 已知： $ f\left(x\right)=3x+4x^{2}, $ $ g\left(x\right)=5x-3 $ ，求：


$ \textcircled{1} f ( x )+g ( x ) $ $ \textcircled{2} f (-1)+g ( 2 ) $ $ \textcircled{3} f ( a+1 )-g ( a-1 ) $ 【分析】


(1) $ f(-1)=\frac{1}{2}. $


(2) $ \textcircled{1} f ( x )+g ( x )=\left( 3 x+4 x^{2} \right)+\left( 5 x-3 \right)=4 x^{2}+8 x-3 $;


![](page=11,bbox=[69, 34, 282, 103])


【巩固6】


如图，已知正方形ABCD的边长为3厘米，动点P从点B出发，沿BC、CD、DA方向运动至点A停止，设点P运动的路程为x厘米， $ \triangle A B P $的面积为y平方厘米.


(1) 当动点 P在 BC上运动时，求 y关于 x的解析式及其定义域.


(2) 当动点 P在 DC上运动时，求 y关于 x的解析式及其定义域.


(3) 当 x 取何值时， $ \triangle A B P $的面积为1.5平方厘米.


![](page=11,bbox=[323, 556, 657, 926])


【分析】


(1) $ S=\frac{3}{2} x\left(0<x\leq 3\right) $


(2) $ S=\frac{9}{2}\left(3\leq x\leq 6\right) $
