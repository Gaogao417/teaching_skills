...
若 m为整数，且关于 x的方程 $ m^{2} x^{2}+( m+1 ) x+1=0 $有有理根，求 m的值.
...
当 $m = 0$ 时， $x + 1 = 0$ ，解得， $x = -1$ ，符合题意，


当 m≠0时，由 $ \Delta=(m+1)^{2}-4 m^{2}=-3 m^{2}+2 m+1\geq0 $ ，解得， $ -\frac{1}{3}\leq m\leq1 $


因为 m为整数，所以 $m = 1$


经检验： $ m=1 $ ，方程两根为 $ x_{1}=x_{2}=1 $ ，符合题意.
