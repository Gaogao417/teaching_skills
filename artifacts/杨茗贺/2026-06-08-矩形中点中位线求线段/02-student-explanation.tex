...
\vspace{0pt}
如图，在矩形 $ABCD$ 中，$AB=6$，$BD=10$，$O$ 为对角线 $BD$ 的中点，
$E$ 为边 $BC$ 上一点，连接 $AE$，取 $AE$ 的中点 $F$，连接 $OF$。
若 $BE=2$，则 $OF$ 的长为多少？

\end{minipage}\hfill
\diagramcolfigure{build/diagram/jobs/explanation-orig-prompt/rendered/prompt.png}{0.30\linewidth}{原题图：看清 $O,F$。}
\end{eduProblemBlock}
...
先求矩形的另一条边$\;\to\;$在 $AB$ 上补取中点 $M$$\;\to\;$看 $\triangle ABD$ 中的中位线$\;\to\;$看 $\triangle ABE$ 中的中位线$\;\to\;$用同一直线上的两段相减\end{eduAuxItem}




\begin{eduProblemBlock}[辅助图]
\noindent
\begin{minipage}[t]{0.64\linewidth}
\vspace{0pt}
关键动作：在 $AB$ 上取中点 $M$。
这样 $M,O$ 进入 $\triangle ABD$，$M,F$ 进入 $\triangle ABE$。

\end{minipage}\hfill
\diagramcolfigure{build/diagram/jobs/explanation-solution-annotated/rendered/solution.png}{0.30\linewidth}{补 $M$：$MO=4$，$MF=1$。}
\end{eduProblemBlock}
...
求 $OF$ 的长。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{提示与易错}
        \eduSideHint{入口}{$O,F$ 已经是中点，但还不能直接组成一条中位线；补取 $AB$ 的中点 $M$。}
        \eduSideMistake{点序}{求出 $MO=4$、$MF=1$ 后要相减，不要相加，因为 $F$ 在 $M,O$ 之间。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先求矩形的另一条边}{在直角三角形 $BCD$ 中，$BD=10$，$CD=AB=6$，所以
$$
BC=\sqrt{10^2-6^2}=8.
$$
因此 $AD=BC=8$。
}
      \eduExplainStep{2}{在 $AB$ 上补取中点 $M$}{题目已经给了两个中点：$O$ 是 $BD$ 的中点，$F$ 是 $AE$ 的中点。
为了让它们进入中位线，补取 $M$ 为 $AB$ 的中点。
}
      \eduExplainStep{3}{看 $\triangle ABD$ 中的中位线}{在 $\triangle ABD$ 中，$M$ 是 $AB$ 的中点，$O$ 是 $BD$ 的中点，
所以 $MO$ 是中位线，且
$$
MO=\frac12 AD=\frac12\times 8=4.
$$
}
      \eduExplainStep{4}{看 $\triangle ABE$ 中的中位线}{在 $\triangle ABE$ 中，$M$ 是 $AB$ 的中点，$F$ 是 $AE$ 的中点，
所以 $MF$ 是中位线，且
$$
MF=\frac12 BE=\frac12\times 2=1.
$$
}
      \eduExplainStep{5}{用同一直线上的两段相减}{$MO\parallel AD$，$MF\parallel BE$，且 $AD\parallel BE$，所以 $M,F,O$ 共线。
图中 $F$ 在 $M$ 与 $O$ 之间，因此
$$
OF=MO-MF=4-1=3.
$$
    ...
    \vspace{1mm}
    {\color{edu-blue}\bfseries\small 这一步为什么自然}\par\vspace{1mm}
    \begin{eduBulletList}
      \item $O$ 和 $F$ 已经是两个中点，但它们不在同一个三角形里；补 $AB$ 的中点 $M$，就是为了让 $O$、$F$ 各自进入一个中位线。      \item 最后不是 $4+1$，而是 $4-1$，因为 $MF$ 是从 $M$ 到 $F$，$MO$ 是从 $M$ 到 $O$，$F$ 在 $M,O$ 之间。    \end{eduBulletList}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduBottomGrid}
  ...
  \begin{eduSummaryRight}
    ...
    \begin{eduBulletList}
      \item 看到两个中点但套不上中位线时，先想：能不能补一个中点。      \item 平行线间的中点连线，常常等于两条对应底边的一半之差。    \end{eduBulletList}
    \end{eduSummaryBlock}
  \end{eduSummaryRight}
\end{eduBottomGrid}
