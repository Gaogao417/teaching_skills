...
\end{minipage}\hfill
\diagramcolfigure{build/diagram/jobs/explanation-v2-orig-prompt/rendered/prompt.png}{0.30\linewidth}{原题图：看清 $O,F$。}
\end{eduProblemBlock}
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{为什么补 $M$}{$O$ 在 $BD$ 上，$F$ 在 $AE$ 上；补 $AB$ 的中点 $M$，才能分别组成 $\triangle ABD$\ \ 和 $\triangle ABE$ 的中位线。}
        \eduSideHint{看哪两个三角形}{先看大三角形 $\triangle ABD$ 得 $MO$，再看小三角形 $\triangle ABE$ 得 $MF$。}
        \eduSideMistake{为什么相减}{$M,F,O$ 共线，且 $F$ 在 $M$ 与 $O$ 之间，所以 $OF=MO-MF$，不是 $MO+MF$。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先求矩形的另一条边}{由矩形得 $CD=AB=6$，在 $\triangle BCD$ 中，
$$
BC=\sqrt{BD^2-CD^2}=\sqrt{10^2-6^2}=8.
$$
所以 $AD=BC=8$。
}
      \eduExplainStep{2}{在 $AB$ 上补取中点 $M$}{取 $M$ 为 $AB$ 的中点。
}
      \begin{center}
        \includegraphics[width=0.58\linewidth]{\detokenize{build/diagram/jobs/explanation-v2-solution-annotated/rendered/solution.png}}
        \par\vspace{0.6mm}{\scriptsize\color{edu-diagram-muted}补 $M$：$MO=4$，$MF=1$。}
      \end{center}
      \eduExplainStep{3}{看 $\triangle ABD$ 中的中位线}{在 $\triangle ABD$ 中，由中位线定理得
$$
MO=\frac12AD=4.
$$
}
      \eduExplainStep{4}{看 $\triangle ABE$ 中的中位线}{在 $\triangle ABE$ 中，由中位线定理得
$$
MF=\frac12BE=1.
$$
}
...
$$
    ...
    \begin{eduBulletList}
      \item 本题的关键不是硬算 $O,F$ 的坐标，而是补一个中点，把两个已有中点接进中位线。    \end{eduBulletList}
  \end{eduExplainRight}
\end{eduDualExplain}
