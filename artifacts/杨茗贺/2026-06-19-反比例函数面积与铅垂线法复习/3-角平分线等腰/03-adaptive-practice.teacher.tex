...
\end{center}
  ...
  \begin{solutionblock}
    \textbf{答案：}$CE=\sqrt{41}$\par\medskip
    \solstep{推出等腰}{$BE$ 平分 $\angle ABC$，且 $AD\parallel BC$，所以 $\angle ABE=\angle AEB$，$AE=AB=4$。}
    \solstep{求 $CE$}{$DE=9-4=5$，$CD=4$，所以 $CE=\sqrt{4^2+5^2}=\sqrt{41}$。}
  \end{solutionblock}
\end{problem}
...
\end{center}
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$AE=\dfrac{25}{4}$\par\medskip
    \solstep{推出等腰}{翻折得 $\angle BCA=\angle ACE$，又 $AD\parallel BC$ 得 $\angle EAC=\angle BCA$，所以 $\angle EAC=\angle ACE$，$AE=CE$。}
    \solstep{列方程}{设 $AE=x$，则 $DE=8-x$，$CD=6$。由 $CE=AE$，得 $x^2=(8-x)^2+6^2$，解得 $x=\dfrac{25}{4}$。}
  \end{solutionblock}
\end{problem}
...
\end{center}
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$CG=2$\par\medskip
    \solstep{推出等腰}{$AB\parallel GE$，且 $AG$ 平分 $\angle BAE$，所以 $\angle GAE=\angle AGE$，$AE=GE=6$。}
    \solstep{求 $CG$}{$GE=CE+CG$，所以 $CG=6-4=2$。}
  \end{solutionblock}
\end{problem}
...
\end{center}
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$D(2,2)$，$F(\sqrt5-1,2)$\par\medskip
    \solstep{求 $D$}{$E$ 是 $CD$ 中点且 $E(2,1)$，正方形边长为 $2$，所以 $D(2,2)$。}
    \solstep{延长求等腰}{延长 $BF$ 交 $CD$ 延长线于 $G$，由 $AB\parallel GE$ 和角平分线得 $\triangle BGE$ 为等腰，$GE=BE=\sqrt5$。}
    \solstep{求 $F$}{$G(2,1+\sqrt5)$，直线 $BG:y=\frac{1+\sqrt5}{2}x$。令 $y=2$ 得 $x=\sqrt5-1$，所以 $F(\sqrt5-1,2)$。}
  \end{solutionblock}
\end{problem}
