...
\eduSectionTitle{模型：中点四边形反看原对角线}


\begin{eduSolutionBox}{核心结论：新边对应原对角线}
  \begingroup
  \setlength{\parskip}{2.5mm}
$EF\parallel AC$，$EF=\dfrac12AC$；$FG\parallel BD$，$FG=\dfrac12BD$。\par
$EFGH$ 一定是平行四边形。\par
$EFGH$ 是菱形 $\Rightarrow AC=BD$；若原图 $ABCD$ 是平行四边形，则原图是矩形。\par
$EFGH$ 是矩形 $\Rightarrow AC\perp BD$；若原图 $ABCD$ 是平行四边形，则原图是菱形。\par
$EFGH$ 是正方形 $\Rightarrow AC=BD$ 且 $AC\perp BD$；若原图是平行四边形，则原图是正方形。\par
  \endgroup
\end{eduSolutionBox}
...
由中点四边形反推原图形$\;\to\;$用菱形性质求边长$\;\to\;$求周长和面积\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
由中点四边形是矩形，反推原图形并继续计算。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{不要停在判形状}{真正的考点常常是：先反推出原图形是菱形/矩形，再用这个特殊图形的性质继续算。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{由中点四边形反推原图形}{$EFGH$ 是矩形，所以 $EF\perp FG$。又 $EF\parallel AC$，$FG\parallel BD$，因此 $AC\perp BD$。平行四边形对角线互相垂直，所以 $ABCD$ 是菱形。}
      \eduExplainStep{2}{用菱形性质求边长}{在菱形 $ABCD$ 中，$AB=BC=s$。$\angle ABC=120^\circ$，所以 $AC^2=s^2+s^2-2s^2\cos120^\circ=3s^2$。由 $AC=6\sqrt3$，得 $s=6$。}
      \eduExplainStep{3}{求周长和面积}{周长为 $4s=24$；面积为 $s^2\sin120^\circ=36\cdot\dfrac{\sqrt3}{2}=18\sqrt3$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduAuxItem}{思路导航}
先写能推出什么$\;\to\;$检查是否能判原图形$\;\to\;$结论\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
判断命题是否正确。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideMistake{任意四边形不能乱反推}{中点四边形告诉你的是原对角线关系；要把原图形判成矩形/菱形，还需要原图形本来是平行四边形。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先写能推出什么}{$EFGH$ 是矩形，只能推出 $EF\perp FG$，即原四边形两条对角线 $AC\perp BD$。}
      \eduExplainStep{2}{检查是否能判原图形}{若 $ABCD$ 只是任意四边形，对角线垂直不能推出它是菱形；只有先知道 $ABCD$ 是平行四边形，才能由对角线垂直推出菱形。}
      \eduExplainStep{3}{结论}{原命题错误。缺少“$ABCD$ 是平行四边形”的条件。}
  \end{eduExplainRight}
\end{eduDualExplain}
