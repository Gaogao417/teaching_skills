...
\noindent
\begin{minipage}[t]{\dimexpr\linewidth-0.38\linewidth-6mm\relax}
\vspace{0pt}
...
\end{minipage}\hfill
\diagramcolfigure{build/diagram/jobs/explanation-orig-prompt/rendered/prompt.png}{0.38\linewidth}{用 $-3,0,2$ 分段后，逐段比较直线和双曲线谁在上方。}
\end{eduProblemBlock}
...
命名两个函数$\;\to\;$把不等式读成 $y_1>y_2$$\;\to\;$图像上找 $y_1$ 在上方$\;\to\;$用交点和 $0$ 分段$\;\to\;$逐段比较上下$\;\to\;$写满足区间并检查端点\end{eduAuxItem}
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{提示与易错}
        \eduSideHint{入口}{先问：不等式左边是哪条图像？右边是哪条图像？}
        \eduSideHint{为什么要分段}{交点处两函数相等；经过交点后，上下关系可能改变。}
        \eduSideMistake{不要漏掉 $0$}{$y_2=\dfrac{6}{x}$ 在 $x=0$ 没有意义，所以不能把 $(-3,0)$ 和 $(0,2)$ 合并。}
        \eduSideMistake{端点是否能取}{$>$ 不包含交点；若题目改成 $\ge$，才考虑把交点横坐标取进来。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{命名两个函数}{设 $y_1=x+1$，$y_2=\dfrac{6}{x}$。}
      \eduExplainStep{2}{把不等式读成 $y_1>y_2$}{$x+1>\dfrac{6}{x}$ 等价于同一个 $x$ 下 $y_1$ 的值大于 $y_2$ 的值。}
      \eduExplainStep{3}{图像上找 $y_1$ 在上方}{$y_1>y_2$ 表示直线 $y_1$ 在双曲线 $y_2$ 的上方。}
      \eduExplainStep{4}{用交点和 $0$ 分段}{交点横坐标为 $-3,2$，再加入反比例函数断点 $0$，断点顺序是 $-3,0,2$。}
      \eduExplainStep{5}{逐段比较上下}{四个区间为 $(-\infty,-3)$、$(-3,0)$、$(0,2)$、$(2,+\infty)$；逐段看图像谁在上方。}
      \eduExplainStep{6}{写满足区间并检查端点}{直线在上方的区间是 $(-3,0)$ 和 $(2,+\infty)$；因为是严格大于，交点不取，且 $x=0$ 不取。}
    \vspace{1mm}
    {\color{edu-blue}\bfseries\small 图像校对}\par\vspace{1mm}
    \begin{eduBulletList}
      \item $(-\infty,-3)$：双曲线在上，不取。      \item $(-3,0)$：直线在上，取。      \item $(0,2)$：双曲线在上，不取。      \item $(2,+\infty)$：直线在上，取。      \item 所以不等式 $x+1>\dfrac{6}{x}$ 的解集为 $-3<x<0$ 或 $x>2$。    \end{eduBulletList}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduVariationTraining}{校对 1}
请把断点 $-3,0,2$ 从小到大写在数轴上，并列出四个分段。
\answerarea[18mm]
...
\begin{eduVariationTraining}{校对 2}
如果题目改为 $x+1<\dfrac{6}{x}$，应选择哪几段？
\answerarea[20mm]
\end{eduVariationTraining}
...
\begin{eduBottomGrid}
  \begin{eduSummaryLeft}
    ...
    \begin{eduPlainList}
      \item $x+1>\dfrac{6}{x}$ 的解集是 $-3<x<0$ 或 $x>2$。    \end{eduPlainList}
    \end{eduSummaryBlock}
  \end{eduSummaryLeft}
  \eduSummaryDivider
  \begin{eduSummaryRight}
    ...
    \begin{eduBulletList}
      \item 先翻译成 $y_1>y_2$。      \item 交点横坐标和 $0$ 都要分段。      \item 严格大于不取交点，$x=0$ 永远不取。    \end{eduBulletList}
    \end{eduSummaryBlock}
  \end{eduSummaryRight}
\end{eduBottomGrid}
