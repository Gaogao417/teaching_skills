...
\eduSectionTitle{模型：平行线对应边比例先定目标比}


\begin{eduSolutionBox}{核心结论：先找目标比，再找对应边}
  \begingroup
  \setlength{\parskip}{2.5mm}
在 $\triangle ACE$ 中，若 $B$ 在线段 $AC$ 上，$D$ 在线段 $AE$ 上，且 $BD\parallel CE$，则 $\triangle ABD\sim\triangle ACE$。\par
对应边比例：$\dfrac{AB}{AC}=\dfrac{AD}{AE}=\dfrac{BD}{CE}$。\par
差段也可以配对：$\dfrac{BC}{AB}=\dfrac{DE}{AD}$，$\dfrac{BC}{AC}=\dfrac{DE}{AE}$。\par
固定动作：先定目标比，再把已知改成目标比，最后乘已知量。\par
  \endgroup
\end{eduSolutionBox}
...
\vspace{0pt}
如图，在 $\triangle ACE$ 中，点 $B$ 在线段 $AC$ 上，点 $D$ 在线段 $AE$ 上，且 $BD\parallel CE$。
已知 $AD=3$，$BC=5$，$AC=7$，求 $DE$。

\end{minipage}\hfill
\begin{diagramcoltikz}{58mm}{例题图：观察 $BD\parallel CE$ 与对应边。}
\input{\detokenize{build/diagram/jobs/explanation-orig-prompt/rendered/prompt.fragment.tex}}
\end{diagramcoltikz}
\end{eduProblemBlock}
...
先定目标比$\;\to\;$把已知改成目标比$\;\to\;$乘已知量\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题}
求线段 $DE$ 的长。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{第一步想什么}{不会做时，先想“目标量 : 已知量”。本题目标量是 $DE$，已知同一直线上的 $AD$，所以先想 $DE:AD$。}
        \eduSideHint{怎么找对应边}{遇到三角形中一条线段平行一边的问题，对应边一般就是共线、共三角形的边：$DE$ 对应 $BC$，$AD$ 对应 $AB$。}
        \eduSideMistake{先改已知}{已知里没有 $AB$，不能停住；先用 $AB=AC-BC$ 把已知改成目标比需要的边。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先定目标比}{要求 $DE$，已知同一直线上的 $AD=3$，先想 $DE:AD$。}
      \eduExplainStep{2}{把已知改成目标比}{由 $BD\parallel CE$ 得对应差段成比例，$DE:AD=BC:AB$。又 $AB=AC-BC=7-5=2$，所以 $DE:AD=5:2$。}
      \eduExplainStep{3}{乘已知量}{$DE=AD\cdot\dfrac{5}{2}=3\cdot\dfrac{5}{2}=\dfrac{15}{2}$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduBulletList}
  \item 求哪条边，就先给它找一条已知边组成目标比。  \item 对应边从两个地方找：一是共线的边，二是同一个小三角形/大三角形里的边。  \item 若已知量不能直接进目标比，先用整段减小段，把它改成需要的边。\end{eduBulletList}
\end{eduSummaryBlock}
