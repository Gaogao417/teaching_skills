...
一条线段被分成较长一段和较短一段。如果按黄金分割来安排，常用的近似关系是：
\[
  \frac{\text{长段}}{\text{全长}}\approx0.618,\qquad
  \frac{\text{短段}}{\text{长段}}\approx0.618.
\]
做题时不要一看到 $0.618$ 就直接乘，先判断题目给的是哪一段、求的是哪一段。
...
已知全长，求长段：长段 $=\text{全长}\times0.618$。\par
已知长段，求短段：短段 $=\text{长段}\times0.618$。\par
已知短段，求长段：长段 $=\text{短段}\div0.618$。\par
...
例：一条装饰线全长 $30\text{ cm}$，按黄金分割分成两段。求较长一段的长度。（结果保留一位小数）
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{读题提示}
        ...
        \eduSideMistake{易错点}{不要把 $0.618$ 固定理解成“答案”。它表示的是某两类边之间的比，要先看清楚是哪两类边。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{示例解法}
      \eduExplainStep{1}{识别已知边和要求的边}{已知边：全长 $30\text{ cm}$；要求边：较长一段。}
      \eduExplainStep{2}{判断这两条边分别是长段、短段还是全长}{已知是全长，所求是长段，对应关系是 $\text{长段}:\text{全长}\approx0.618$。}
      \eduExplainStep{3}{选择关系并列式计算}{长段 $=30\times0.618=18.54\text{ cm}$，保留一位小数为 $18.5\text{ cm}$。}
  \end{eduExplainRight}
\end{eduDualExplain}
