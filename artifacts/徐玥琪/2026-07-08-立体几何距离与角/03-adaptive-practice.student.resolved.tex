...
\textbf{提示：}线面角 = 斜线与其在面内射影的夹角。先有垂线才有射影。\end{hintbox}
    ...
    \end{choices}
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{斜线 $l$、斜足 $A$、射影 $AP'$。}
  \input{\detokenize{build/diagram/jobs/practice-c1-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
\end{question}
...
\textbf{提示：}点面距、垂足位置不显然时，用等体积法：选一个三棱锥列 $V$ 相等。可试 $V_{A_1\text{-}BDD_1}=V_{D\text{-}A_1BD_1}$。\end{hintbox}
  ...
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{正方体，标出 $A_1$ 与对角面 $BDD_1B_1$。}
  \input{\detokenize{build/diagram/jobs/practice-f2-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
\end{question}
...
\textbf{提示：}侧棱与底面的线面角 = 斜线 $PA$ 与射影 $AO$ 的夹角。先求高 $PO$ 和 $AO$。\end{hintbox}
  ...
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{正四棱锥，标出侧棱 $PA$、底面中心 $O$ 与高 $PO$。}
  \input{\detokenize{build/diagram/jobs/practice-f4-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
\end{question}
...
\textbf{提示：}（1）用三垂线定理：$BD\perp AC$（正方形对角线互相垂直），$D_1D\perp$ 底面，得 $BD_1\perp AC$。（2）截面 $AB_1C$ 与底面的棱为 $AC$，取 $AC$ 中点 $O$，$OB\perp AC$，由三垂线 $OB_1\perp AC$，$\angle B_1OB$ 是平面角。\end{hintbox}
  ...
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{正方体，标出截面 $AB_1C$ 与对角线 $BD_1$。}
  \input{\detokenize{build/diagram/jobs/practice-p3-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
  \par\medskip
  \answerarea[62mm]
\end{problem}
...
\textbf{提示：}点 $A$ 到面 $PBC$ 的距离用等体积法：$V_{P\text{-}ABC}=V_{A\text{-}PBC}$。先算 $S_{\triangle ABC}$ 和 $S_{\triangle PBC}$。注意 $PA\perp$ 底面，可先算 $PB$、$PC$。\end{hintbox}
  ...
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{三棱锥，标出 $PA\perp$ 底面及点 $A$ 到面 $PBC$ 的距离。}
  \input{\detokenize{build/diagram/jobs/practice-p5-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
  \par\medskip
  \answerarea[70mm]
\end{problem}
...
\textbf{提示：}（1）$AD\perp BC$（正三角形中线即高），$A_1A\perp$ 底面，由三垂线定理得 $A_1D\perp BC$。（2）$A_1B$ 与底面所成角 = $A_1B$ 与射影 $AB$ 的夹角，在 $\text{Rt}\triangle A_1AB$ 中算。\end{hintbox}
  ...
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{正三棱柱，标出 $A_1D$、$BC$ 及 $A_1B$ 与底面的线面角。}
  \input{\detokenize{build/diagram/jobs/practice-p6-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
  \par\medskip
  \answerarea[62mm]
\end{problem}
...
\textbf{提示：}连 $BD$ 交 $AC$ 于 $O$。$D_1D\perp$ 底面，$BD\perp AC$，由三垂线定理 $D_1O\perp AC$，$D_1O$ 即为所求。\end{hintbox}
  ...
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{正方体，标出 $D_1$ 到 $AC$ 的垂线段 $D_1O$。}
  \input{\detokenize{build/diagram/jobs/practice-p8-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
  \par\medskip
  \answerarea[55mm]
\end{problem}
...
\textbf{提示：}（1）$PO\perp$ 底面，在 $\text{Rt}\triangle PAM$（$M$ 为 $BC$ 中点）里算 $PO$。（2）$E$、$F$ 为 $PA$、$BC$ 中点，$EF$ 为异面直线 $PA$ 与 $BC$ 的公垂线段，在 $\triangle PAM$ 里求 $EF$。\end{hintbox}
  ...
  \end{minipage}\hfill
  \begin{diagramcoltikz}{70mm}{正四面体，标出高 $PO$ 与中点 $E$、$F$。}
  \input{\detokenize{build/diagram/jobs/practice-p9-prompt/rendered/prompt.fragment.tex}}
  \end{diagramcoltikz}
  \par\medskip
  \answerarea[70mm]
\end{problem}
