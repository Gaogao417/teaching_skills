...
\eduSectionTitle{线段和角：先在图上标份数}


\begin{eduSolutionBox}{图形题的三格填空}
  \begingroup
  \setlength{\parskip}{2.5mm}
第一格：把比例或方程式翻成图上的带圈份数。\par
第二格：写“目标量 : 整体 = 目标份数 : 总份数”。\par
第三格：填出题目问的线段或角；不问的量不用作为最终答案。\par
若条件是 $ma=nb$，则 $a:b=n:m$，份数和系数交叉对应。\par
  \endgroup
\end{eduSolutionBox}
...
\vspace{0pt}
点 $A,B,C$ 在同一直线上，$B$ 在 $A,C$ 之间。若 $AB:BC=3:5$，且 $AC=32$，求 $AB$。
\end{minipage}\hfill
\begin{diagramcoltikz}{60mm}{先把 AB 标 ③，BC 标 ⑤。}
...
标份数$\;\to\;$写目标比例$\;\to\;$填答案\end{eduAuxItem}
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{看图填空}
        \eduSideHint{先标图}{图上的 \textcircled{\scriptsize 3} 和 \textcircled{\scriptsize 5} 比文字说明更直观。}
        \eduSideHint{只看目标}{题目问 $AB$，就写 $AB$ 对整体 $AC$ 的份数。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{填法}
      \eduExplainStep{1}{标份数}{在图上标 $AB$ 为 \textcircled{\scriptsize 3}，$BC$ 为 \textcircled{\scriptsize 5}，所以 $AC$ 共 $8$ 份。}
      \eduExplainStep{2}{写目标比例}{$\dfrac{AB}{32}=\dfrac{3}{8}$。}
      \eduExplainStep{3}{填答案}{$AB=32\times\dfrac{3}{8}=12$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{minipage}[t]{\dimexpr\linewidth-60mm-6mm\relax}
\vspace{0pt}
点 $A,B,C$ 在同一直线上，$B$ 在 $A,C$ 之间。若 $5AB=3BC$，且 $AC=56$，求 $AB$。
\end{minipage}\hfill
\begin{diagramcoltikz}{60mm}{由 5AB=3BC 得 AB:BC=3:5。}
\begin{tikzpicture}[x=1cm,y=1cm,baseline=(current bounding box.center)]
  \draw[line width=1.05pt] (0,0) -- (5.0,0);
  \fill (0,0) circle (1.7pt) node[below] {$A$};
  \fill (1.9,0) circle (1.7pt) node[below] {$B$};
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{看图填空}
        \eduSideMistake{交叉对应}{$5AB=3BC$ 时，$AB$ 是 $3$ 份，不是 $5$ 份。}
        \eduSideHint{再写目标比例}{标完份数后，只写目标量 $AB$ 和整体 $AC$ 的比例。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{填法}
      \eduExplainStep{1}{标份数}{$5AB=3BC$，所以 $AB:BC=3:5$，在图上标 $AB$ 为 ③，$BC$ 为 ⑤。}
      \eduExplainStep{2}{写目标比例}{$\dfrac{AB}{56}=\dfrac{3}{8}$。}
      \eduExplainStep{3}{填答案}{$AB=56\times\dfrac{3}{8}=21$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}方程式关系要先转成份数。$ma=nb$ 时，$a:b=n:m$；份数和系数是交叉对应。\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 先在图上标带圈份数，再写目标量对整体的比例。  \item 线段整体：$AC=AB+BC$；角度题同构：$\angle AOC=\angle AOB+\angle BOC$。  \item 目标量 : 整体 = 目标份数 : 总份数。\end{eduBulletList}
\end{eduSummaryBlock}
