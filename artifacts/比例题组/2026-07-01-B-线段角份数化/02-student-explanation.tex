...
\eduSectionTitle{线段和角：先翻译，再调用比例工具}


\begin{eduSolutionBox}{从 A 组接过来的动作}
  \begingroup
  \setlength{\parskip}{2.5mm}
B 组不是新计算；新动作是把几何条件翻译成比例式或份数。\par
线段入口：$B$ 在 $A,C$ 之间，则 $AC=AB+BC$。\par
角度入口：射线 $OB$ 在 $\angle AOC$ 内部，则 $\angle AOC=\angle AOB+\angle BOC$。\par
方程式入口：$4a=3b$ 推出 $a:b=3:4$，比例份数与系数交叉对应。\par
  \endgroup
\end{eduSolutionBox}
...
\input{\detokenize{build/diagram/jobs/b-ex1-prompt/rendered/prompt.fragment.tex}}
\end{diagramcoltikz}
\end{eduProblemBlock}
...
找整体$\;\to\;$翻译份数$\;\to\;$只求目标量\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
求 $BC$。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{策略判断}
        \eduSideHint{承接 A 组}{几何题先翻译成“几份”，计算部分仍是比例工具。}
        \eduSideMistake{目标量}{题目只求 $BC$，不用把 $AB$ 也作为最终答案写出。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{找整体}{$AC=AB+BC$。}
      \eduExplainStep{2}{翻译份数}{$AB=2BC$，所以 $AB:BC=2:1$，总共 $3$ 份。}
      \eduExplainStep{3}{只求目标量}{$18\div3=6$，目标 $BC$ 是 $1$ 份，所以 $BC=6$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\end{minipage}\hfill
\begin{diagramcoltikz}{60mm}{观察整体 AC 由 AB 和 BC 组成。}
\input{\detokenize{build/diagram/jobs/b-ex2-prompt/rendered/prompt.fragment.tex}}
\end{diagramcoltikz}
\end{eduProblemBlock}
...
\begin{eduAuxItem}{思路导航}
方程式转比例$\;\to\;$整体对应总份数$\;\to\;$求目标量\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
求 $AB$。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{策略判断}
        \eduSideMistake{比例顺序}{$4AB=3BC$ 时，$AB$ 不是 $4$ 份，而是 $3$ 份。}
        \eduSideHint{底层工具}{转出 $AB:BC=3:4$ 后，剩下就是 A 组的份数计算。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{方程式转比例}{$4AB=3BC$，所以 $AB:BC=3:4$。}
      \eduExplainStep{2}{整体对应总份数}{$AC=AB+BC$，总共 $7$ 份，$21\div7=3$。}
      \eduExplainStep{3}{求目标量}{$AB$ 是 $3$ 份，所以 $AB=9$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\input{\detokenize{build/diagram/jobs/b-ex3-prompt/rendered/prompt.fragment.tex}}
\end{diagramcoltikz}
\end{eduProblemBlock}
...
\begin{eduAuxItem}{思路导航}
角度也先找整体$\;\to\;$方程式转比例$\;\to\;$求目标角\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 3}
求 $\angle AOB$。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{策略判断}
        \eduSideHint{同构}{线段换成角，底层还是“整体由两个部分组成”。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{角度也先找整体}{$\angle AOC=\angle AOB+\angle BOC$。}
      \eduExplainStep{2}{方程式转比例}{$5\angle AOB=2\angle BOC$，所以 $\angle AOB:\angle BOC=2:5$。}
      \eduExplainStep{3}{求目标角}{总共 $7$ 份，$112^\circ\div7=16^\circ$，故 $\angle AOB=32^\circ$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}做 B 组题时不要急着设未知数。先写整体式，再把条件翻译成比例；如果条件是 $ma=nb$，份数是 $a:b=n:m$。\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 读目标：本题只求哪一个量？  \item 找整体：线段整体或角整体。  \item 翻译条件：倍数、比例、方程式都转成份数。  \item 调用 A 组：总份数、一份、目标份数。\end{eduBulletList}
\end{eduSummaryBlock}
