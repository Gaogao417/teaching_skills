...
第二格：用“目标量 : 整体 = 目标份数 : 总份数”写比例方程。\par
第三格：解出题目问的线段或角；不问的量不用作为最终答案。\par
...
图上标份数$\;\to\;$写比例方程$\;\to\;$求目标量\end{eduAuxItem}
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{看图填空}
        \eduSideHint{先标图}{图上的 \textcircled{\scriptsize 3} 和 \textcircled{\scriptsize 5} 比直接写文字更稳。}
        \eduSideHint{比例方程}{目标量对整体，等于目标份数对总份数。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{填法}
      \eduExplainStep{1}{图上标份数}{在图上标 $AB$ 为 \textcircled{\scriptsize 3}，$BC$ 为 \textcircled{\scriptsize 5}，所以 $AC$ 共 $8$ 份。}
      \eduExplainStep{2}{写比例方程}{$\dfrac{AB}{32}=\dfrac{3}{8}$。}
      \eduExplainStep{3}{求目标量}{$AB=32\times\dfrac{3}{8}=12$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduAuxItem}{思路导航}
方程式转份数$\;\to\;$写比例方程$\;\to\;$求目标量\end{eduAuxItem}
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{看图填空}
        ...
        \eduSideHint{再写方程}{标完份数后，必须写 $AB:AC=3:8$ 或 $\dfrac{AB}{56}=\dfrac{3}{8}$。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{填法}
      \eduExplainStep{1}{方程式转份数}{$5AB=3BC$，所以 $AB:BC=3:5$。}
      \eduExplainStep{2}{写比例方程}{$\dfrac{AB}{56}=\dfrac{3}{8}$。}
      \eduExplainStep{3}{求目标量}{$AB=56\times\dfrac{3}{8}=21$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduBulletList}
  \item 先在图上标带圈份数，再写比例方程。  \item 线段整体：$AC=AB+BC$；角整体：$\angle AOC=\angle AOB+\angle BOC$。  \item 目标量 : 整体 = 目标份数 : 总份数。\end{eduBulletList}
\end{eduSummaryBlock}
