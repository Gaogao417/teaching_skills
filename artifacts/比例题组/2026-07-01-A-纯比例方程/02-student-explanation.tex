...
\eduSectionTitle{比例方程：看信号，填关键式}


\begin{eduSolutionBox}{看信号：先判断能不能直接填}
  \begingroup
  \setlength{\parskip}{2.5mm}
已经给出比例式时，先看同位置有没有明显倍数或份数。\par
第四比例项题，前三个数顺序不乱，$x$ 是最后要补的数。\par
比例中项题，$x$ 是夹在两个数中间的正数，可以先想“中间数平方等于两端乘积”。\par
看不出倍数或份数时，再写交叉相乘关键式；含一次式时括号必须整体保留。\par
  \endgroup
\end{eduSolutionBox}
...
\begin{enumerate}[label=(\arabic*)]
  \item $4:12=5:x$，$x=\underline{\hspace{1.5cm}}$。
  \item 求 $5,20,3$ 的第四比例项 $x$，$x=\underline{\hspace{1.5cm}}$。
  \item 正数 $x$ 是 $9$ 和 $16$ 的比例中项，$x=\underline{\hspace{1.5cm}}$。
  \item $\dfrac{x+2}{5}=\dfrac{3}{4}$，交叉相乘得 $\underline{\hspace{2.6cm}}$，$x=\underline{\hspace{1.5cm}}$。
\end{enumerate}
...
比例式先看倍数$\;\to\;$第四比例项保顺序$\;\to\;$比例中项看两端$\;\to\;$一次式整体交叉\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题}
完成上面四个填空。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{先找入口}{能看倍数先看倍数；遇到“中项”看两端；含一次式先整体保留。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{填法}
      \eduExplainStep{1}{比例式先看倍数}{$4\to12$ 是 $\times3$，所以 $5\to x$ 也乘 $3$，$x=15$。}
      \eduExplainStep{2}{第四比例项保顺序}{前两个数 $5\to20$ 是 $\times4$，所以 $3\to x$ 也乘 $4$，$x=12$。}
      \eduExplainStep{3}{比例中项看两端}{两端乘积 $9\times16=144$，所以 $x^2=144$；$x$ 为正数，$x=12$。}
      \eduExplainStep{4}{一次式整体交叉}{交叉相乘得 $4(x+2)=15$，解得 $x=\dfrac{7}{4}$。}
  \end{eduExplainRight}
\end{eduDualExplain}
