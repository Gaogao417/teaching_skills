...
\begin{eduSolutionBox}{核心定位：这是填空反应训练}
  \begingroup
  \setlength{\parskip}{2.5mm}
比例方程先训练“看一眼填什么”，不要把观察过程写成完整推导。\par
每题只保留三格：题面信号、关键式、答案。\par
有明显倍数或份数，直接填目标量；没有明显入口，再填交叉相乘关键式。\par
含一次式时，括号必须跟着整体走。\par
  \endgroup
\end{eduSolutionBox}
...
\begin{eduBottomGrid}
  \begin{eduSummaryLeft}
    \begin{eduSummaryBlock}{\eduIconCheck}{题面信号}
    \begin{eduPlainList}
      \item $a,b,c,x$ 成比例 $\Rightarrow a:b=c:x$。      \item 第四比例项：前三个数顺序不换，$x$ 放第四位。      \item 分数式：上下位置照原式读成比例。      \item 正比例中项：正数 $x$ 满足 $a:x=x:b$。    \end{eduPlainList}
    \end{eduSummaryBlock}
  \end{eduSummaryLeft}
  \eduSummaryDivider
  \begin{eduSummaryRight}
    \begin{eduSummaryBlock}{\eduIconPin}{关键式}
    \begin{eduBulletList}
      \item 倍数法：$4:12=5:x$，看 $4\to12$ 是 $\times3$，直接 $x=15$。      \item 份数法：$x:24=5:8$，$24$ 对应 $8$ 份，$x$ 对应 $5$ 份。      \item 交叉相乘：$7:x=3:5$，只填 $3x=35$。      \item 含一次式：$\dfrac{x+2}{5}=\dfrac{3}{4}$，只填 $4(x+2)=15$。    \end{eduBulletList}
    \end{eduSummaryBlock}
  \end{eduSummaryRight}
\end{eduBottomGrid}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}比例方程最容易错在顺序和整体。第四比例项不要随便换位置；$x+2$、$2x-1$ 这类一次式要整块参与交叉相乘。\end{eduMistakeBox}




\begin{eduSummaryBlock}{\eduIconPin}{填法提醒}
\begin{eduBulletList}
  \item 先看有没有明显倍数或份数，能直接填就直接填。  \item 看不出倍数或份数时，再写交叉相乘关键式。  \item 填空题只要关键式和答案稳，不把观察过程展开成完整推导。\end{eduBulletList}
\end{eduSummaryBlock}
