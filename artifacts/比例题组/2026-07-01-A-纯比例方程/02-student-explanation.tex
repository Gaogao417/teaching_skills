...
\eduSectionTitle{比例方程：把计算压缩成填空动作}


\begin{eduSolutionBox}{填空题只保留三件事}
  \begingroup
  \setlength{\parskip}{2.5mm}
比例方程题不适合把“观察、判断、换法”写成长步骤；卷面上只填关键式、关键数和答案。\par
倍数/份数明显时：填“几份、总几份、目标几份”。\par
倍数/份数不明显时：填交叉相乘后的方程。\par
第四比例项先写成 $a:b=c:x$；比例中项先写成 $a:x=x:b$。\par
  \endgroup
\end{eduSolutionBox}
...
\begin{eduProblemBlock}[例题填空]
完成填空。
\begin{enumerate}[label=(\arabic*)]
  \item $4,12,5,x$ 成比例，则 $4:12=5:x$，$x=\underline{\hspace{1.5cm}}$；
  \item $7:x=3:5$，交叉相乘得 $\underline{\hspace{2.2cm}}$，$x=\underline{\hspace{1.5cm}}$；
  \item 正数 $x$ 是 $4$ 和 $9$ 的比例中项，则 $4:x=x:9$，$x=\underline{\hspace{1.5cm}}$。
\end{enumerate}
\end{eduProblemBlock}
...
第四比例项填空$\;\to\;$交叉相乘填空$\;\to\;$比例中项填空\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题填空}
完成三个比例方程填空。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{卷面动作}
        \eduSideHint{不要写长过程}{这类题训练的是反应速度，卷面只需要关键式和结果。}
        \eduSideMistake{先定比例顺序}{第四比例项与比例中项的比例式不能随便换位置。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{填法}
      \eduExplainStep{1}{第四比例项填空}{$4:12=5:x$，$4\to12$ 是 $\times3$，所以 $x=15$。}
      \eduExplainStep{2}{交叉相乘填空}{$7:x=3:5$，交叉相乘得 $3x=35$，所以 $x=\dfrac{35}{3}$。}
      \eduExplainStep{3}{比例中项填空}{$4:x=x:9$，得 $x^2=36$；因为 $x$ 为正数，所以 $x=6$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduBulletList}
  \item 看见 $a,b,c,x$ 成比例：先填 $a:b=c:x$。  \item 看见“比例中项”：先填 $a:x=x:b$。  \item 没有明显倍数时，直接填交叉相乘方程。\end{eduBulletList}
\end{eduSummaryBlock}
