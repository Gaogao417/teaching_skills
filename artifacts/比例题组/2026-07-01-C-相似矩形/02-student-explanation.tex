...
\eduSectionTitle{相似矩形：对应关系与多解}


\begin{eduSolutionBox}{核心结论：先确定对应，再列比例}
  \begingroup
  \setlength{\parskip}{2.5mm}
如果题目明确“长、宽”，通常长对长、宽对宽。\par
如果只说“两边长为 $2$ 和 $3$”，没有指定哪条边对应，就要分类。\par
判断相似先比较长宽比；求未知边先写对应边比例。\par
  \endgroup
\end{eduSolutionBox}
...
\vspace{0pt}
矩形甲的长为 $4$、宽为 $6$，矩形乙的长为 $8$、宽为 $12$。判断两个矩形是否相似。
\end{minipage}\hfill
\begin{diagramcoltikz}{60mm}{两个矩形的长宽对应关系。}
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{判断入口}{判断相似时先比较两个矩形的长宽比，不先列未知数。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{写长宽比}{甲的长宽比为 $4:6=2:3$，乙的长宽比为 $8:12=2:3$。}
      \eduExplainStep{2}{判断相似}{两个长宽比相同，所以两个矩形相似。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\end{minipage}\hfill
\begin{diagramcoltikz}{60mm}{没有指定 6 对应哪条边，要分类。}
...
\begin{eduAuxItem}{思路导航}
情况一：$6$ 对应 $2$$\;\to\;$情况二：$6$ 对应 $3$$\;\to\;$写完整答案\end{eduAuxItem}
...
\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideMistake{多解入口}{题目没有说明 $6$ 是长边还是短边，所以不能只列一种比例。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{情况一：$6$ 对应 $2$}{$2:3=6:x$，解得 $x=9$。}
      \eduExplainStep{2}{情况二：$6$ 对应 $3$}{$2:3=x:6$，解得 $x=4$。}
      \eduExplainStep{3}{写完整答案}{所以 $x=9$ 或 $x=4$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}相似矩形题最容易错在“对应关系没判定就计算”。如果题目没有明确长宽或对应边，先分类。\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 判断题：先写两个长宽比。  \item 求边题：先写对应边关系。  \item 未指定对应：分两种情况。\end{eduBulletList}
\end{eduSummaryBlock}
