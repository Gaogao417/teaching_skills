...
\eduSectionTitle{面积比链：先拆四边形，再串中间面积}


\begin{eduSolutionBox}{核心结论：面积比必须有底高依据}
  \begingroup
  \setlength{\parskip}{2.5mm}
同高三角形面积比等于底边比；同底三角形面积比等于高的比。\par
四边形 $OECD$ 要先拆：$S_{OECD}=S_{\triangle OEC}+S_{\triangle OCD}$。\par
本类题使用 $AO:OD$、$BO:OE$、$AE:EC$ 这样的同线分点比；不能只给 $AD:BE$。\par
  \endgroup
\end{eduSolutionBox}
...
\end{minipage}\hfill
\begin{diagramcoltikz}{60mm}{已知点 D、E、O 的位置，目标是四边形 OECD。}
\input{\detokenize{build/diagram/jobs/d-ex-prompt/rendered/prompt.fragment.tex}}
\end{diagramcoltikz}
\end{eduProblemBlock}
...
\begin{eduSolutionBox}{面积任务卡：先把要求量放进份数系统}
  \begingroup
  \setlength{\parskip}{2.5mm}
\begin{tabular}{p{0.28\linewidth}p{0.62\linewidth}}
目标面积 & $S_{OECD}$，它是四边形面积。\\
参照面积 & $S_{\triangle OAB}$，最后要和它作比。\\
是否要拆 & 要拆，$S_{OECD}=S_{\triangle OEC}+S_{\triangle OCD}$。\\
中间面积 & 设 $S_{\triangle OAE}=1$ 份。\\
比例来源 & $BO:OE$ 控制 $OAB$ 与 $OAE$；$AE:EC$ 控制 $OAE$ 与 $OEC$；$AO:OD$ 控制 $OAC$ 与 $OCD$。\\
份数表 & $OAE=1$，$OAB=3$，$OEC=2$，$OAC=3$，$OCD=3$，$OECD=5$。\\
\end{tabular}
\par
  \endgroup
\end{eduSolutionBox}
...
拆四边形$\;\to\;$设中间面积$\;\to\;$由 $BO:OE$ 求 $OAB$$\;\to\;$由 $AE:EC$ 求 $OEC$$\;\to\;$由 $AO:OD$ 求 $OCD$$\;\to\;$合并\end{eduAuxItem}




\begin{eduSubQuestionItem}{综合例题}
求 $S_{OECD}:S_{\triangle OAB}$。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideMistake{目标对象}{$OECD$ 是四边形，不是 $\triangle OEC$。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{拆四边形}{$S_{OECD}=S_{\triangle OEC}+S_{\triangle OCD}$。}
      \eduExplainStep{2}{设中间面积}{设 $S_{\triangle OAE}=1$ 份。}
      \eduExplainStep{3}{由 $BO:OE$ 求 $OAB$}{$\triangle OAB$ 与 $\triangle OAE$ 共底 $AO$，得 $S_{\triangle OAB}=3$ 份。}
      \eduExplainStep{4}{由 $AE:EC$ 求 $OEC$}{$\triangle OAE$ 与 $\triangle OEC$ 同高，得 $S_{\triangle OEC}=2$ 份。}
      \eduExplainStep{5}{由 $AO:OD$ 求 $OCD$}{$S_{\triangle OAC}=1+2=3$ 份，且 $S_{\triangle OAC}:S_{\triangle OCD}=AO:OD=1:1$，所以 $S_{\triangle OCD}=3$ 份。}
      \eduExplainStep{6}{合并}{$S_{OECD}=2+3=5$ 份，$S_{\triangle OAB}=3$ 份，所以目标比为 $5:3$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}只算 $\triangle OEC$ 会少掉 $\triangle OCD$。第一步必须先写四边形拆分式。\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 先拆目标面积。  \item 再找中间面积。  \item 每一步都检查同底或同高依据。\end{eduBulletList}
\end{eduSummaryBlock}
