...
\eduSectionTitle{知识点2：顶点式 y=a(x-h)²+k 的顶点、对称轴与开口方向}


\begin{eduSolutionBox}{核心结论：顶点式 y=a(x-h)²+k 能直接读出全部几何信息}
  \begingroup
  \setlength{\parskip}{2.5mm}
顶点式 $y=a(x-h)^2+k$（$a\ne0$）中，顶点是 $(h,k)$，对称轴是直线 $x=h$。\par
\textbf{符号关键：} 括号里写成 $x-h$。遇到 $x+n$ 要先看成 $x-(-n)$，所以 $y=-(x+3)^2+5$ 的 $h=-3$，对称轴是 $x=-3$。\par
\textbf{开口方向：} 由 $a$ 的正负决定，与 $h,k$ 无关。$a>0$ 开口向上，$a<0$ 开口向下。\par
\begin{tabular}{p{0.30\linewidth}p{0.30\linewidth}p{0.28\linewidth}}
解析式片段 & 顶点 & 对称轴\\
$y=2(x-3)^2+1$ & $(3,1)$ & $x=3$\\
$y=-(x+2)^2-4$ & $(-2,-4)$ & $x=-2$\\
$y=-\tfrac12(x+3)^2+5$ & $(-3,5)$ & $x=-3$\\
\end{tabular}
\par
  \endgroup
\end{eduSolutionBox}
...
认括号读顶点 $(h,k)$$\;\to\;$对称轴取 $x=h$$\;\to\;$看 $a$ 定开口\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
求 $y=2(x-3)^2+1$ 的顶点、对称轴和开口方向。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{先认括号}{括号里是 $x-3$，是“$x$ 减一个正数”，直接把这个数当 $h$。}
        \eduSideMistake{对称轴别加负号}{$x-3$ 对应对称轴 $x=3$，不要写成 $x=-3$。}
        \eduSideHint{开口看谁}{开口只看平方项前面那个数 $a$，与 $h,k$ 无关。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{认括号读顶点 $(h,k)$}{括号内是 $x-3$，即 $h=3$；常数项 $k=1$，所以顶点为 $(3,1)$。}
      \eduExplainStep{2}{对称轴取 $x=h$}{对称轴是直线 $x=3$。}
      \eduExplainStep{3}{看 $a$ 定开口}{$a=2>0$，所以抛物线开口向上。}
    \vspace{1mm}
    {\color{edu-blue}\bfseries\small 验算}\par\vspace{1mm}
    \begin{eduBulletList}
      \item 把 $x=3$ 代入：$2(3-3)^2+1=1=k$，顶点 $(3,1)$ 在抛物线上。    \end{eduBulletList}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduAuxItem}{思路导航}
把 $x+2$ 改写成 $x-(-2)$$\;\to\;$读顶点 $(h,k)$$\;\to\;$对称轴取 $x=h$$\;\to\;$看 $a$ 定开口\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
求 $y=-(x+2)^2-4$ 的顶点、对称轴和开口方向。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideMistake{加号要变负号}{$x+2$ 里是“加”，所以 $h=-2$，对称轴是 $x=-2$，不能写成 $x=2$。}
        \eduSideHint{统一口诀}{不管括号里是加还是减，都先写成 $x-h$：减几 $h$ 就是几，加几 $h$ 就是负几。}
        \eduSideMistake{顶点横坐标}{顶点横坐标就是 $h$，不是 $-h$；本题顶点是 $(-2,-4)$。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{把 $x+2$ 改写成 $x-(-2)$}{$x+2=x-(-2)$，所以 $h=-2$。}
      \eduExplainStep{2}{读顶点 $(h,k)$}{$h=-2$，常数项 $k=-4$，所以顶点为 $(-2,-4)$。}
      \eduExplainStep{3}{对称轴取 $x=h$}{对称轴是直线 $x=-2$。}
      \eduExplainStep{4}{看 $a$ 定开口}{$a=-1<0$，所以抛物线开口向下。}
    \vspace{1mm}
    {\color{edu-blue}\bfseries\small 验算}\par\vspace{1mm}
    \begin{eduBulletList}
      \item 把 $x=-2$ 代入：$-(-2+2)^2-4=-4=k$，顶点 $(-2,-4)$ 在抛物线上。    \end{eduBulletList}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduAuxItem}{思路导航}
把 $x+3$ 改写成 $x-(-3)$$\;\to\;$读顶点 $(h,k)$$\;\to\;$对称轴取 $x=h$$\;\to\;$看 $a$ 定开口\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 3}
求 $y=-\dfrac12(x+3)^2+5$ 的顶点、对称轴和开口方向。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideMistake{$x+3$ 的对称轴}{$x+3$ 对应 $h=-3$，对称轴是 $x=-3$，不是 $x=3$。}
        \eduSideHint{分数 $a$ 不影响读取}{$a=-\dfrac12$ 只用来判开口方向，不影响顶点和对称轴的读取。}
        \eduSideHint{自己验一遍}{把对称轴的 $x$ 代回去，应得到 $y=k$，说明顶点读对了。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{把 $x+3$ 改写成 $x-(-3)$}{$x+3=x-(-3)$，所以 $h=-3$。}
      \eduExplainStep{2}{读顶点 $(h,k)$}{$h=-3$，常数项 $k=5$，所以顶点为 $(-3,5)$。}
      \eduExplainStep{3}{对称轴取 $x=h$}{对称轴是直线 $x=-3$。}
      \eduExplainStep{4}{看 $a$ 定开口}{$a=-\dfrac12<0$，所以抛物线开口向下。}
    \vspace{1mm}
    {\color{edu-blue}\bfseries\small 验算}\par\vspace{1mm}
    \begin{eduBulletList}
      \item 把 $x=-3$ 代入：$-\dfrac12(-3+3)^2+5=5=k$，顶点 $(-3,5)$ 在抛物线上。    \end{eduBulletList}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}括号里是“加”还是“减”直接决定 $h$ 的符号：$x-3$ 的 $h=3$，对称轴 $x=3$；$x+3$ 要看成 $x-(-3)$，$h=-3$，对称轴 $x=-3$。最常见的错误就是把 $x+3$ 的对称轴写成 $x=3$，或把顶点横坐标写成 $-h$。另外开口方向只由 $a$ 决定，与 $h,k$ 无关。
\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 先认括号：把 $x\pm n$ 统一写成 $x-h$，减几 $h$ 就是几，加几 $h$ 就是负几。  \item 顶点 $(h,k)$，对称轴 $x=h$，两者共用同一个 $h$。  \item 开口看 $a$：$a>0$ 向上，$a<0$ 向下。  \item 验算：把对称轴的 $x$ 代入，应得 $y=k$。\end{eduBulletList}
\end{eduSummaryBlock}
