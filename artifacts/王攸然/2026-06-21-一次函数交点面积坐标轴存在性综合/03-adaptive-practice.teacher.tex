...
\begin{enumerate}[label=(\arabic*)]
  \item 求点 $A,C$ 的坐标及一次函数 $y_2$ 的解析式；
  \item 求 $\triangle ABC$ 的面积；
  \item 在坐标轴上，是否存在一点 $P$，使得 $S_{\triangle AOP}=2S_{\triangle BCP}$？若存在，直接写出点 $P$ 的坐标；若不存在，说明理由。
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 点 $P$ 在 $x$ 轴上，若 $S_{\triangle AOP}=S_{\triangle PAB}$，求点 $P$ 的坐标。
\end{enumerate}
