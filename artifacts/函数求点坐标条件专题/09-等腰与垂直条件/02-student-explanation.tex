...
\eduSectionTitle{模型九：等腰与垂直条件怎样变成坐标方程}


\begin{eduSolutionBox}{核心结论：等长和垂直都要先方程化}
  \begingroup
  \setlength{\parskip}{2.5mm}
等腰条件要先分类：$PA=PB$，$PA=AB$，$PB=AB$。\par
距离相等通常用平方表示，例如 $PA^2=PB^2$，可以避免根号。\par
垂直条件优先用数量积：$\vec{PA}\cdot\vec{PB}=0$，这样不容易漏掉竖直线情况。\par
点在函数图像上时，先设 $P(t,f(t))$，再把几何条件转成关于 $t$ 的方程。\par
  \endgroup
\end{eduSolutionBox}
...
设点并写距离平方$\;\to\;$第一类：$PA=PB$$\;\to\;$第二类：$PA=AB$$\;\to\;$第三类：$PB=AB$\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
等腰条件怎样不漏分类？\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{先列三类}{题目只说等腰，没有指定哪两边相等，就要三类都列。}
        \eduSideMistake{别默认左右对称}{$PA=PB$ 只是其中一类，不能代表所有等腰情况。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{设点并写距离平方}{设 $P(t,3)$，则 $PA^2=t^2+9$，$PB^2=(t-5)^2+9$，$AB^2=25$。}
      \eduExplainStep{2}{第一类：$PA=PB$}{$t^2+9=(t-5)^2+9$，解得 $t=\dfrac52$，所以 $P\left(\dfrac52,3\right)$。}
      \eduExplainStep{3}{第二类：$PA=AB$}{$t^2+9=25$，解得 $t=4$ 或 $t=-4$，所以 $P(4,3)$ 或 $P(-4,3)$。}
      \eduExplainStep{4}{第三类：$PB=AB$}{$(t-5)^2+9=25$，解得 $t=1$ 或 $t=9$，所以 $P(1,3)$ 或 $P(9,3)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduAuxItem}{思路导航}
设函数图像上的点$\;\to\;$写两个方向向量$\;\to\;$用数量积列垂直方程$\;\to\;$解方程并写点\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
垂直条件怎样转成坐标方程？\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{数量积更稳}{用斜率时可能遇到竖直线；数量积为 $0$ 可以统一处理。}
        \eduSideMistake{答案要代回}{$P(0,0)$ 时一条线竖直、一条线水平，也满足垂直。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{设函数图像上的点}{设 $P(t,t)$。}
      \eduExplainStep{2}{写两个方向向量}{$\overrightarrow{PA}=(-t,2-t)$，$\overrightarrow{PB}=(4-t,-t)$。}
      \eduExplainStep{3}{用数量积列垂直方程}{$(-t)(4-t)+(2-t)(-t)=0$，化简得 $2t^2-6t=0$。}
      \eduExplainStep{4}{解方程并写点}{$t=0$ 或 $t=3$，所以 $P(0,0)$ 或 $P(3,3)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}等腰题先分类，垂直题先确认对象。用距离平方和数量积时，解出来的候选点还要代回轨迹和题意检查。\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 动点在函数上：先设 $P(t,f(t))$。  \item 等腰：先列相等边分类，再写距离平方方程。  \item 垂直：优先写方向向量，令数量积为 $0$。\end{eduBulletList}
\end{eduSummaryBlock}
