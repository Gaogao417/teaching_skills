...
\eduSectionTitle{模型二：坐标轴交点与零坐标条件}


\begin{eduSolutionBox}{核心结论：哪条轴就给哪个零坐标}
  \begingroup
  \setlength{\parskip}{2.5mm}
与 $x$ 轴交点的纵坐标为 $0$，所以令 $y=0$。\par
与 $y$ 轴交点的横坐标为 $0$，所以令 $x=0$。\par
若点 $B(0,b)$ 在 $y=kx+b$ 上，$b$ 就是 $y$ 轴截距。\par
截距点求出后，可以继续作为后续点的横坐标或纵坐标条件。\par
  \endgroup
\end{eduSolutionBox}
...
求 $x$ 轴交点$\;\to\;$求 $y$ 轴交点$\;\to\;$代回检查\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
求一次函数与两条坐标轴的交点坐标。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{别记反}{$x$ 轴上点形如 $(a,0)$，所以是 $y=0$。}
        \eduSideHint{快算 $y$ 轴}{令 $x=0$ 时，$y=kx+b$ 直接变成 $y=b$。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{求 $x$ 轴交点}{令 $y=0$，得 $0=-2x+6$，解得 $x=3$，所以 $A(3,0)$。}
      \eduExplainStep{2}{求 $y$ 轴交点}{令 $x=0$，得 $y=6$，所以 $B(0,6)$。}
      \eduExplainStep{3}{代回检查}{$A,B$ 都满足 $y=-2x+6$，且分别在对应坐标轴上。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduAuxItem}{思路导航}
先求截距点$\;\to\;$把 $A$ 的横坐标传给 $P$$\;\to\;$把 $B$ 的纵坐标传给 $Q$\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
先求截距，再求点 $P,Q$。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{截距不是终点}{本题的 $A,B$ 是后续条件来源，求出后要继续读题。}
        \eduSideMistake{传递方向}{$P$ 继承的是 $A$ 的横坐标，$Q$ 继承的是 $B$ 的纵坐标，不能混用。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先求截距点}{由 $l:y=-2x+6$ 得 $A(3,0)$，$B(0,6)$。}
      \eduExplainStep{2}{把 $A$ 的横坐标传给 $P$}{$x_P=3$，代入 $y=x+1$，得 $y_P=4$，所以 $P(3,4)$。}
      \eduExplainStep{3}{把 $B$ 的纵坐标传给 $Q$}{$y_Q=6$，代入 $y=-x+8$，得 $6=-x+8$，解得 $x=2$，所以 $Q(2,6)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}与 $x$ 轴交点令 $y=0$，与 $y$ 轴交点令 $x=0$。后续题若说“横坐标等于”“纵坐标等于”，要把已求截距点的对应坐标单独标出来。
\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 先写坐标轴点形式：$x$ 轴 $(a,0)$，$y$ 轴 $(0,b)$。  \item 正向求截距用代入 $0$；反向求解析式把截距点代入。  \item 传递型题目先求 $A,B$，再把对应坐标交给新点。\end{eduBulletList}
\end{eduSummaryBlock}
