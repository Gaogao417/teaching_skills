...
求直线 $l$ 与 $y$ 轴的交点坐标。
  ...
  \begin{solutionblock}
    \textbf{答案：}$(0,2)$\par\medskip
    \solstep{写平移后的解析式}{$y=2x-3$ 向上平移 $5$ 个单位，得到 $l:y=2x+2$。}
    \solstep{求与y轴交点}{与 $y$ 轴交点的横坐标为 $0$，代入得 $y=2$，所以交点为 $(0,2)$。}
  \end{solutionblock}
\end{problem}
...
求原函数的解析式。
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$y=-x+3$\par\medskip
    \solstep{写平移后的解析式}{向右平移 $3$ 个单位，把 $x$ 换成 $x-3$，得 $y=-(x-3)+b=-x+b+3$。}
    \solstep{代入经过的点}{点 $A(5,1)$ 在平移后的直线上，所以 $1=-5+b+3$。}
    \solstep{求原函数}{解得 $b=3$，原函数为 $y=-x+3$。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  直线 $l: y=kx+2$ 向下平移 $4$ 个单位后得到直线 $m$。已知 $m$ 经过点 $P(3,1)$，且与 $x$ 轴交于点 $B$。
求点 $B$ 的坐标。

  \answerarea[42mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$B(2,0)$\par\medskip
    \solstep{写平移后的直线}{$l:y=kx+2$ 向下平移 $4$ 个单位，得到 $m:y=kx-2$。}
    \solstep{用经过点求k}{将 $P(3,1)$ 代入 $m$，得 $1=3k-2$，所以 $k=1$。}
    \solstep{求x轴交点}{$m:y=x-2$。与 $x$ 轴交点满足 $y=0$，所以 $0=x-2$，得 $x=2$。}
    \solstep{写点坐标}{因此 $B(2,0)$。}
  \end{solutionblock}
\end{problem}
