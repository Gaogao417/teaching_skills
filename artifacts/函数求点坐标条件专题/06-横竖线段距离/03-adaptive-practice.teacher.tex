...
\section*{函数求点坐标条件专题：横竖线段距离}

\section*{一、核心练习（每题 10 分，共 30 分）}
...
\begin{problem}[points=10]
  ...
  \begin{solutionblock}
    \textbf{答案：}$B(4,3)$\par\medskip
    \solstep{确定纵坐标}{$B$ 与 $A$ 在同一条水平线上，所以 $B$ 的纵坐标也是 $3$。}
    \solstep{确定横坐标}{$B$ 在 $A$ 的右侧，且 $AB=6$，所以横坐标为 $-2+6=4$。}
    \solstep{写坐标}{$B(4,3)$。}
  \end{solutionblock}
\end{problem}
...
若点 $P$ 在第一象限，求点 $P$ 的坐标。
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$P(2,3)$\par\medskip
    \solstep{把距离转成纵坐标}{点到 $x$ 轴的距离是 $|y|$。第一象限中 $y>0$，所以 $y=3$。}
    \solstep{代入函数}{将 $y=3$ 代入 $y=-2x+7$，得 $3=-2x+7$。}
    \solstep{求点坐标}{解得 $x=2$，所以 $P(2,3)$。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  在平面直角坐标系中，学校记为 $S(1,2)$，书店记为 $B(7,2)$。小明从学校出发，先沿水平方向走到书店，再沿竖直方向走到图书馆 $L$。已知书店到图书馆的竖直距离为 $5$，且图书馆在书店上方。
求图书馆 $L$ 的坐标，并求小明一共走了多少个单位长度。

  \answerarea[44mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$L(7,7)$，一共走 $11$ 个单位长度\par\medskip
    \solstep{求L坐标}{$L$ 在 $B$ 的上方，且竖直距离为 $5$，所以横坐标仍为 $7$，纵坐标为 $2+5=7$，即 $L(7,7)$。}
    \solstep{求水平路程}{$S(1,2)$ 到 $B(7,2)$ 是水平移动，距离为 $|7-1|=6$。}
    \solstep{求总路程}{再竖直走 $5$ 个单位，所以一共走 $6+5=11$ 个单位长度。}
  \end{solutionblock}
\end{problem}
