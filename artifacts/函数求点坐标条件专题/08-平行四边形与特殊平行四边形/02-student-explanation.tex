...
\eduSectionTitle{模型八：平行四边形条件怎样给点坐标}


\begin{eduSolutionBox}{核心结论：先判顺序，再用对角线}
  \begingroup
  \setlength{\parskip}{2.5mm}
固定顺序 $ABCD$ 是平行四边形时，$A,C$ 为对角点，$B,D$ 为对角点，所以 $D=A+C-B$。\par
只说 $A,B,C,D$ 构成平行四边形时，三种候选第四点都要讨论：$A+B-C$，$A+C-B$，$B+C-A$。\par
矩形在平行四边形基础上筛垂直，菱形筛邻边相等，正方形同时满足两者。\par
  \endgroup
\end{eduSolutionBox}
...
已知 $A(1,2)$，$B(5,2)$，$C(7,5)$。
\begin{enumerate}[label=(\arabic*)]
  \item 若四边形 $ABCD$ 是平行四边形，求点 $D$；
  \item 若 $A,B,C,D$ 构成平行四边形，求所有可能的点 $D$。
\end{enumerate}
...
固定顺序求第四点$\;\to\;$构成题：$A,B$ 对角$\;\to\;$构成题：$A,C$ 对角$\;\to\;$构成题：$A,D$ 对角\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
比较固定顺序和无序构成的平行四边形求点。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{先读题面}{写出 $ABCD$ 时顺序固定；只说“构成”时顺序没有固定。}
        \eduSideMistake{构成题要三类}{无序构成不能只套 $D=A+C-B$。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{固定顺序求第四点}{$ABCD$ 中 $A,C$ 对角，所以 $D=A+C-B=(1+7-5,2+5-2)=(3,5)$。}
      \eduExplainStep{2}{构成题：$A,B$ 对角}{$D=A+B-C=(1+5-7,2+2-5)=(-1,-1)$。}
      \eduExplainStep{3}{构成题：$A,C$ 对角}{$D=A+C-B=(3,5)$。}
      \eduExplainStep{4}{构成题：$A,D$ 对角}{$D=B+C-A=(5+7-1,2+5-2)=(11,5)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduAuxItem}{思路导航}
先统一求 $D$$\;\to\;$矩形筛垂直$\;\to\;$菱形筛等长$\;\to\;$正方形取交集\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
特殊平行四边形怎样从候选点中筛出来？\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{先平行四边形}{先保证每个候选点能构造平行四边形，再检查特殊条件。}
        \eduSideMistake{正方形不是只看直角}{正方形要同时满足矩形和菱形条件。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先统一求 $D$}{固定顺序下 $D=A+C-B$。三点分别对应 $D_1(-3,4)$，$D_2(0,4)$，$D_3(0,5)$。}
      \eduExplainStep{2}{矩形筛垂直}{$AB$ 是水平线段，矩形要求 $BC\perp AB$，所以 $x_C=5$，得到 $C_2,C_3$。}
      \eduExplainStep{3}{菱形筛等长}{$AB=5$，$BC_1=5$，$BC_2=4$，$BC_3=5$，所以 $C_1,C_3$。}
      \eduExplainStep{4}{正方形取交集}{正方形同时是矩形和菱形，所以只有 $C_3(5,5)$，对应 $D_3(0,5)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}平行四边形题最容易错在顺序。先判断固定顺序还是无序构成，再决定是一种公式还是三种候选。\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 固定顺序：先找两组对角点。  \item 无序构成：先列三种对角分类。  \item 特殊类型：在平行四边形候选上继续筛垂直、等长或两者交集。\end{eduBulletList}
\end{eduSummaryBlock}
