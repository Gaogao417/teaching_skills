...
\section*{函数求点坐标条件专题 08 · 平行四边形与特殊平行四边形}

\section*{一、平行四边形条件怎么给坐标}

\needspace{8\baselineskip}

\begin{problem}[points=10]
  已知 $A(1,2)$，$B(5,2)$，$C(7,5)$。
若四边形 $ABCD$ 是平行四边形，求点 $D$ 的坐标。

  \answerarea[34mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$D(3,5)$\par\medskip
    \solstep{写出对角关系}{平行四边形 $ABCD$ 中，$A,C$ 是一组对角顶点，所以 $A+C=B+D$。}
    \solstep{求 $D$}{$D=A+C-B=(1+7-5,2+5-2)=(3,5)$。}
    \solstep{验算}{$AC$ 中点为 $(4,\frac72)$，$BD$ 中点也为 $(4,\frac72)$。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  已知 $A(1,1)$，$C(7,5)$，点 $B$ 在直线 $y=x$ 上，点 $D$ 在直线 $y=2x-5$ 上。
若四边形 $ABCD$ 是平行四边形，求点 $B,D$ 的坐标。

  \answerarea[48mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$B(5,5)$，$D(3,1)$\par\medskip
    \solstep{求已知对角线中点}{$AC$ 的中点是 $\left(\dfrac{1+7}{2},\dfrac{1+5}{2}\right)=(4,3)$。}
    \solstep{设动点}{设 $B(t,t)$。因为 $BD$ 的中点也是 $(4,3)$，所以 $D=(8-t,6-t)$。}
    \solstep{代入 $D$ 的函数条件}{$D$ 在 $y=2x-5$ 上，所以 $6-t=2(8-t)-5$，解得 $t=5$。}
    \solstep{写出坐标}{$B(5,5)$，$D(3,1)$。}
  \end{solutionblock}
\end{problem}
...
中选取点 $C$，并按固定顺序构造平行四边形 $ABCD$。分别求使 $ABCD$ 成为矩形、菱形、正方形的点 $C$ 和对应的点 $D$。
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}矩形：$C_2(5,4),D_2(0,4)$ 和 $C_3(5,5),D_3(0,5)$；菱形：$C_1(2,4),D_1(-3,4)$ 和 $C_3(5,5),D_3(0,5)$；正方形：$C_3(5,5),D_3(0,5)$。\par\medskip
    \solstep{统一求对应 $D$}{固定顺序下 $D=A+C-B$。所以 $C_1$ 对应 $D_1(-3,4)$，$C_2$ 对应 $D_2(0,4)$，$C_3$ 对应 $D_3(0,5)$。}
    \solstep{筛矩形}{$AB$ 是水平线段。要使 $AB\perp BC$，点 $C$ 的横坐标应与 $B$ 相同，即 $x_C=5$，所以 $C_2,C_3$ 都使四边形成为矩形。}
    \solstep{筛菱形}{$AB=5$。$BC_1=\sqrt{(2-5)^2+4^2}=5$，$BC_2=4$，$BC_3=5$，所以 $C_1,C_3$ 使四边形成为菱形。}
    \solstep{筛正方形}{正方形要同时是矩形和菱形，只有 $C_3(5,5)$，对应 $D_3(0,5)$。}
  \end{solutionblock}
\end{problem}
