...
\eduSectionTitle{模型三：两个函数交点与相等关系}


\begin{eduSolutionBox}{核心结论：交点处两个函数值相等}
  \begingroup
  \setlength{\parskip}{2.5mm}
点 $P$ 是 $y=f(x)$ 与 $y=g(x)$ 的交点，说明 $P$ 同时在两条图像上。\par
求交点横坐标时，先令 $f(x)=g(x)$。\par
求出 $x$ 后，代回任意一个函数求 $y$。\par
若不等式大小关系在某个 $x$ 两侧改变，这个 $x$ 通常就是交点横坐标。\par
  \endgroup
\end{eduSolutionBox}
...
一次函数 $y=2x+1$ 与一次函数 $y=-x+7$ 的图像交于点 $P$。求点 $P$ 的坐标。
...
列交点方程$\;\to\;$求横坐标$\;\to\;$求纵坐标并验算\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
求两个一次函数图像的交点坐标。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{同一个点}{交点不是两边各求一个点，而是同一个 $x$ 对应同一个 $y$。}
        \eduSideMistake{先求横坐标}{令两个解析式相等得到的是交点横坐标，不要把它直接当纵坐标。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{列交点方程}{交点处两个函数值相等，所以 $2x+1=-x+7$。}
      \eduExplainStep{2}{求横坐标}{解得 $3x=6$，所以 $x=2$。}
      \eduExplainStep{3}{求纵坐标并验算}{代入 $y=2x+1$ 得 $y=5$；代入 $y=-x+7$ 也得 $5$，所以 $P(2,5)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
设 $f(x)=x+1$，$g(x)=-x+b$。已知当 $x<3$ 时，$f(x)<g(x)$；当 $x>3$ 时，$f(x)>g(x)$。求 $b$ 的值，并求两个函数图像的交点坐标。
...
\begin{eduAuxItem}{思路导航}
确定分界点$\;\to\;$列相等关系求参数$\;\to\;$写交点\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
由不等式分界反求参数和交点。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{分界就是相等}{从 $<$ 变成 $>$ 的位置，正是两个函数图像相交的位置。}
        \eduSideMistake{别只求参数}{题目还要求交点坐标，求出 $b$ 后要写出完整点坐标。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{确定分界点}{大小关系在 $x=3$ 两侧改变，说明 $x=3$ 是两函数值相等的位置。}
      \eduExplainStep{2}{列相等关系求参数}{$f(3)=4$，$g(3)=-3+b$。由 $f(3)=g(3)$ 得 $4=-3+b$，所以 $b=7$。}
      \eduExplainStep{3}{写交点}{交点横坐标为 $3$，纵坐标为 $4$，所以交点为 $(3,4)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}交点题的入口不是分别代入两个函数，而是让两个函数值相等。若题目给的是不等式范围，先找大小关系改变的分界点，再把它当作交点横坐标。
\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 直接求交点：写 $f(x)=g(x)$，解 $x$，代回求 $y$。  \item 已知一个坐标：先用参数少的一条函数定位交点。  \item 图像不等式：交点横坐标是区间分界，端点取舍看 $<,>,\le,\ge$。\end{eduBulletList}
\end{eduSummaryBlock}
