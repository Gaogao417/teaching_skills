...
\begin{problem}[points=10]
  ...
  \begin{solutionblock}
    \textbf{答案：}$P(2,5)$。\par\medskip
    \solstep{列交点方程}{$2x+1=-x+7$。}
    \solstep{求交点}{解得 $x=2$，代入 $y=2x+1$ 得 $y=5$，所以 $P(2,5)$。}
    \solstep{验算}{代入 $y=-x+7$，得 $5$，两式一致。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  一次函数 $y=mx+3$ 与一次函数 $y=-x+9$ 的图像交于点 $P$。已知点 $P$ 的纵坐标为 $5$，求 $m$ 的值和点 $P$ 的坐标。

  \answerarea[46mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$m=\dfrac12$，$P(4,5)$。\par\medskip
    \solstep{定位交点}{点 $P$ 在 $y=-x+9$ 上且纵坐标为 $5$，所以 $5=-x+9$，解得 $x=4$，即 $P(4,5)$。}
    \solstep{反求参数}{$P(4,5)$ 也在 $y=mx+3$ 上，所以 $5=4m+3$，解得 $m=\dfrac12$。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$b=7$，交点为 $(3,4)$。\par\medskip
    \solstep{找分界点}{大小关系在 $x=3$ 两侧改变，所以 $x=3$ 是两函数相等的位置。}
    \solstep{求参数}{$f(3)=4$，$g(3)=-3+b$。由 $f(3)=g(3)$ 得 $4=-3+b$，所以 $b=7$。}
    \solstep{写交点}{交点横坐标为 $3$，纵坐标为 $4$，所以交点为 $(3,4)$。}
  \end{solutionblock}
\end{problem}
