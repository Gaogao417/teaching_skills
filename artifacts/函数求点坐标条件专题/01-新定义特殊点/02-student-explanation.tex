...
\eduSectionTitle{模型一：新定义特殊点与坐标约束}


\begin{eduSolutionBox}{核心结论：先翻译定义，再联立函数}
  \begingroup
  \setlength{\parskip}{2.5mm}
\textbf{新定义不是新公式：} 先把题干中的定义翻译成 $x,y$ 的关系。\par
\textbf{函数条件：} 点 $P(x,y)$ 在 $y=kx+b$ 上，就有 $y=kx+b$。\par
\textbf{筛选检查：} 若有象限、正负、范围条件，最后用它筛掉不合格候选点。\par
\textbf{常见距离：} 点 $(x,y)$ 到 $x$ 轴距离是 $|y|$，到 $y$ 轴距离是 $|x|$。\par
  \endgroup
\end{eduSolutionBox}
...
我们把到 $x$ 轴和到 $y$ 轴距离相等的点叫做“等轴点”。点 $P(x,y)$ 在一次函数 $y=2x-3$ 的图像上，且 $P$ 是第一象限内的等轴点。求点 $P$ 的坐标。
...
翻译等轴点$\;\to\;$联立函数条件$\;\to\;$求坐标并验算\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
求第一象限内等轴点 $P$ 的坐标。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{先读定义}{看到“叫做”类新定义，先问：它限制的是 $x,y$ 的哪一个关系？}
        \eduSideMistake{绝对值别漏}{到坐标轴的距离永远非负，不能把到 $x$ 轴距离写成 $y$。}
        \eduSideHint{象限的作用}{第一象限把 $|x|=|y|$ 简化成 $x=y$；没有象限时要保留分支。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{翻译等轴点}{第一象限内 $x>0,y>0$，由 $|x|=|y|$ 得 $x=y$。}
      \eduExplainStep{2}{联立函数条件}{点在 $y=2x-3$ 上，所以 $y=2x-3$。与 $x=y$ 联立，得 $x=2x-3$。}
      \eduExplainStep{3}{求坐标并验算}{解得 $x=3$，所以 $y=3$，点 $P(3,3)$。代入函数和定义均成立。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
对一次函数 $y=ax+2$，若它的图像上横坐标为 $2$ 的点是等轴点，则称 $a$ 为这个函数的“等轴参数”。求所有等轴参数 $a$，并写出对应点所在的象限。
...
\begin{eduAuxItem}{思路导航}
先表示点$\;\to\;$列定义方程$\;\to\;$分支求参数$\;\to\;$回代判断象限\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
求所有等轴参数 $a$。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{参数题第一步}{先把待研究的点坐标写成含参数的形式，再使用定义。}
        \eduSideMistake{两个分支}{$|2a+2|=2$ 有正负两个分支，只写一个会漏解。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先表示点}{横坐标为 $2$，代入 $y=ax+2$，得到点 $(2,2a+2)$。}
      \eduExplainStep{2}{列定义方程}{等轴点满足 $|2|=|2a+2|$，即 $|2a+2|=2$。}
      \eduExplainStep{3}{分支求参数}{由 $2a+2=2$ 得 $a=0$；由 $2a+2=-2$ 得 $a=-2$。}
      \eduExplainStep{4}{回代判断象限}{$a=0$ 时点为 $(2,2)$，在第一象限；$a=-2$ 时点为 $(2,-2)$，在第四象限。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}“特殊点”这类新定义题最容易错在两处：一是没有把定义翻译成坐标关系，二是绝对值方程只解一个分支。若题目没有给足筛选条件，答案应保留所有候选点。
\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 第一行写定义条件，第二行写函数条件，第三行写筛选条件。  \item 含参题先把点坐标表示出来，再代入定义。  \item 最后必须把候选点代回定义和函数图像验算。\end{eduBulletList}
\end{eduSummaryBlock}
