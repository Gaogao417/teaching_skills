...
\begin{problem}[points=10]
  我们把到 $x$ 轴和到 $y$ 轴距离相等的点叫做“等轴点”。已知点 $P(a,-3)$ 是等轴点，且 $a>0$，求点 $P$ 的坐标。

  \answerarea[36mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$P(3,-3)$。\par\medskip
    \solstep{翻译定义}{点 $P(a,-3)$ 到 $x$ 轴距离为 $3$，到 $y$ 轴距离为 $|a|$。}
    \solstep{筛选符号}{由 $|a|=3$ 得 $a=\pm3$。又 $a>0$，所以 $a=3$，$P(3,-3)$。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  我们仍把到 $x$ 轴和到 $y$ 轴距离相等的点叫做“等轴点”。点 $P(x,y)$ 在一次函数 $y=2x-3$ 的图像上，且 $P$ 是第一象限内的等轴点。求点 $P$ 的坐标。

  \answerarea[40mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$P(3,3)$。\par\medskip
    \solstep{写定义条件}{第一象限内 $x>0,y>0$，等轴点条件 $|x|=|y|$ 可写成 $x=y$。}
    \solstep{联立函数条件}{又 $y=2x-3$，联立得 $x=2x-3$，所以 $x=3$，$y=3$。}
    \solstep{验算}{$P(3,3)$ 在第一象限，且 $3=2\times3-3$，满足题意。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  对一次函数 $y=ax+2$，若它的图像上横坐标为 $2$ 的点是等轴点，则称 $a$ 为这个函数的“等轴参数”。求所有等轴参数 $a$，并分别写出对应点所在的象限。

  \answerarea[46mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$a=0$，对应点 $(2,2)$，在第一象限；$a=-2$，对应点 $(2,-2)$，在第四象限。\par\medskip
    \solstep{表示点}{横坐标为 $2$，代入 $y=ax+2$ 得点 $(2,2a+2)$。}
    \solstep{列定义方程}{等轴点满足 $|2|=|2a+2|$，即 $|2a+2|=2$。}
    \solstep{分类求解}{$2a+2=2$ 得 $a=0$，点 $(2,2)$；$2a+2=-2$ 得 $a=-2$，点 $(2,-2)$。}
    \solstep{判断象限}{$(2,2)$ 在第一象限，$(2,-2)$ 在第四象限。}
  \end{solutionblock}
\end{problem}
...
第 3 题：$a=0$，点 $(2,2)$，第一象限；$a=-2$，点 $(2,-2)$，第四象限。
