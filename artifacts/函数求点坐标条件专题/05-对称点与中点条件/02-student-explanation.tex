...
\eduSectionTitle{模型五：对称与中点把条件翻译成坐标}


\begin{eduSolutionBox}{核心结论：横纵坐标分开处理}
  \begingroup
  \setlength{\parskip}{2.5mm}
关于 $x$ 轴对称：$(x,y)\to(x,-y)$。\par
关于 $y$ 轴对称：$(x,y)\to(-x,y)$。\par
关于原点对称：$(x,y)\to(-x,-y)$。\par
中点 $M$ 的坐标满足 $x_M=\dfrac{x_1+x_2}{2}$，$y_M=\dfrac{y_1+y_2}{2}$。\par
  \endgroup
\end{eduSolutionBox}
...
点 $A(3,-4)$ 关于 $y$ 轴的对称点为 $B$，点 $B$ 关于原点的对称点为 $C$。
求点 $B$、$C$ 的坐标。
...
关于 $y$ 轴求 $B$$\;\to\;$关于原点求 $C$\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 1}
连续使用对称条件求点坐标。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{不要背乱}{关于 $y$ 轴对称时，点左右翻过去，所以变横坐标。}
        \eduSideMistake{第二步的源点}{求 $C$ 时源点是 $B$，不是最初的 $A$。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{关于 $y$ 轴求 $B$}{关于 $y$ 轴对称，横坐标相反，纵坐标不变，所以 $B(-3,-4)$。}
      \eduExplainStep{2}{关于原点求 $C$}{关于原点对称，横、纵坐标都相反，所以 $C(3,4)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
点 $A(a,4)$ 在一次函数 $y=2x-2$ 的图像上。点 $B$ 与点 $A$ 关于 $x$ 轴对称，点 $C$ 是 $A$、$B$ 的中点。
求点 $C$ 的坐标。
...
\begin{eduAuxItem}{思路导航}
先由函数求 $A$$\;\to\;$再求对称点 $B$$\;\to\;$最后求中点 $C$\end{eduAuxItem}




\begin{eduSubQuestionItem}{例题 2}
先由函数确定源点，再接对称与中点。\end{eduSubQuestionItem}

\begin{eduDualExplain}
  \begin{eduExplainLeft}
    \eduColumnTitle{\eduIconBulb}{小贴士}
        \eduSideHint{条件顺序}{$A$ 还没确定时，不能直接求 $B$ 和 $C$。}
        \eduSideHint{中点看平均}{求中点时横坐标平均、纵坐标平均，两个方向分开算。}
  \end{eduExplainLeft}

  \begin{eduExplainRight}
    \eduColumnTitle{\eduIconBook}{解答}
      \eduExplainStep{1}{先由函数求 $A$}{将 $A(a,4)$ 代入 $y=2x-2$，得 $4=2a-2$，所以 $a=3$，$A(3,4)$。}
      \eduExplainStep{2}{再求对称点 $B$}{关于 $x$ 轴对称，横坐标不变，纵坐标相反，所以 $B(3,-4)$。}
      \eduExplainStep{3}{最后求中点 $C$}{$C\left(\dfrac{3+3}{2},\dfrac{4+(-4)}{2}\right)=(3,0)$。}
  \end{eduExplainRight}
\end{eduDualExplain}
...
\begin{eduMistakeBox}
\eduSubTitle{易错提醒}对称题先问“关于谁对称”：关于 $x$ 轴变纵坐标，关于 $y$ 轴变横坐标，关于原点两个都变。中点反求端点时要列平均关系，不能把中点坐标再除以 $2$。
\end{eduMistakeBox}
...
\begin{eduBulletList}
  \item 先确定源点，再处理对称或中点。  \item 横坐标关系和纵坐标关系分开写。  \item 最后用变号规则或平均关系回代检查。\end{eduBulletList}
\end{eduSummaryBlock}
