...
求点 $B$、$C$ 的坐标。
  ...
  \begin{solutionblock}
    \textbf{答案：}$B(-3,-4)$，$C(3,4)$\par\medskip
    \solstep{求B}{$A(3,-4)$ 关于 $y$ 轴对称，得到 $B(-3,-4)$。}
    \solstep{求C}{$B(-3,-4)$ 关于原点对称，得到 $C(3,4)$。}
  \end{solutionblock}
\end{problem}
...
\begin{problem}[points=10]
  已知点 $A(-2,5)$，点 $B$ 与点 $A$ 的中点是 $M(1,2)$。
求点 $B$ 的坐标。

  \answerarea[35mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$B(4,-1)$\par\medskip
    \solstep{设未知点}{设 $B(x,y)$。}
    \solstep{列中点公式}{$M(1,2)$ 是 $A(-2,5)$ 与 $B(x,y)$ 的中点，所以 $\dfrac{-2+x}{2}=1$，$\dfrac{5+y}{2}=2$。}
    \solstep{解坐标}{解得 $x=4$，$y=-1$，故 $B(4,-1)$。}
  \end{solutionblock}
\end{problem}
...
求点 $C$ 的坐标。
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$C(3,0)$\par\medskip
    \solstep{用函数条件求A}{点 $A(a,4)$ 在 $y=2x-2$ 上，代入得 $4=2a-2$，所以 $a=3$，$A(3,4)$。}
    \solstep{求对称点B}{关于 $x$ 轴对称，横坐标不变，纵坐标相反，所以 $B(3,-4)$。}
    \solstep{求中点C}{$C\left(\dfrac{3+3}{2},\dfrac{4+(-4)}{2}\right)=(3,0)$。}
  \end{solutionblock}
\end{problem}
