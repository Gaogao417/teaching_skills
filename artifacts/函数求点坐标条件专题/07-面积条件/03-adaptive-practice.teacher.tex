...
\section*{函数求点坐标条件专题 07 · 面积条件}

\section*{一、面积条件怎么变成坐标方程}

\needspace{8\baselineskip}

\begin{problem}[points=10]
  已知 $A(2,0)$，$B(8,0)$，点 $P$ 在 $y$ 轴上。
若 $\triangle ABP$ 的面积为 $18$，求点 $P$ 的坐标。

  \answerarea[34mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$P(0,6)$ 或 $P(0,-6)$\par\medskip
    \solstep{确定底和高}{$AB=8-2=6$。点 $P$ 在 $y$ 轴上，设 $P(0,t)$，则到 $x$ 轴的距离为 $|t|$。}
    \solstep{列面积方程}{$\dfrac12\times6\times|t|=18$，所以 $|t|=6$。}
    \solstep{写出两种位置}{$t=6$ 或 $t=-6$，所以 $P(0,6)$ 或 $P(0,-6)$。}
  \end{solutionblock}
\end{problem}
...
若 $\triangle ABP$ 的面积为 $10$，求点 $P$ 的坐标。
  ...
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$P(3,5)$ 或 $P(-2,-5)$\par\medskip
    \solstep{设函数图像上的点}{设 $P(t,2t-1)$。}
    \solstep{列面积方程}{$AB=5-1=4$，点 $P$ 到 $x$ 轴的距离为 $|2t-1|$，所以 $\dfrac12\times4\times|2t-1|=10$。}
    \solstep{解绝对值方程}{$|2t-1|=5$，所以 $2t-1=5$ 或 $2t-1=-5$，解得 $t=3$ 或 $t=-2$。}
    \solstep{回到坐标}{当 $t=3$ 时 $P(3,5)$；当 $t=-2$ 时 $P(-2,-5)$。}
  \end{solutionblock}
\end{problem}
...
若 $\triangle ABP$ 与 $\triangle ABC$ 的面积相等，求所有可能的点 $P$，并说明这些点分别在直线 $AB$ 的哪一侧。

  \answerarea[48mm]
  \setcounter{solstep}{0}
  \begin{solutionblock}
    \textbf{答案：}$P(6,4)$ 或 $P(-2,-4)$；$P(6,4)$ 在 $AB$ 上方，$P(-2,-4)$ 在 $AB$ 下方。\par\medskip
    \solstep{求原三角形面积}{$AB=6$，点 $C(2,4)$ 到 $AB$ 所在的 $x$ 轴距离为 $4$，所以 $S_{\triangle ABC}=\dfrac12\times6\times4=12$。}
    \solstep{把等面积转成距离}{设 $P(t,t-2)$，则 $\dfrac12\times6\times|t-2|=12$，所以 $|t-2|=4$。}
    \solstep{分类求点}{$t-2=4$ 得 $t=6$，$P(6,4)$；$t-2=-4$ 得 $t=-2$，$P(-2,-4)$。}
    \solstep{说明位置}{$P(6,4)$ 的纵坐标为正，在 $AB$ 上方；$P(-2,-4)$ 的纵坐标为负，在 $AB$ 下方。}
  \end{solutionblock}
\end{problem}
