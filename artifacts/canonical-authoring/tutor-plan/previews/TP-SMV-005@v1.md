...
- 提示 L1：先别急着算——这一步的关键是「角平分线给第一组角」。说说你打算怎么找？
- 提示 L2：提示：先写出这一步要用到的已知条件，再看它们如何组合出「角平分线给第一组角」。
- 确认探针：快速确认：角平分线给第一组角——你能指出它在图或题干中对应的具体对象吗？
- Skill 标注 SKILL-SMV-008：select 类证据在 recognize/plan 间共享（capability-skill-map gap note）；以本 checkpoint 语境（角平分线给第一组角）作为该 skill 的低置信线索。（证据 TA-SMV-017@v1#S1）
...
- 提示 L1：先别急着算——这一步的关键是「等腰外角配第二组角」。说说你打算怎么找？
- 提示 L2：提示：先写出这一步要用到的已知条件，再看它们如何组合出「等腰外角配第二组角」。
- 确认探针：快速确认：等腰外角配第二组角——你能指出它在图或题干中对应的具体对象吗？
- Skill 标注 SKILL-SMV-002：该 checkpoint 对应教学步骤「等腰外角配第二组角」，预期学生能学生能用「等腰 + 外角」补出第二组角。；此处证据可直接支持该 skill 的判定。（证据 TA-SMV-017@v1#S2）

### CP3
- 预期推理：学生能写明判定依据（AA）并核对顶点对应顺序。
- 讲解（explanation）：两组对应角相等，由 AA 判定证得 $\triangle CEA\sim\triangle CDB$。
- 提示 L1：先别急着算——这一步的关键是「AA 判定收尾」。说说你打算怎么找？
- 提示 L2：提示：先写出这一步要用到的已知条件，再看它们如何组合出「AA 判定收尾」。
- 未锚定原因：v2 skill_ids 仅作 provisional hint：SKILL-SMV-009 未在此节点锚定（已在他处使用或粗粒度语境不足）

## Part 2（approach TA-SMV-018@v1）
...
- 提示 L1：先别急着算——这一步的关键是「平行给新等角」。说说你打算怎么找？
- 提示 L2：提示：先写出这一步要用到的已知条件，再看它们如何组合出「平行给新等角」。
- 确认探针：快速确认：平行给新等角——你能指出它在图或题干中对应的具体对象吗？
- Skill 标注 SKILL-SMV-005：该 checkpoint 对应教学步骤「平行给新等角」，预期学生能学生能指出平行线截哪两条线产生这对内错角。；此处证据可直接支持该 skill 的判定。（证据 TA-SMV-018@v1#S1）
...
- 提示 L1：先别急着算——这一步的关键是「外角和分解 + 公共角配相似」。说说你打算怎么找？
- 提示 L2：提示：先写出这一步要用到的已知条件，再看它们如何组合出「外角和分解 + 公共角配相似」。
...
- 提示 L1：先别急着算——这一步的关键是「比例转移收尾」。说说你打算怎么找？
- 提示 L2：提示：先写出这一步要用到的已知条件，再看它们如何组合出「比例转移收尾」。
- 确认探针：快速确认：比例转移收尾——你能指出它在图或题干中对应的具体对象吗？
- Skill 标注 SKILL-SMV-007：该 checkpoint 对应教学步骤「比例转移收尾」，预期学生能学生能规划「先相似比、再平行比」的转移顺序，不硬凑。；此处证据可直接支持该 skill 的判定。（证据 TA-SMV-018@v1#S3）
